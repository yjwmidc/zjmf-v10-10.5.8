  ...
  template:
    `
      <div class="com-auto-renew">
        <el-switch :value="isAutoRenew" active-color="#0058ff"
  ...
  methods: {
    ...
    async autoRenewChange (val) {
      try {
        ...
        this.host.allIp = (this.host.dedicate_ip + "," + this.host.assign_ip).split(",");
        this.calcTitle = lang.auto_renew_tip1 + (this.isAutoRenew ? lang_obj.auto_renew_tip3 : lang_obj.auto_renew_tip2);
        this.loading = false;
      } catch (error) {
        this.loading = false;
        this.$message.error(error.data.msg);
