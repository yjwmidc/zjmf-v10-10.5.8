...
use gateway\wx_pay\lib\WxPayRefund;
...
class WxPay extends Plugin
{
    ...
    public function WxPayHandle($param)
    {
        ...
        $data = [
            ...
            'trade_type' => 'NATIVE',
            'product_name' => $param['product'][0]??('订单'.$param['out_trade_no']),
    ...
    private function setPay($param)
    {
        ...
        if($result){
            ...
            $response = \QRcode::png($result['code_url'],false,0,4,0); # 这里需要修改扩展库的代码
    ...
    public function WxPayHandleRefund($param)
    {
        ...
        $inputObj->SetTransaction_id($param['transaction_number']);

        $inputObj->SetOut_refund_no($param['out_request_no']);

        $inputObj->SetRefund_fee($param['amount']*100);

        $inputObj->SetOp_user_id($config->GetMerchantId());

        $inputObj->SetTotal_fee($param['total_fee']*100);

        try{
            $result = WxPayApi::refund($config,$inputObj);
            if ($result && $result['result_code']=='SUCCESS'){
                // 退款交易流水号
                return ['status'=>200,'msg'=>'退款成功','data'=>['trade_no'=>$result['refund_id']]];
            }else{
                return ['status'=>400,'msg'=>'退款失败'];
            }
        }catch (\Exception $e){
            if ($e->getMessage()=='curl出错，错误码:58'){
                // 参考文档：https://blog.csdn.net/qq_37170740/article/details/124384455
                $msg = "请到微信商户号后台下载证书，并将证书文件放在‘网站根目录/cert/’目录下";
            }else{
                $msg = $e->getMessage();
            }
