...
class PromoCodeModel extends Model
{
    ...
    {
        $promoCode =  $this->field('id,code,type,value,client_type,client_level,start_time,end_time,max_times,single_user_once,upgrade,host_upgrade,renew,loop,cycle_limit,cycle,notes')
            ->where('delete_time',0)
            ->find($param['id']);
        if (empty($promoCode)){
            return (object)[];
                ...
                if(isset($param['host_id']) && !empty($param['host_id']) && $param['scene']=='new'){
                    ...
                    if(!empty($host) && $host['is_delete'] == 0){
                        ...
                        HostModel::update([
                            ...
                            'renew_amount' => $PromoCodeModel['loop']==1 ? (($host['renew_amount']-$resultRenew['data']['discount']??0)>0 ? ($host['renew_amount']-$resultRenew['data']['discount']??0) : 0) : $host['renew_amount'],
                        ], ['id' => $param['host_id']]);
                    }
                }
    ...
    public function afterOrderCreate($param)
    {
        ...
        foreach ($orderItems as $orderItem){
            ...
            if ($result['status']==200){
                ...
                if (($promoCodeExist && $orderItem['type']=='upgrade') || (!empty($promoCodeParam) || !empty($hostPromoCode[$orderItem['host_id']]??"")) || ($promoCodeExist && isset($result['data']['loop']) && $result['data']['loop']==1 && $orderItem['type']=='renew' && $host['billing_cycle_name']!=$renew['new_billing_cycle'])){
