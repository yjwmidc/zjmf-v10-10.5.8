 ...
 * @title 优惠码插件(基础版)
 * @desc 优惠码插件(基础版)
 * @use addon\promo_code\controller\AdminIndexController
 */
class AdminIndexController extends PluginAdminBaseController
{
     ...
     * @param string orderby - 排序 id,code
     ...
     * @return string promo_code.client_type - 适用客户:all不限,new无产品用户,old用户必须存在激活中的产品,not_have_client_level未拥有指定用户等级
     * @return array promo_code.client_level - 用户等级ID,为空数组代表所有等级
     ...
     * @param string client_type - 适用客户:all不限,new无产品用户,old用户必须存在激活中的产品,not_have_client_level未拥有指定用户等级 required
     ...
     * @param int upgrade - 升降级:0关闭,1开启,仅百分比和免费支持开启 required
     * @param int host_upgrade - 升降级商品配置:0关闭,1开启,仅百分比支持开启 required
     * @param int renew - 续费:0关闭,1开启,仅百分比和免费支持开启 required
     * @param int loop - 循环优惠:0关闭,1开启,仅百分比支持开启 required
    ...
    public function delete()
    {
        ...
        $result = $PromoCodeModel->deletePromoCode($param);
     ...
     * @author theworld
     * @version v1
     * @url /admin/v1/promo_code/status
     * @method  PUT
     * @param array id - 优惠码ID required
