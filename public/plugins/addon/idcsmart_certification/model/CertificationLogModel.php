...
class CertificationLogModel extends Model
{
    ...
    public function certificationList($param)
    {
        ...
        $where = function (Query $query) use($param,$ids) {
            if(!empty($param['keywords'])){
                $query->where('cl.id|cl.card_name|cl.company|c.username', 'like', "%{$param['keywords']}%");
            }

            if (!empty($param['status'])){
                $query->whereIn('cl.status',$param['status']);
            }
        ...
        $logs = $this->alias('cl')
            ->field('cl.id,cl.card_name,cl.company,cl.type,cl.status,cl.create_time,c.username,cl.client_id,cl.auth_fail,cl.company_organ_code,cl.certify_id')
    ...
    public function certificationIndex($param)
    {
        ...
        $log = $this->alias('cl')
            ->field('cl.id,c.username,cl.company,cl.create_time,p.title,cl.card_name,cl.card_type,cl.card_number,cl.img,cl.company_organ_code,cl.type,cl.plugin_name,cl.custom_fields_json,cl.certify_id')
        ...
        if (!empty($log['img'])){
            ...
            // 特殊处理
            if (!empty($customFields['custom_fields3'][0])){
                array_unshift($imgs,$customFields['custom_fields3'][0]);
            }
            if (!empty($customFields['custom_fields4'][0])){
                array_unshift($imgs,$customFields['custom_fields4'][0]);
            }

            $certificationUrl = IdcsmartCertificationLogic::getDefaultConfig('certification_upload_url');

            $tmp  = [];
            foreach ($imgs as $img){
                //$imgUrl = $certificationUrl . $img;
                $tmp[] = getOssUrl([
                    'file_path' => $certificationUrl,
                    'file_name' => $img
                ])['url'];
            }
        ...
        try{
            ...
            if (IdcsmartCertificationLogic::getDefaultConfig('certification_update_client_name')){
                if ($log['type']==1){
                    ...
                    $update = [
                        'username' => $log['company'],
                        'update_time' => time()
                    ];
                }
                $ClientModel = new ClientModel();
                $ClientModel->where('id',$log['client_id'])->update($update);
            }
            ...
            if(IdcsmartCertificationLogic::getDefaultConfig('certification_notice')){
                system_notice([
                    ...
                    'email_description' => '实名认证通过,发送邮件',
                    'sms_description' => '实名认证通过,发送短信',
                    'task_data' => [
                        'client_id'=> $clientId,//客户ID
                        'template_param'=>[
                            'certification_pass_time'   => date('Y-m-d H:i:s'),
                        ],
                    ],
                ]);
            }
    ...
    public function reject($param)
    {
        ...
        system_notice([
            ...
            'email_description' => '实名认证拒绝,发送邮件',
            'sms_description' => '实名认证拒绝,发送短信',
            'task_data' => [
                'client_id'=>$clientId,//客户ID
            ],
        ]);
    ...
    public function certificationInfo($param)
    {
        ...
        $configuration = [
            ...
            'certification_company_open' => $config['certification_company_open']??0,
        ...
        $person = $CertificationPersonModel->alias('cp')
            ->field('c.username,c.company,cp.card_name,cp.card_number,cp.status,cp.create_time,cp.certify_id,cp.auth_fail')
        ...
        $company = $CertificationCompanyModel->alias('cc')
            ->field('c.username,c.company,cc.card_name,cc.card_number,cc.company as certification_company,cc.company_organ_code,cc.status,cc.create_time,cc.certify_id,cc.auth_fail')
    ...
    public function certificationPerson($param)
    {
        ...
        try{
            ...
            if (!$this->isAdmin){
                ...
                $IdcsmartCertificationLogic->checkPay([
                    'type' => 'person',
                    'name' => $param['plugin_name'],
                    'client_id' => $clientId
                ]);
            }
            ...
            $order = $OrderItemModel->alias('oi')
                ...
                ->where('oi.type','certification'.'|'.$param['plugin_name'])
    ...
    public function certificationCompany($param)
    {
        ...
        try{
            ...
            if (isset($config['certification_company_open']) && $config['certification_company_open']==0){
                throw new \Exception(lang_plugins("certification_company_not_support"));
            }
            ...
            # 验证身份证号是否被其他人使用
            if (isset($param['custom_fields']['card'])){ // 这里对身份证号做统一处理，未定义身份证号自定义字段card的，不处理
                $param['card_number'] = $param['custom_fields']['card'];
            }
            $this->checkOtherClientUsed($param['card_number'],$clientId,'company');
            $this->checkOtherCompanyOrganCodeUsed($param['company_organ_code'],$clientId);
            # 验证是否企业认证
            # $this->checkCompanyCertification($clientId);
            # 验证自定义字段
                ...
                foreach ($customFields as $key=>$customField){
                    if ($customField['type'] == 'file'){
                        if (!empty($tmp['custom_fields' . ($key+1)])){
                            $customFieldData[$customField['field']] = getOssUrl([
                                'file_path' => $config['certification_upload_url'],
                                'file_name' => $tmp['custom_fields' . ($key+1)]
                            ])['url'];//;$config['certification_upload_url'] . $tmp['custom_fields' . ($key+1)];
                        }else{
        ...
        if ($status==1){
            ...
            if ($config['certification_update_client_name']){
                if ($certificationLog['type']==1){
                    ...
                    $update = [
                        'username' => $certificationLog['company'],
                        'update_time' => time()
                    ];
                }
                $ClientModel = new ClientModel();
                $ClientModel->where('id',$clientId)->update($update);
            }
    ...
    public function certificationUnsuspend($client_id)
    {
        if ($this->checkCertification($client_id)){
            ...
            $where = function (Query $query) use ($supportOrderRecycleBin) {
    ...
    # 检查客户是否实名认证
    public function certificationDetail($client_id)
    {
        $CertificationPersonModel = new CertificationPersonModel();
    ...
    protected function checkOtherCompanyOrganCodeUsed($company_organ_code,$client_id){
        $CertificationCompanyModel = new CertificationCompanyModel();
        $tmp = $CertificationCompanyModel->where('company_organ_code',$company_organ_code)
            ->where('status','<>',2)
            ->where('client_id','<>',$client_id)
            ->find();
        if (!empty($tmp)){
            throw new \Exception(lang_plugins('certification_company_organ_code_other_client_used'));
        }
     ...
     */
    public function certificationConfig($param)
    {
        $IdcsmartCertificationLogic = new IdcsmartCertificationLogic();
        ...
        return $IdcsmartCertificationLogic->checkPayData($param);
    }
     ...
     */
    public function certificationOrder($param)
    {
        if (!isset($param['name']) || empty($param['name'])){
            return ['status'=>400,'msg'=>lang_plugins('plugin_is_not_exist')];
        }
        if (!isset($param['type']) || empty($param['type']) || !in_array($param['type'],['person','company'])){
...
//            }
            ...
            $data = $IdcsmartCertificationLogic->checkPayData($param);
            // 不需要生产订单：存在订单，并且未支付
            if (!empty($data['order']) && isset($data['order']['status']) && in_array($data['order']['status'], ['Unpaid','WaitUpload','WaitReview','ReviewFail'])){
                throw new \Exception(lang_plugins('certification_order_unpaid'));
            }
            // 不需要生产订单：存在订单，并且已支付(且未提交资料)
            if (!empty($data['order']) && isset($data['order']['status']) && $data['order']['status']=='Paid'){
                throw new \Exception(lang_plugins('certification_order_paid'));
            }
            // 不需要生成订单
            if ($data['pay']==0){
                throw new \Exception(lang_plugins('certification_do_not_need_order'));
            }
            // 生成订单
            ...
            $orderData = [
                ...
                'type'        => 'new',
                'amount'      => $data['amount'],
                'description' => lang_plugins('certification_order_amount_'.$certificationType),
                'items'       => [
                    [
                        'type' => $certificationType . '|' . $param['name'],
                        'client_id' => $clientId,
                        'description' => lang_plugins('certification_order_amount_'.$certificationType),
                    ],
                ],
            ];
            $orderId = $OrderModel->createOrderBase($orderData);
        ...
        return ['status'=>200,'msg'=>lang_plugins('success_message'),'data'=>['order_id'=>$orderId]];
    }
