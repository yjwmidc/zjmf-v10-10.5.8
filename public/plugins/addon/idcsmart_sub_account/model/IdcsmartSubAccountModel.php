        ...
        if(!$hasProject){
            if($param['visible_product']=='module'){
                ...
                if($supportOrderRecycleBin){
                    $host = HostModel::where('client_id', $clientId)->where('is_delete', 0)->column('id');
                }else{
    ...
    public function updateIdcsmartSubAccount($param, $app='home')
    {
        ...
        if(!$hasProject){
            if($param['visible_product']=='module'){
                ...
                $supportOrderRecycleBin = is_numeric(configuration('order_recycle_bin'));
                if($supportOrderRecycleBin){
                    $host = HostModel::where('client_id', $idcsmartSubAccount['parent_id'])->where('is_delete', 0)->column('id');
                }else{
        ...
        if(!empty($subAccount)){
            ...
            if(!$hasProject){
                ...
                if($subAccount['visible_product']=='module'){
                    ...
                    if($supportOrderRecycleBin){
                        $hostId = HostModel::where('client_id', $clientId)->whereIn('id', $subAccount['host_id'])->where('is_delete', 0)->column('id');
                    }else{
