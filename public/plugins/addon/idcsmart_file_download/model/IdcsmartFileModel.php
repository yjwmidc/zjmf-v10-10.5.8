...
class IdcsmartFileModel extends Model
{
     ...
     * @param string param.orderby - 排序 create_time,status
    ...
    public function idcsmartFileList($param, $app = '')
    {
        ...
        $param['orderby'] = isset($param['orderby']) && in_array($param['orderby'], ['create_time','hidden']) ? 'aif.'.$param['orderby'] : '';
        ...
        if($app=='home'){
            ...
            if($supportOrderRecycleBin){
                $hostCount = HostModel::where('status', 'Active')->where('client_id', $clientId)->where('is_delete', 0)->count();
                $productId = HostModel::where('status', 'Active')->where('client_id', $clientId)->where('is_delete', 0)->column('product_id');
            }else{
        ...
        if(!empty($param['orderby'])){
            $list = $query
                ...
                ->select()
                ->toArray();
        }else if(!empty($param['addon_idcsmart_file_folder_id'])){
            $list = $query
                ->order('aif.order', 'asc')
                ->select()
                ->toArray();
    ...
    {
        $idcsmartFile = $this->field('id,addon_idcsmart_file_folder_id,name,filename,filetype,visible_range,hidden,description')->find($id);
        ...
        $idcsmartFile['file_path'] = $fileUpload;
        $idcsmartFile['file_name'] = $idcsmartFile['filename'];
    ...
    public function createIdcsmartFile($param)
    {
        ...
        try {
            ...
            foreach ($param['file'] as $key => $value) {
                // wyh 20240508 改
                $value['filesize'] = filesize(UPLOAD_DEFAULT.$value['filename']);
                $value['filetype'] = pathinfo(UPLOAD_DEFAULT.$value['filename'], PATHINFO_EXTENSION);
                # 移动附件
