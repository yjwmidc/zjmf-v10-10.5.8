...
<div id="content" class="template" v-cloak>
  <t-card class="list-card-container">
    ...
    <div class="order-search-wrapper internal">
      ...
      <t-input v-model="params.keywords" :placeholder="lang.please_search_order" clearable
        @keypress.enter.native="doInternalOrderSearch" @clear="doInternalOrderClear">
        <template #prefix-icon>
          <t-icon name="search" size="20px" @click="doInternalOrderSearch"></t-icon>
        </template>
      </t-input>
    </div>
    <t-table :data="internalOrderData" :columns="internalOrderColumns" :loading="internalOrderTableloading" row-key="id"
      :max-height="tableHeight" size="small" table-layout="fixed" table-content-width="fixed" resizable>
      ...
      <template #priority="slotProps">
        <div v-for="(item,index) in priorityOptions" :key="index">
          <span v-if="item.id === slotProps.row.priority"
            :style="{color: slotProps.row.priority==='high'?'#E34D59':'#333333'}">
            {{ item.name }}
          </span>
        </div>
      </template>
      <template #operation="slotProps">
        <!-- 接收 -->
        <t-button :title="lang.receive" v-if="slotProps.row.status==='Pending'" shape="circle" variant="text"
          @click="internalOrderReceive(slotProps.row)">
          <t-icon name="download" size="small" style="color:#0052D9" />
        </t-button>
        <!-- 回复 -->
        <t-button :title="lang.reply" shape="circle" class="icon-reply" variant="text"
          @click="internalOrderReply(slotProps.row)">
          <!-- <t-icon name="chat" size="small" style="color:#0052D9" /> -->
        </t-button>
        <!-- 转发 -->
        <t-button :title="lang.forward" v-if="slotProps.row.status!=='Pending'" shape="circle" variant="text"
          @click="internalOrderForward(slotProps.row)">
          <t-icon name="rollback" size="small" style="color:#0052D9" />
        </t-button>
        <!-- 已解决-->
        <t-button :title="lang.order_resolved"
          v-if="slotProps.row.status!=='Resolved'&&slotProps.row.status!=='Pending'" shape="circle" variant="text"
          @click="internalOrderResolved(slotProps.row)">
  ...
  <t-dialog :header="lang.order_forward" :footer="false" :visible.sync="forwardDialogVisible" destroy-on-close>
    <t-form :data="forwardFormData" :rules="forwardFormRules" ref="forwardForm" label-align="left" :label-width="80"
      ...
      <t-form-item :label="lang.order_designated_department" name="admin_role_id">
        <t-select v-model="forwardFormData.admin_role_id" @change="departmentChange" clearable filterable
          style="width:100%">
  ...
  <t-dialog :header="lang.order_new_rder" :footer="false" placement="center" width="600px"
    ...
    <t-form :data="addOrderFormData" :rules="addOrderFormRules" ref="addOrderForm" label-align="left" :label-width="80"
      ...
      <!-- 指定部门 -->
      <t-form-item :label="lang.order_designated_department" name="admin_role_id">
        <t-select v-model="addOrderFormData.admin_role_id" @change="departmentChange" clearable filterable
          style="width:100%">
          <t-option v-for="(item, index) in departmentOptions" :value="item.id" :label="item.name" :key="index">
            {{ item.name }}
          </t-option>
      ...
      <t-form-item class="order-upload-wrapper" :label="lang.order_attachment" name="attachment">
        <t-upload theme="custom" v-model="addOrderFormData.attachment" action="http://101.35.248.14/admin/v1/upload"
          :headers="uploadHeaders" :format-response="uploadFormatResponse" show-upload-progress
          @progress="uploadProgress" @success="uploadSuccess" multiple :max="0">
