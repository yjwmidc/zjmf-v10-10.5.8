    ...
    new Vue({
      ...
      methods: {
        ...
        async getPlugin() {
          try {
            ...
            this.searchList =
              this.pluginList =
              this.originList =
                temp.filter((item) => item.status !== 3);
          } catch (error) {}
        },
        // 获取已添加的数据来源
        async getSourceList() {
        ...
        transferData() {
          ...
          this.searchList = this.originList = this.pluginList.filter(
        ...
        delItem(id) {
          ...
          this.searchList = this.originList = this.originList
            .concat(temp)
            .sort((a, b) => {
              return a.id - b.id;
            });
        ...
        // 跳转
        jump() {
          location.href = `withdrawal_create.htm`;
        },
        // 编辑规则详情
        editHandler(row) {
          location.href = `withdrawal_update.htm?id=${row.id}`;
        },
        // 删除规则
        deleteHandler(row) {
          this.delId = row.id;
        ...
        sourceHandel() {
          ...
          this.originList = this.pluginList.filter(
            (item) => !temp.includes(item.name)
          );
          this.checkList = this.pluginList.filter((item) =>
        ...
        // 本地搜索插件
        seacrhPlugin() {
          this.originList = this.searchList.filter(
            (item) => item.title.indexOf(this.pluginKey) !== -1
          );
        },
        clearPluginKey() {
          this.pluginKey = "";
