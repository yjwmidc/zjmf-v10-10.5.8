...
class IdcsmartRefundProductModel extends Model
{
     ...
     * @param string param.rule - 退款规则:Day按天退款,Month按月退款,Ratio按比例退款
     * @param string param.action - 退款后产品操作：Suspend暂停，Terminate删除
     * @param string param.type - 退款类型:Artificial人工，Auto自动
     ...
     * @return string list[].require - 退款要求:First首次订购,Same同类商品首次订购
     * @return int list[].range_control - 是否开启购买后X天内控制:0否默认,1是
     * @return int list[].range - 购买后X天内
     * @return string list[].rule - 退款规则:Day按天退款,Month按月退款,Ratio按比例退款
     * @return string list[].action - 退款后产品操作：Suspend暂停，Terminate删除
     * @return string list[].type - 退款类型:Artificial人工，Auto自动
     */
...
//        }
     ...
     * @author wyh
     * @version v1
     * @param array product_ids - 商品ID required
     ...
     * @param string action - 退款后产品操作：Suspend暂停，Terminate删除
     */
    public function createRefundProduct($param)
    {
        ...
        try{
            foreach ($param['product_ids'] as $productId){
                # 验证商品是否已加至退款商品
                $existRefundProduct = $this->where('product_id',$productId)->find();
                ...
                $refundProduct = $this->create([
                    ...
                    'create_time' => time(),
                    'action' => $param['action']??'Suspend',
                ]);

                $ProductModel = new ProductModel();
                $product = $ProductModel->find($productId);
                active_log(lang_plugins('refund_create_refund_product', ['{admin}'=>'admin#'.get_admin_id().'#'.request()->admin_name.'#','{product}'=>'product#'.$productId.'#'.$product['name'].'#']), 'addon_idcsmart_refund_product', $refundProduct->id);
            }
    ...
    {
        $refundProduct = $this->alias('rp')
            ->field('rp.id,rp.product_id,rp.type,rp.require,rp.range_control,rp.range,rp.rule,rp.ratio_value,p.pay_type,rp.action')
            ->leftJoin('product p','p.id=rp.product_id')
            ->where('rp.id',$id)
            ->find();
