...
const clientOperateVue = new Vue({
  ...
  methods: {
      ...
      try {
        ...
        if (this.lineDetail.ip) {
          if (this.cloudData?.ip_num !== 0) {
            // 初次回填
            this.params.ip_num = this.cloudData.ip_num * 1;
          } else {
            this.params.ip_num =
              this.lineDetail.ip[0]?.value || this.cloudData.ip_num * 1;
          }
    ...
    getCloudDetail() {
      ...
      cloudDetail(params).then((res) => {
        if (res.data.status === 200) {
          ...
          this.osIcon =
            "/plugins/reserver/mf_finance_dcim/template/clientarea/pc/default/img/remf_finance_dcim/" +
            this.osData[0]?.name +
            ".svg";
    ...
    getImage() {
      ...
      image(params).then((res) => {
        if (res.data.status === 200) {
          ...
          this.reinstallData.osGroupId = this.osData[0].id;
          this.osIcon =
            "/plugins/reserver/rewhmcs_dcim/view/img/" +
            this.osData[0].name +
            ".png";
    ...
    subCgBs() {
      ...
      if (!data.name) {
        isPass = false;
        this.errText = lang.common_cloud_text69;
        return false;
      }
      if (!data.disk_id) {
        isPass = false;
