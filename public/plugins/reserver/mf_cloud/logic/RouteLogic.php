...
namespace reserver\mf_cloud\logic;
...
class RouteLogic{
	...
	// 是否是API
	public $isOnlyApi = true;

	// 代理模块
	protected $module = 'mf_cloud';
	...
	public function routeByProduct($id)
	{
		...
		if(empty($upstreamProduct) || $upstreamProduct['res_module'] != $this->module){
	...
	public function routeByHost($id)
	{
        ...
        $upstreamProduct = $UpstreamProductModel->where('product_id',$host['product_id']??0)->where('mode','only_api')->find();
        ...
        $this->isOnlyApi = $upstreamProduct['mode'] == 'only_api';
	}
	...
	{
		$clientId = $this->clientId ?: get_client_id();
		if (request()->is_api){
			$param = request()->param();
