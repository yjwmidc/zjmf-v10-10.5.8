...
class CloudController
{
	...
	public function adminDetail(){
		...
		try{
			...
			// 获取详情
			$res = $RouteLogic->curl( sprintf('console/v1/remf_cloud/%d', $RouteLogic->upstream_host_id), [], 'GET');
			$result['data']['line']['bill_type'] = $res['data']['line']['bill_type'] ?? 'bw';
		...
		try{
			...
			if($result['status'] == 200){
				if(!empty($result['data']['vnc_url_http']) && !empty($result['data']['vnc_url_https'])){
					...
					// 转到当前res模块
					$result['data']['url'] = request()->domain(). '/' . DIR_ADMIN . '/v1/remf_cloud/'.$param['id'].'/vnc?tmp_token='.$cache['token'];
				}
			}
	 ...
	 * @title 获取网络流量
	 * @desc  获取网络流量
	 * @url /admin/v1/remf_cloud/:id/flow
	 * @method  GET
	 * @author hh
	 * @version v1
	 * @param   int id - 产品ID require
