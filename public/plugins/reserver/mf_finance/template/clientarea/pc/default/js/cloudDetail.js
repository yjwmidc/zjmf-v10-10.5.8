  ...
  methods: {
      ...
      if (e.name === "1") {
        ...
        configApi(this.id)
          .then((res) => {
            ...
            const allOptIds = this.upgradeList.map(item => item.id);
            const tempLimit = res.data.data.links.filter(sub => {
      ...
      if (resultLimit.length === 0) {
        ...
        resultArr.forEach(item => {
          ...
          if (curIndex !== -1) {
            ...
            if (rangeArr.includes(curType)) {
              this.$set(this.configoptions[curIndex], 'qty_minimum', chooseArr[0]);
              this.$set(this.configoptions[curIndex], 'qty_maximum', chooseArr[chooseArr.length - 1]);
            ...
            if (systemArr.includes(curType)) {
              ...
              this.$set(this.configoptions[curIndex], 'sub', curSubs);
              this.$set(this.configoptions[curIndex], 'systemArr', tempSystemArr);
    ...
    async getCloudDetail () {
      try {
        ...
        const res = await cloudDetail(params);
        if (res.data.status === 200) {
          ...
          this.isShowPort =
            this.cloudData.custom_field_data.filter(
              (item) => item.fieldname === "端口" || item.fieldname === "port"
            ).length > 0;
          // 升降级展示
          // if (this.cloudData.system_button.upgrade) {
          //   this.isShowProUpdate = !this.cloudData.system_button.upgrade.disabled
          // }
          ...
          this.osSelectData = this.cloudData.cloud_os.filter(
            (item) => item.group === this.osData[0]?.id
          );
    ...
    refundDgClose () { },
    // 删除实例点击
    showRefund (e, remember_operate_password = 0) {
      const params = {
        host_id: this.id,
      };
      ...
      refundPage(params).then((res) => {
        if (res.data.status == 200) {
          ...
          //   return this.$message.warning("不支持退款")
          // }
