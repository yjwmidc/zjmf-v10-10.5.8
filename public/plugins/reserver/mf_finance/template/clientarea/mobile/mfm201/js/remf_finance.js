  ...
  methods: {
    ...
    filterLimit (opt) {
      ...
      if (opt) {
        ...
        const curOptLimit = this.limit.filter(item => item.config_id === opt.id).reduce((all, cur) => {
          all.push(...cur.result.map(sub => sub.config_id));
          return all;
        }, []);
        this.configoptions.filter(item => curOptLimit.includes(item.id)).forEach(item => {
          const index = this.tempOriginData.findIndex(sub => sub.id === item.id);
      ...
      const initLimit = this.limit.filter(item => {
        ...
        const curOpt = this.configoptions.find(sub => sub.id === item.config_id);
        const tempRange = [];
        if (rangeArr.includes(curOpt.option_type)) {
          Object.values(item.sub_id).map(sub => [sub.qty_minimum * 1, sub.qty_maximum * 1]).forEach(sub => {
            tempRange.push(...this.createArr(sub));
          });
        }
        const bol = temp.includes(this.configForm[item.config_id]) || (tempRange || []).includes(this.configForm[item.config_id]);
    ...
    changeOptItem (data) {
      ...
      const resultLimit = filterLimit.filter(sub => {
        ...
        let tempRange = [];
        const type = this.tempOriginData.find(option => option.id === sub.config_id).option_type;
        if (rangeArr.includes(type)) {
          Object.values(sub.sub_id).map(sub => [sub.qty_minimum * 1, sub.qty_maximum * 1]).forEach(sub => {
            tempRange.push(...this.createArr(sub));
          });
        }
        const bol = temp.includes(this.configForm[data.id]) || (tempRange || []).includes(this.configForm[data.id]);
        return sub.relation === "seq" ? bol : !bol;
      });
      if (resultLimit.length === 0) { } else {
        // 未考虑多个条件对应同一个结果的情况
        const resultArr = resultLimit[0].result;
        resultArr.forEach(item => {
          const curType = this.tempOriginData.find(sub => sub.id === item.config_id).option_type;
          const curIndex = this.tempOriginData.findIndex(sub => sub.id === item.config_id);
          if (curIndex !== -1) {
            let filterArr = [];
            // 取 sub_id 里面的 id
            if (subArr.includes(curType) || switchArr.includes(curType) || areaArr.includes(curType) || systemArr.includes(curType)) {
            ...
            if (item.relation === "sneq") {
              ...
              if (subArr.includes(curType)) {
                allId = this.tempOriginData[curIndex].sub.map(item => item.id);
              }
              if (rangeArr.includes(curType)) {
                this.tempOriginData[curIndex].sub.map(sub => [sub.qty_minimum * 1, sub.qty_maximum * 1]).forEach(sub => {
                  allId.push(...this.createArr(sub));
                });
              }
              if (areaArr.includes(curType)) {
                allId = this.tempOriginData[curIndex].sub.map(item => item.area[0].id);
              }
              if (systemArr.includes(curType)) {
                allId = Object.values(this.tempOriginData[curIndex].sub).reduce((all, cur) => {
            ...
            if (switchArr.includes(curType)) {
              this.configoptions[curIndex].disabled = true;
            }
            /* === 重置可选项及默认值 === */
            let curSubs = [];
            if (subArr.includes(curType)) {
              curSubs = this.tempOriginData[curIndex].sub.filter(sub => chooseArr.includes(sub.id));
              this.configoptions[curIndex].sub = curSubs;
            }
            if (rangeArr.includes(curType)) {
              this.configoptions[curIndex].qty_minimum = chooseArr[0];
              this.configoptions[curIndex].qty_maximum = chooseArr[chooseArr.length - 1];
              this.configoptions[curIndex].qty_range = chooseArr;
            ...
            if (systemArr.includes(curType)) {
              curSubs = Object.keys(this.tempOriginData[curIndex].sub).reduce((all, cur) => {
                const temp = this.tempOriginData[curIndex].sub[cur].child.filter(item => chooseArr.includes(item.id));
                if (temp.length > 0) {
                  all[cur] = {
                    ...data[cur],
                    child: temp
              ...
              this.configoptions[curIndex].sub = curSubs;
              this.configoptions[curIndex].systemArr = tempSystemArr;
              this.curSystem = Object.keys(curSubs)[0];
              this.osIcon =
              "/plugins/reserver/mf_finance/template/clientarea/mobile/mfm201/img/remf_finance/" +
              this.curSystem + ".svg";
             this.imageName = curSubs[this.curSystem].child[0].version;
            }
            ...
            if (!chooseArr.includes(this.configForm[item.config_id])) {
              ...
              if (switchArr.includes(curType)) { // 有限制的时候 是否不可改变
                resetData = chooseArr[0];
              }
              if (areaArr.includes(curType)) {
    ...
    async getConfig () {
      try {
        ...
        const tempLimit = temp.links.filter(sub => {
          sub.result = sub.result.filter(item => allOptIds.includes(item.config_id));
        ...
        this.selectDuration = this.custom_cycles[0];
        ...
        this.configoptions = temp.option
          ...
          .map((item) => {
            ...
            if (numArr.includes(item.option_type)) {
              let qty_range = [];
              item.sub.map(sub => [sub.qty_minimum * 1, sub.qty_maximum * 1]).forEach(sub => {
            ...
            if (item.option_type === 20) {
              ...
              this.cascaderObj[item.id] = { id: item.sub[0]?.id };
              const cid = item.id;
              ...
              this.getCascaderList(cid, sub_id).then((res) => {
                ...
                this.configoptions = JSON.parse(
                  JSON.stringify(this.configoptions)
                );
              });
            }
            return item;
        ...
        const obj = this.configoptions.reduce((all, cur) => {
          if (cur.option_type === 3) {
            ...
            this.osIcon =
              ...
              this.curSystem +
              ".svg";
            this.imageName = cur.sub[this.curSystem].child[0].version;
          } else if (cur.option_type === 12) {
            // 区域
            cur.sub = cur.sub.map((subitem) => {
              subitem.id = subitem.area[0]?.id;
              return subitem;
            });
            all[cur.id] = cur.sub[0].area[0]?.id;
            this.countryName = cur.sub[0]?.option_name;
            this.areImg = cur.sub[0]?.country_code;
          } else {
    ...
    changeArea (item, e) {
      ...
      this.configForm[item.id] = e[0].area[0].id;
      if (this.filterLimit(item)) {
        return this.changeOptItem(item);
      }
      this.changeConfig();
    ...
    handelSelectImg () {
      ...
      this.osIcon =
        ...
        e[0].icon +
        ".svg";
      this.imageName = e[0].version;
      this.configForm[this.curOsItem.id] = e[0].id;
      this.showImgPick = false;
      this.isShowImage = false;
      if (this.filterLimit(this.imageItem)) {
        return this.changeOptItem(this.imageItem);
      }
      this.changeConfig();
    },
    clickOsItem (item) {
    ...
    changeCycle (item, index) {
      if (item.billingcycle === this.cycle) {
        return;
      }
    ...
    verifyCustomFiled () {
      try {
        ...
        if (temp) {
          showtoast(`${temp.field_name}${lang.common_cloud_text295}`);
          return false;
        }
        ...
        if (valItem) {
          showtoast(`${valItem.field_name}${lang.common_cloud_text296}`);
          return false;
        }
        return true;
      } catch (error) {
