...
const clientOperateVue = new Vue({
  ...
  methods: {
    ...
    changeIp(e) {
      this.params.ip_num = e.replace("个", "") * 1;
      setTimeout(() => {
        this.getCycleList();
      }, 0);
    },
    ...
    getCloudDetail() {
      ...
      cloudDetail(params).then((res) => {
        if (res.data.status === 200) {
          ...
          this.supportUpgrade = this.cloudData.server.configoptionsupgrade;
          this.configDetails = [];
          const moreData = [
            {name: lang.cloud_os, sub_name: this.cloudData.server.osname},
            {
              name: `${lang.crack_success_info}`,
              sub_name: this.cloudData.server.crack_success_info,
            },
            {
              name: `${lang.cloud_os}${lang.common_cloud_label14}`,
              sub_name: this.cloudData.server.osusername,
            },
          ];
          const customFile = [];
          const temp = Object.values(this.cloudData.configoptions)
            .reduce((all, cur) => {
              all.push({
                name: cur.name,
                sub_name: cur.value,
            ...
            .concat(moreData, customFile)
            .map((item) => {
              this.$set(item, "show", false);
              return item;
            });
          this.configDetails = temp;
          this.main_ip = this.cloudData.server.main_ip;
          this.netDataList = [
            {
              ipaddress: this.main_ip,
              gateway: "--",
              subnetmask: "--",
            },
          ].concat(this.cloudData.ip);
    ...
    osSelectGroupChange(e) {
      this.osData.map((item) => {
        if (item.id == e) {
          ...
          this.osIcon =
            "/plugins/reserver/whmcs_cloud/template/clientarea/img/rewhmcs_cloud/" +
            item.name +
            ".svg";
      ...
      chartList(params)
        .then((res) => {
          if (res.data.status === 200) {
            ...
            list.forEach((item) => {
              xAxis.push(formateDate(item.time));
    ...
    getMemoryList() {
      ...
      chartList(params)
        .then((res) => {
          if (res.data.status === 200) {
            const temp = res.data;
            const inData = temp.in;
            const outData = temp.out;
            // const list = res.data.data.list
            this.in_info_tip = temp.in_info;
            this.out_info_tip = temp.out_info;
            this.chartUnit = temp.y_unit;
            let xAxis = [];
            let yAxis = [];
            let yAxis2 = [];
            Object.keys(inData).forEach((item) => {
              xAxis.push(formateDate(item * 1));
              yAxis.push(inData[item].toFixed(2));
            });
            Object.keys(outData).forEach((item) => {
              yAxis2.push(outData[item].toFixed(2));
            });
            ...
            const options = {
              title: {
                text: lang.common_cloud_text297,
              },
              color: ["#088A29", "#0101DF"],
              // legend: {
              //   data: [`进方向${temp.y_unit}`,`出方向${temp.y_unit}`]
              // },
              tooltip: {
                show: true,
                trigger: "axis",
              ...
              yAxis: {
                ...
                max: temp.y_max,
                axisLabel: {
                  formatter: `{value}${temp.y_unit}`,
                },
              },
              series: [
                {
                  name: `${lang.common_cloud_text298}(${temp.y_unit})`,
                  data: yAxis,
                  type: "line",
                  areaStyle: {},
                },
                {
                  name: `${lang.common_cloud_text299}(${temp.y_unit})`,
                  data: yAxis2,
                  type: "line",
                  areaStyle: {},
                },
