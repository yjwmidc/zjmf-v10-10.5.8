...
const app2 = Vue.createApp({
  ...
  methods: {
    ...
    // 切换子商品周期
    changeSubCycle(item, ind, index) {
      this.sonCycle[index] = item.id;
      this.sonCurCycle[index] = ind;
      this.changeSonConfig();
    },
    // 切换子商品国家
    changeSubCountry(id, index, subInd) {
      this.sonCurCountry[index].id = subInd;
      this.sonConfigForm[index][id] = this.sonCountry[index][id][subInd][0]?.id;
      this.changeSonConfig();
    },
    // 切换子商品城市
    changeSubCity(e, item, index) {
      this.sonConfigForm[index][item.id] = e[1].id;
      this.sonCountryName = e[0].option_name;
      this.sonCity = e[1].option_name;
      this.changeSonConfig();
    },
    // 切换单击选择
    changeClick(id, el) {
        ...
        if (tabVal === "1") {
          ...
          const obj = this.configoptions.reduce((all, cur) => {
            ...
            if (cur.option_type === "area") {
              ...
              const index = this.filterCountry[cur.id].findIndex((item) =>
                item
                  .reduce((sumC, cc) => {
                    sumC.push(cc.id);
                    return sumC;
              ...
              this.curCountry[cur.id] = index;
              this.countryName = this.calcCountry(
                this.filterCountry[cur.id][index][0].country
              );
              this.city = this.dataCenterList[0].city[0].option_name;
              this.areImg = this.dataCenterList[0].option_name;
            }
      ...
      try {
        ...
        if (tabVal === "1") {
          ...
          result.data.data.son ||
            [].forEach((el, ind) => {
              this.upSon[ind].custom_cycles = el.custom_cycles;
              this.upSon[ind].onetime = el.cycles.onetime;
            });
        } else {
        ...
        if (this.isShowLevel) {
          ...
          const discount = await clientLevelAmount({
            id:
              tabVal === "1"
                ? this.upgradeList[this.selectUpIndex * 1].configoptions &&
                  this.upgradeList[this.selectUpIndex * 1].configoptions[0]
                    ?.product_id
                : this.product_id,
    ...
    // 点击可升级授权
    selectUpItem(e) {
      this.selectUpIndex = this.upgradeList.findIndex(
    ...
    autoRenewDgClose() {
      this.isShowPayMsg = this.isShowPayMsg === 1 ? 0 : 1;
      ...
      try {
        if (this.refundDialog.reason_custom) {
          ...
          if (!this.refundForm.str) {
            return showToast(lang.common_cloud_label44);
          }
        } else {
          if (this.refundForm.arr.length === 0) {
            return showToast(lang.common_cloud_text58);
          }
        }
        if (!this.client_operate_password) {
    ...
    async cancelRefund() {
      ...
      try {
        ...
        showToast(lang.common_cloud_text220);
        this.getRefundInfo();
      } catch (error) {
        showToast(error.data.msg);
      }
    ...
    async getDetail() {
      try {
        ...
        if (this.osData.subs && this.osData.subs[0]) {
          ...
          this.osIcon =
            "/plugins/reserver/idcsmart_common/template/clientarea/mobile/mfm201/img/idcsmart_common/" +
              this.osData.subs[0]?.os.toLowerCase() +
              ".svg" || "";
          this.osName = this.osData.subs[0].version[0].option_name;
