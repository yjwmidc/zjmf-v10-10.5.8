...
class IdcsmartCommon
{
    ...
    {
        return ['display_name'=>'通用产品', 'version'=>'2.3.2'];
    }
  ...
  `upstream_use` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否上下游使用',
  PRIMARY KEY (`id`) USING BTREE,
  KEY `gid` (`gid`)
) ENGINE=INNODB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 ROW_FORMAT=COMPACT;",
...
//        return $res;
    ...
    public function changePackage($params){
        ...
        try{
            ...
            $upstreamProduct = UpstreamProductModel::where('product_id', $params['host']['product_id'])
                ->where('mode','sync')
                ->find();
            if ($upstreamProduct){ // 同步代理模式
                $IdcsmartCommonProductModel = new IdcsmartCommonProductModel();
                $configoptions = $IdcsmartCommonProductModel->exchangeParams($upstreamProduct['product_id'],$params['custom'],'upgrade_config');
                $UpstreamHostModel = new UpstreamHostModel();
                $upstreamHost = $UpstreamHostModel->where('host_id',$params['host']['id'])->find();
                $result = idcsmart_api_curl($upstreamProduct['supplier_id'],"/console/v1/idcsmart_common/host/{$upstreamHost['upstream_host_id']}/upgrade_config",$configoptions);
                if ($result['status']==200){
                    ...
                    $result = idcsmart_api_curl($upstreamProduct['supplier_id'],'/console/v1/credit',$creditData);
                    if ($result['status']==200){
                        $payData = [
                            'id' => $result['data']['id'],
                            'gateway' => 'credit'
                        ];
                        $result = idcsmart_api_curl($upstreamProduct['supplier_id'],'/console/v1/pay',$payData);
                    }
                }
                return $result;
            }else{
            ...
            file_put_contents(IDCSMART_ROOT.'/upgrade_config.txt', $e);
            return ['status'=>200,'msg'=>$e->getMessage()];
        }
    }
...
-----END PUBLIC KEY-----";
            ...
            $res = [
                'vars' => [
                    ...
                    'themes' => $flag?'mobile/default':'mobile/'.configuration('clientarea_theme'),
                    'addons' => $addons['list']
                ],
    ...
    {
        $IdcsmartCommonProductModel = new \server\idcsmart_common\model\IdcsmartCommonProductModel();
        return $IdcsmartCommonProductModel->otherParams($params['product']['id']);
    }

    public function syncOtherParams($params)
    {
        $IdcsmartCommonProductModel = new \server\idcsmart_common\model\IdcsmartCommonProductModel();
        return $IdcsmartCommonProductModel->syncOtherParams($params['product']['id'],$params['param'],$params['other_params'],$params['upstream_product']);
    }

    public function exchangeParams($params)
    {
        $IdcsmartCommonProductModel = new \server\idcsmart_common\model\IdcsmartCommonProductModel();
        return $IdcsmartCommonProductModel->exchangeParams($params['product']['id'],$params['param'],$params['sence']);
    }

//    public function hostOtherParams($params)
//    {
//        $IdcsmartCommonProductModel = new \server\idcsmart_common\model\IdcsmartCommonProductModel();
//        return $IdcsmartCommonProductModel->hostOtherParams($params['host']);
//    }
    ...
    public function durationPresets($params)
    {
        ...
        foreach ($durations as $duration){
            $map[$duration['id']] = $IdcsmartCommonCustomCycleModel->insertCyclePrice([
                'product_id' => $productId,
                'name' => $duration['name'],
                'cycle_time' => $duration['num'],
                'cycle_unit' => $duration['unit'],
                'rel_id' => $productId,
                'type' => 'product',
                'amount' => 0,
            ]);
    ...
    public function durationPresetsDelete($params)
    {
        ...
        $IdcsmartCommonCustomCycleModel->deleteCyclePrice($productId);
