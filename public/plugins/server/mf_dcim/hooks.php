	...
	foreach($host as $v){
        ...
        if($date == date('d') && !$isCreateDay){
            ...
            if(empty($v['reset_flow_time']) || date('Ymd') != date('Ymd', $v['reset_flow_time'])){
                // +-10分钟内清零
                $diffTime = time() - ($v['active_time'] ?: $v['create_time']);
                $diffTime = abs($diffTime%3600);
	...
	try{
		...
		if(!empty($orderItem) && isset($orderItem['host_id']) && isset($orderItem['product_id'])){
			...
			if(!empty($hostLink)){
	            ...
	            if(isset($config['data']['manual_manage']) && $config['data']['manual_resource']==1){
					system_notice([
