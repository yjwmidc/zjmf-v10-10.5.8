            ...
            if(isset($v['result']['model_config'])){
                foreach($v['result']['model_config'] as $kk=>$resultItem){
                    if(isset($resultItem['id'])){
                        ...
                        $v['result']['model_config'][$kk]['name'] = $name;
                    }
                }
            }
            $list[$k]['rule'] = $v['rule'];
    ...
    public function limitRuleResultMatch($rule, $param, $checkRule = [])
    {
        ...
        if(isset($rule['model_config'])){
            if(!empty($checkRule) && !in_array('model_config', $checkRule)){
                ...
                foreach($rule['model_config'] as $resultItem){
                    $match = in_array($param['model_config_id'], $resultItem['id']);
                    if($resultItem['opt'] == 'neq'){
                        $match = !$match;
                    }
                    if($match){
