...
class DataCenterModel extends Model
{
    ...
    public function dataCenterList($param)
    {
        ...
        if(!empty($dataCenter)){
            ...
            $line = LineModel::alias('l')
                ->field('l.id,l.data_center_id,l.name,l.bill_type,l.order,l.hidden')
    ...
    public function createDataCenter($param)
    {
        ...
        $description = lang_plugins('mf_dcim_log_create_data_center_success', [
            '{product}' => 'product#'.$ProductModel->id.'#'.$ProductModel->name.'#',
            '{name}'    => $CountryModel[ $countryName ].$param['city'].$param['area'],
        ]);
    ...
    public function deleteDataCenter($id)
    {
        ...
        $description = lang_plugins('mf_dcim_log_delete_data_center_success', [
            '{product}' => 'product#'.$dataCenter['product_id'].'#'.$productName.'#',
     ...
     * @return  string limit_rule[].rule.ipv4_num.min - IP数量最小值
     * @return  string limit_rule[].rule.ipv4_num.max - IP数量最大值
     ...
     * @return  array limit_rule[].rule.image_group.id - 操作系统分类ID
     * @return  array limit_rule[].rule.model_config.id - 型号配置ID
     * @return  array limit_rule[].rule.duration.id - 周期ID
     */
    public function orderPage($param)
    {
        ...
        $data['model_config'] = ModelConfigModel::field('id,name,cpu,cpu_param,memory,disk,support_optional,optional_only_for_upgrade,leave_memory,max_memory_num,max_disk_num,gpu,max_gpu_num,qty,ontrial,ontrial_price,ontrial_stock_control,ontrial_qty')
            ->where($where)
            ->where('hidden', 0)
            ->withAttr('name', function($value){
                $multiLanguage = hook_one('multi_language', [
            ...
            ->withAttr('gpu', function($value){
                $multiLanguage = hook_one('multi_language', [
                    'replace' => [
                        'gpu' => $value,
                    ],
                ]);
                if(isset($multiLanguage['gpu'])){
                    $value = $multiLanguage['gpu'];
                }
                return $value;
            })
        ...
        foreach($data['model_config'] as $k=>$v){
            if($config['data']['auto_sync_dcim_stock']==1 && $v['qty']==0){
                unset($data['model_config'][$k]);
                continue;
            }
            $data['model_config'][$k]['auto_sync_dcim_stock'] = $config['data']['auto_sync_dcim_stock'];
        ...
        $data['model_config'] = array_values($data['model_config']);
