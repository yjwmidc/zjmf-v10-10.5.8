...
class ModelConfigModel extends Model
{
     ...
     * @param   int param.ontrial - 是否开启试用：0否默认，1是
     * @param   float param.ontrial_price - 试用价格
     * @param   int param.ontrial_stock_control - 试用库存开关：0否，1是
     * @param   int param.ontrial_qty - 试用库存
    ...
    public function modelConfigCreate($param)
    {
        ...
        $this->startTrans();
        try{
        	$modelConfig = $this->create($param, ['product_id','name','group_id','cpu','cpu_param','memory','disk','support_optional','optional_only_for_upgrade','leave_memory','max_memory_num','max_disk_num','create_time','gpu','max_gpu_num','ontrial','ontrial_price','ontrial_stock_control','ontrial_qty']);
    ...
    public function modelConfigList($param)
    {
        ...
        $list = $this
                ->field('id,name,group_id,cpu,cpu_param,memory,disk,gpu,support_optional,hidden,qty,ontrial,ontrial_price,ontrial_stock_control,ontrial_qty')
                ->where($where)
        ...
        $config = $ConfigModel->indexConfig(['product_id'=>$param['product_id'] ]);

        foreach ($list as $k => $v) {
            $list[$k]['auto_sync_dcim_stock'] = $config['data']['auto_sync_dcim_stock'];
        }
    ...
    public function modelConfigUpdate($param)
    {
        ...
        $this->startTrans();
        try{
        	$this->update($param, ['id'=>$modelConfig['id']], ['name','group_id','cpu','cpu_param','memory','disk','support_optional','optional_only_for_upgrade','leave_memory','max_memory_num','max_disk_num','update_time','gpu','max_gpu_num','ontrial','ontrial_price','ontrial_stock_control','ontrial_qty']);
    ...
    public function modelConfigDelete($id)
    {
        ...
        $productName = ProductModel::where('id', $modelConfig['product_id'])->value('name');

        $description = lang_plugins('mf_dcim_log_delete_model_config_success', [
            '{product}' => 'product#'.$modelConfig['product_id'].'#'.$productName.'#',
    ...
    {
        $modelConfig = $this->field('id,product_id,name,group_id,cpu,cpu_param,memory,disk,gpu,support_optional,leave_memory,max_memory_num,max_disk_num,max_gpu_num,optional_only_for_upgrade,ontrial,ontrial_price,ontrial_stock_control,ontrial_qty')->find($id);
        if(empty($modelConfig)){
            return (object)[];
        }
     ...
     * @version v1
     */
    public function syncDcimStock($productId = 0)
    {
        try{
            $where = [];
            if(!empty($productId)){
            ...
            // 获取关联了接口且启用了同步库存的套餐
            $modelConfig = $this->alias('a')
                ->field('a.id,a.group_id,a.product_id,p.rel_id')
                ->leftjoin('product p', 'a.product_id=p.id')
                ->leftjoin('module_mf_dcim_config c', 'a.product_id=c.product_id')
                ->where('p.type', 'server')
                ->where('p.rel_id', '>', 0)
                ->where('c.auto_sync_dcim_stock', 1)
                ->where($where)
                ->select()
                ->toArray();
        }catch(\Exception $e){
        ...
        $server = ServerModel::whereIn('id', array_column($modelConfig, 'rel_id'))->select()->toArray();
        //$serverFreeNum = [];
        $serverGroupFreeNum = [];
        foreach ($server as $v) {
            $v['password'] = aes_password_decode($v['password']);

            $Dcim = new Dcim($v);

            $res = $Dcim->getServerGroupFreeNum();

            if($res['status'] == 200){
                //$serverFreeNum[$v['id']] = array_sum(array_column($res['data'], 'free_num'));
                $serverGroupFreeNum[$v['id']] = array_column($res['data'], 'free_num', 'id');
            }
        }
        ...
        foreach($modelConfig as $v){
            if(!isset($serverGroupFreeNum[$v['rel_id']][$v['group_id']])){
                continue;
            }

            $freeNum = $serverGroupFreeNum[$v['rel_id']][$v['group_id']];

            $updateArr[] = ['id' => $v['id'], 'qty' => $freeNum];

            $productFreeArr[$v['product_id']][$v['group_id']] = $freeNum;
        }

        $limit = 1000;
        $maxPage = ceil(count($updateArr)/$limit);
        for ($i=0; $i < $maxPage; $i++) { 
            $this->saveAll(array_slice($updateArr, $i*$limit, $limit));
        }

        $updateArr = [];
        foreach ($productFreeArr as $k => $v){
            $updateArr[] = ['id' => $k, 'qty' => array_sum($v)];
        }
        ...
        $maxPage = ceil(count($updateArr)/$limit);
        for ($i=0; $i < $maxPage; $i++) {
            $ProductModel->saveAll(array_slice($updateArr, $i*$limit, $limit));
        }
//        foreach ($serverFreeNum as $k => $v) {
//            $ProductModel->where('type', 'server')->where('rel_id', $k)->whereIn('id', array_column($modelConfig, 'product_id'))->update(['qty' => $v]);
//        }
    ...
    public function updateOntrial($param)
    {
        ...
        if($modelConfig['ontrial'] == $param['ontrial']){
            return ['status'=>200, 'msg'=>lang_plugins('success_message')];
        }
        $this->update(['ontrial'=>$param['ontrial']], ['id'=>$modelConfig['id']]);
