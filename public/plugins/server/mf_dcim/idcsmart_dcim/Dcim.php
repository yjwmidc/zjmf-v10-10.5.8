...
class Dcim{
	...
	public function create($params){
		return $this->request('ipmiCreate', $params, 300);
	}
	 ...
	 * @author hh
	 * @version v1
	 * @param   int params.id - 服务器ID require
	 */
	public function resume($params)
	{
		return $this->request('serverResume', $params);
	}

	/**
	 * @时间 2024-12-09
	 * @title 获取服务器分组空闲服务器数量
	 * @desc  获取服务器分组空闲服务器数量
	 * @author theworld
	 * @version v1
	 */
	public function getServerGroupFreeNum()
	{
		return $this->request('getServerGroupFreeNum');
	}
	 ...
	 */
	public function request($path, $data = [], $timeout = NULL){
		$url = $this->url . $path;
		...
		// 调用公共curl方法
		$res = curl($url, $data, $timeout ?? $this->timeout, 'POST');
		if(!empty($res['error'])){
			return ['status'=>400, 'msg'=>lang_plugins('mf_dcim_curl_error', ['{error}'=>$res['error']])];
		}
		if($res['http_code'] != 200){
			return ['status'=>400, 'msg'=>lang_plugins('mf_dcim_curl_http_code_error', ['{http_code}'=>$res['http_code']])];
		}
		$content = json_decode($res['content'] ?? '', true) ?: [];
		if(isset($content['status']) && $content['status'] == 'error'){
			$result = ['status'=>400, 'msg'=>$content['msg'] ?? lang_plugins('mf_dcim_curl_fail') ];
		}else{
			$content['status'] = 200;
