...
const app2 = Vue.createApp({
  ...
  computed: {
    calcRelaTionText() {
      if (this.ipv4DelArr.length > 0) {
        const first = this.ipv4Select.filter((item) => {
          return item.value === this.ipv4Select[0]?.value;
        });
        return this.ipv4DelArr.length > 1
          ? `${first[0]?.text} +${this.ipv4DelArr.length - 1}`
          : first[0]?.text;
      } else {
        return "";
      }
    ...
    calcDelNum () {
      ...
      if (this.params.ip_num * 1 < this.cloudData.ip_num * 1) {
        num = this.cloudData.ip_num * 1 - this.params.ip_num * 1;
      }
      return num;
    },
    passOption() {
    ...
    calcMemoryOption() {
      return (num) => {
        return this.curPackageDetails.optional_memory.map((item) => {
          item.disabled = this.calcConfigDisabled(item, num);
          return item;
        });
      };
    },
    calcDiskOption() {
      return this.curPackageDetails.optional_disk.map((item) => {
        item.disabled = this.calcDiskDisabled(item);
        return item;
      });
    },
    calcGpuOption() {
      return this.curPackageDetails.optional_gpu.map((item) => {
    ...
    calcImageList() {
      ...
      if (this.configLimitList.length > 0) {
        let tempLimit =
          JSON.parse(JSON.stringify(this.configLimitList))
            .reduce((all, cur) => {
              if (cur.result.image) {
                all.push(cur);
              }
    ...
    showFlexIp() {
      let tempIp = [];
      if (this.lineDetail.ip.length === 0) {
        return;
      }
      if (this.lineDetail.ip[0].type === "radio") {
        tempIp = this.lineDetail.ip.reduce((all, cur) => {
          all.push(cur.value);
          return all;
        }, []);
      }
  ...
  watch: {
    "params.bw"(val) {
      if (val) {
        this.params.bw = val === "NC" ? "NC" : val * 1;
      }
    },
    // 获取订购页磁盘的价格/扩容页磁盘的价格
    moreDiskData: {
    ...
    return {
      ...
      powerStatusArr: {
        on: {
          ...
          icon: `/plugins/server/mf_dcim/template/clientarea/mobile/mfm201/img/mf_dcim/on.svg`,
        },
        off: {
          text: lang.common_cloud_text11,
          icon: `/plugins/server/mf_dcim/template/clientarea/mobile/mfm201/img/mf_dcim/off.svg`,
        },
        operating: {
          text: lang.common_cloud_text12,
          icon: `/plugins/server/mf_dcim/template/clientarea/mobile/mfm201/img/mf_dcim/operating.svg`,
        },
        fault: {
          text: lang.common_cloud_text86,
          icon: `/plugins/server/mf_dcim/template/clientarea/mobile/mfm201/img/mf_dcim/fault.svg`,
        },
        suspend: {
          text: lang.common_cloud_text87,
          icon: `/plugins/server/mf_dcim/template/clientarea/mobile/mfm201/img/mf_dcim/suspended.svg`,
        },
      },
      addons_js_arr: [], // 插件列表
  ...
  methods: {
    ...
    async getIpDetail() {
      try {
        ...
        this.ipv4Select = this.allIp.map(item => ({
          value: item,
          text: item
        }));
      } catch (error) {}
        ...
        if (this.lineDetail.ip) {
          ...
          if (this.cloudData?.ip_num !== 0) {
            ...
            if (this.lineDetail.ip[0].type === "radio") {
              ...
              this.params.ip_num =
                this.lineDetail.ip[0].min_value * 1 || temp_ip;
            }
          }
          // this.ipName = this.params.ip_num + '个'
          if (this.lineDetail.ip[0].type !== "radio") {
            this.params.ip_num = this.params.ip_num * 1;
          }
    ...
    changeIp(ipItem) {
      if (
        ...
        this.disabledIp(ipItem.value)
      ) {
        return;
      }

      this.ipName = ipItem.desc;
      // 判读e是否为带NC
      const isNc = this.calcIPList.filter(
        (item) => item.desc === ipItem.desc && item.value === "NC"
      );
      if (isNc.length > 0) {
        this.params.ip_num = "NC";
      } else {
        let temp = ipItem.desc.replace(lang.mf_one, "") * 1;
    ...
    showReinstall() {
      ...
      //   return;
      // }
      if (this.cloudConfig.manual_resource_control_mode === "dcim_client") {
        this.getManualOs();
      } else {
        this.getImage();
      }
    ...
    getManualOs() {
      manualResourceOs(this.id).then((res) => {
        if (res.data.status === 200) {
          ...
          this.osIcon =
            "/plugins/server/mf_dcim/template/clientarea/pc/default/img/mf_dcim/" +
            this.osData[0].name +
            ".svg";
    ...
    osSelectGroupChange(e) {
      this.osIcon =
        "/plugins/server/mf_dcim/template/clientarea/mobile/mfm201/img/mf_dcim/" +
        e[0].icon +
        ".svg";
    ...
    handelSelectImg() {
      ...
      const icon =
        this.calcImageList.filter((item) => item.id === e[0].image_group_id)[0]
          ?.icon || "";
      this.osIcon =
        "/plugins/server/mf_cloud/template/clientarea/mobile/mfm201/img/mf_cloud/" +
        icon +
        ".svg";
    ...
    doPowerOn() {
      ...
      powerOn(params)
        .then((res) => {
          if (res.data.status === 200) {
            showToast(lang.common_cloud_text49);
    ...
    doPowerOff() {
      ...
      powerOff(params)
        .then((res) => {
          if (res.data.status === 200) {
            showToast(lang.common_cloud_text50);
            this.status = "operating";
            this.getCloudStatus();
          }
          this.loading1 = false;
    ...
    doReboot() {
      ...
      reboot(params)
        .then((res) => {
          if (res.data.status === 200) {
            showToast(lang.common_cloud_text51);
            this.status = "operating";
            this.getCloudStatus();
          }
          this.loading1 = false;
    ...
    doHardReboot() {
      ...
      hardReboot(params)
        .then((res) => {
          if (res.data.status === 200) {
            showToast(lang.common_cloud_text52);
            this.status = "operating";
            this.getCloudStatus();
          }
          this.loading1 = false;
    ...
    doHardOff() {
      ...
      hardOff(params)
        .then((res) => {
          if (res.data.status === 200) {
            showToast(lang.common_cloud_text53);
            this.status = "operating";
            this.getCloudStatus();
          }
          this.loading1 = false;
      ...
      upgradePackagePrice(this.id, params)
        .then(async (res) => {
          if (res.data.status == 200) {
            ...
            this.upParams.original_price =
              price * 1 + res.data.data.discount * 1;
            ...
            this.upParams.totalPrice =
              ...
              0
                ...
                : 0;
            this.upgradePriceLoading = false;
            // this.upParams.totalPrice = this.upParams.totalPrice * 1 + res.data.data.discount * 1
          } else {
            this.upParams.original_price = 0;
            this.upParams.clDiscount = 0;
            this.upParams.isUseDiscountCode = false;
    ...
    upgradeSub() {
      ...
      if ((this.ipv4DelArr.length === 0 || this.ipv4DelArr.length === this.calcDelNum)) { }
      else {
        return showNotify({
          type: "warning",
        ...
        (res) => {
          if (res.data.data.ip && res.data.data.ip.length > 0) {
            ...
            if (this.ipValueData.length === 0) {
              showToast(lang.baidu_plugin_text34);
              return;
            }
    ...
    subCgBs() {
      ...
      if (!data.name) {
        isPass = false;
        showToast(lang.appstore_text160);
        return false;
      }
      if (!data.disk_id) {
        isPass = false;
      ...
      chartList(params)
        .then((res) => {
          if (res.data.status === 200) {
            ...
            if (bw2ChartDom) {
              var bwChart = echarts.init(bw2ChartDom);
