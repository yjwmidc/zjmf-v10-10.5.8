...
class CartValidate extends Validate
{
    ...
    protected $scene = [
        // 下单验证
        'cal'               => ['data_center_id','cpu','memory','image_id','system_disk','data_disk','backup_num','snap_num','duration_id','password','ssh_key_id','notes','network_type','bw','recommend_config_id','resource_package_id','flow','ip_num','gpu_num','port','ipv6_num'],
        'upgrade_config'    => ['cpu','memory','bw'],
        'check_disk'        => ['size','disk_type'],
        'all_duration_price'=> ['recommend_config_id','cpu','memory','system_disk','data_disk','line_id','bw','flow','ip_num','gpu_num','backup_num','snap_num','ipv6_num'],
        'upgrade_package_config'    => ['ip_num','ipv6_num','flow','bw','peak_defence'],
    ];
