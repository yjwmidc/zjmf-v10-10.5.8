...
use server\mf_cloud\logic\ImageLogic;

/**
 * @title 魔方云(自定义配置)-操作系统
 * @desc 魔方云(自定义配置)-操作系统
 * @use server\mf_cloud\controller\admin\ImageController
 */
class ImageController
{
	 ...
	 * @title 添加操作系统分类
	 * @desc  添加操作系统分类
	 * @url /admin/v1/mf_cloud/image_group
	 * @method  POST
	 * @author hh
	 * @version v1
	 ...
	 * @title 修改操作系统分类
	 * @desc 修改操作系统分类
	 * @url /admin/v1/mf_cloud/image_group/:id
	 * @method  PUT
	 * @author hh
	 * @version v1
	 ...
	 * @title 镜像分组排序
	 * @desc  镜像分组排序
	 * @url /admin/v1/mf_cloud/image_group/order
	 * @method  PUT
	 * @author hh
	 * @version v1
	 ...
	 * @title 修改操作系统
	 * @desc 修改操作系统
	 * @url /admin/v1/mf_cloud/image/:id
	 * @method  PUT
	 * @author hh
	 * @version v1
	 ...
	 * @title 拉取操作系统
	 * @desc 拉取操作系统
	 * @url /admin/v1/mf_cloud/image/sync
	 * @method  GET
	 * @author hh
	 * @version v1
	 * @param   int product_id - 商品ID require
	 ...
	 * @title 拉取本地操作系统
	 * @desc 拉取本地操作系统
	 * @url /admin/v1/mf_cloud/local_image/sync
	 * @method  GET
	 * @author theworld
	 * @version v1
	 * @param   int product_id - 商品ID require
	 */
	public function localImageSync()
	{
		$param = request()->param();

		$ImageModel = new ImageModel();

		$result = $ImageModel->localImageSync($param['product_id'] ?? 0);
		return json($result);
	}
	 ...
	 * @title 切换是否可用
	 * @desc 切换是否可用
	 * @url /admin/v1/mf_cloud/image/:id/enable
	 * @method  PUT
	 * @author hh
	 * @version v1
     * @param   int id - 操作系统ID require
	 ...
	 * @title 拖动排序
	 * @desc  拖动排序
	 * @url /admin/v1/mf_cloud/image/:id/drag
	 * @method  PUT
	 * @author hh
	 * @version v1
	...
	public function dragToSort()
	{
		...
		$ImageValidate = new ImageValidate();
		if (!$ImageValidate->scene('drag')->check($param)){
            return json(['status' => 400 , 'msg' => lang_plugins($ImageValidate->getError())]);
        }
		$ImageModel = new ImageModel();

		$result = $ImageModel->dragToSort($param);
		return json($result);
	}
