...
class CloudController
{
	 ...
	 * @param   string scene custom 场景(recommend=套餐,custom=自定义)
      ...
      * @return  int data_center[].city[].area[].reommend_config[].ontrial - 是否开启试用：0否默认，1是
      * @return  int data_center[].city[].area[].reommend_config[].ontrial_price - 试用价格
      * @return  int data_center[].city[].area[].reommend_config[].ontrial_stock_control - 试用库存开关：0否，1是
      * @return  int data_center[].city[].area[].reommend_config[].ontrial_qty - 试用库存
      ...
      * @return  string config.manual_resource_control_mode - 手动资源控制方式not_support不支持,cloud_client客户端
     ...
     * @return  int config_limit[].image_group_id - 操作系统分组ID
	...
	*/
     ...
     * @return  int image.image_group_id - 镜像分组ID
     ...
     * @return  int data_disk.count - 数据盘数量
     * @return  int data_disk.total_size - 数据盘总大小
     * @return  int support_apply_for_suspend - 是否支持申请停用(0=否,1=是)
	 */
   ...
   * @param   string client_operate_password - 操作密码,需要验证时传 
     ...
     * @title 批量操作
     * @desc 批量操作
     * @url /console/v1/mf_cloud/batch_operate
     * @method  POST
     * @author theworld
     * @version v1
   ...
   * @param   string client_operate_password - 操作密码,需要验证时传 
	 ...
	 * @version v1
	 * @param   int id - 产品ID require
	 * @param   int resize_data_disk[].id - 磁盘ID
	 ...
	 * @version v1
	 * @param   int id - 产品ID require
	 * @param   int ip_num - 附加IPv4数量 require
     * @param   int ipv6_num - 附加IPv6数量
	 * @param   int is_downstream 0 是否下游发起(0=否,1=是)
     * @return  string price - 价格
     * @return  string description - 生成的订单描述
     * @return  string price_difference - 差价
     ...
     * @param   int ipv6_num - 附加IPv6数量
	 * @param   array ip - 移除的IP地址
	 * @param   array ipv6 - 移除的IPv6地址
	 * @return  string id - 订单ID
	 */
 ...
 //        $VpcNetworkModel = new VpcNetworkModel();
     ...
     * @param   int bw - 带宽
     * @param   int flow - 流量
     * @param   string peak_defence - 防御峰值
     * @param   int is_downstream 0 是否下游发起(0=否,1=是)
     * @return  string price - 价格
     * @return  string description - 描述
     * @return  string price_difference - 差价
     ...
     * @return  int flow[].other_config.out_bw - 出带宽(显示这个)
     ...
     * @return  int order_default_defence - 防御默认选中配置ID
     ...
     * @title 模拟物理机运行
     * @desc  模拟物理机运行
     * @url /console/v1/mf_cloud/:id/simulate_physical_machine
     * @method  POST
     * @author hh
     * @version v1
     * @param   int id - 产品ID require
    ...
    public function simulatePhysicalMachine()
    {
        ...
        $CloudValidate = new CloudValidate();
        if (!$CloudValidate->scene('simulate_physical_machine')->check($param)){
            return json(['status' => 400 , 'msg' => lang_plugins($CloudValidate->getError())]);
        }
        ...
        try{
            ...
            $result = $CloudLogic->simulatePhysicalMachine($param);
            return json($result);
        }catch(\Exception $e){
            return json(['status'=>400, 'msg'=>$e->getMessage()]);
        }
     ...
     * @title IPv6列表
     * @desc  IPv6列表
     * @url /console/v1/mf_cloud/:id/ipv6
     * @method  GET
     * @author hh
     * @version v1
     * @param  int id - 产品ID require
    ...
    public function ipv6List()
	{
		...
		try{
			...
			$data = $CloudLogic->ipv6List($param);
		}catch(\Exception $e){
			$data = [
				'list' => [],
				'count'=> 0,
     ...
     * @title 创建VPC网络
     * @desc  创建VPC网络
     * @url /console/v1/product/:id/mf_cloud/vpc_network
     * @method  POST
     * @author hh
     * @version v1
     * @param   int id - 商品ID require
     ...
     * @return  int id - VPC网络ID
     */
    public function vpcNetworkCreateNew()
    {
        $param = request()->param();

        $VpcNetworkValidate = new VpcNetworkValidate();
        if (!$VpcNetworkValidate->scene('create_new')->check($param)){
            return json(['status' => 400 , 'msg' => lang_plugins($VpcNetworkValidate->getError())]);
        }

        $VpcNetworkModel = new VpcNetworkModel();
        $result = $VpcNetworkModel->vpcNetworkCreateNew($param);
        return json($result);
    }
     ...
     * @param   int vpc_network_id - VPC网络ID require
     */
    public function vpcNetworkDeleteNew()
    {
        $param = request()->param();

        $VpcNetworkModel = new VpcNetworkModel();
        $result = $VpcNetworkModel->vpcNetworkDeleteNew($param);
        return json($result);
    }
     ...
     * @title 获取VPC可分配产品列表
     * @desc  获取VPC可分配产品列表
     * @url /console/v1/mf_cloud/vpc_network/host
     * @method  GET
     * @author hh
     * @version v1
     * @param   int page 1 页数
     ...
     * @return  int count - 总条数
     */
    public function enableVpcHost()
    {
        $param = array_merge(request()->param(), ['page' => request()->page, 'limit' => request()->limit, 'sort' => request()->sort]);

        $HostLinkModel = new HostLinkModel();
        $data = $HostLinkModel->enableVpcHost($param);
     ...
     * @title 下载RDP
     * @desc  下载RDP
     * @url /console/v1/mf_cloud/:id/download_rdp
     * @method  GET
     * @author  hh
     * @version v1
     * @param   int id - 产品ID require
     ...
     * @desc  计算套餐产品配置升级价格
     * @url /console/v1/mf_cloud/:id/package/config/price
     * @method  GET
     * @author hh
     * @version v1
     * @param   int id - 产品ID require
     ...
     * @return  string renew_price_difference - 续费差价
     */
    public function calPackageConfigPrice()
    {
        $param = request()->param();

        $CartValidate = new CartValidate();
        if (!$CartValidate->scene('upgrade_package_config')->check($param)){
            return json(['status' => 400 , 'msg' => lang_plugins($CartValidate->getError())]);
        }
        ...
        try{
            ...
            $result = $CloudLogic->calPackageConfigPrice($param);
            if(isset($result['data']['new_config_data'])) unset($result['data']['new_config_data']);
        }catch(\Exception $e){
            $result = ['status'=>400, 'msg'=>$e->getMessage()];
        }
     ...
     * @author hh
     * @version v1
     * @url  /console/v1/mf_cloud/:id/package/config/order
     * @method  POST
     * @param   int id - 产品ID require
     * @param   int ip_num - IPv4数量
     * @param   int ipv6_num - IPv6数量
    ...
    public function createPackageConfigOrder()
    {
        ...
        try{
            ...
            $result = $CloudLogic->createPackageConfigOrder($param);
        }catch(\Exception $e){
            $result = ['status'=>400, 'msg'=>$e->getMessage()];
        }
        return json($result);
     ...
     * @title 获取升级防御配置
     * @desc  获取升级防御配置
     * @url /console/v1/mf_cloud/:id/upgrade_defence_config
     * @method  GET
     * @author theworld
     * @version v1
     * @param   int id - 产品ID  require
     ...
     * @title 计算升级防御价格
     * @desc  计算升级防御价格
     * @url /console/v1/mf_cloud/:id/upgrade_defence/price
     * @method  GET
     * @author hh
     * @version v1
     * @param   int id - 产品ID require
    ...
    public function calDefencePrice()
    {
        ...
        try{
            ...
            if(isset($result['data']['defence'])) unset($result['data']['defence']);
        }catch(\Exception $e){
            $result = ['status'=>400, 'msg'=>$e->getMessage()];
        }
        return json($result);
     ...
     * @title 生成升级防御订单
     * @desc  生成升级防御订单
     * @url /console/v1/mf_cloud/:id/upgrade_defence/order
     * @method  POST
     * @author theworld
     * @version v1
     * @param   int id - 产品ID require
     ...
     * @return  string data.amount - 订单金额
     */
     ...
     * @title 是否可以续费
     * @desc  是否可以续费
     * @url /console/v1/mf_cloud/:id/whether_renew
     * @method  GET
     * @author hh
     * @version v1
     * @param   int id - 产品ID require
