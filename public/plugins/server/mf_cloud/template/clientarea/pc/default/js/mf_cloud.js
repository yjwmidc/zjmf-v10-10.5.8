...
new Vue({
  ...
  data() {
    return {
      ...
      sliderNum: 0,
      isSync: false, // 本地商品代理
      isEn: localStorage.getItem("lang") === "en-us" ? true : false,
      freeDataRange: [],
    };
  },
  methods: {
    ...
    getGoodsName() {
      productInfo(this.id).then((res) => {
        ...
        this.isSync = res.data.data.product.mode === "sync";
        document.title =
    ...
    handleClick() {
      ...
      this.isHide = true;
      this.curImage = 0;
      this.imageName = this.imageList[0]?.name;
      this.version = this.imageList[0]?.image[0]?.name;
    ...
    // 切换套餐
    async changeRecommend(item, index) {
    ...
    async getCycleList() {
      try {
        ...
        if (this.isInit && this.isUpdate) {
          ...
          const defaultCycle = this.cycleList.filter(
            (item) => item.id !== "ontrial"
          );
