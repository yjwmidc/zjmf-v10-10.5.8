...
new Vue({
      ...
      return (type) => {
        switch (type) {
          ...
          case "line_bw":
            return `${lang.bw}（Mbps）`;
          case "line_ip":
            return `IPv${this.ipType === "ipv4" ? 4 : 6}${lang.auth_num}（${
              lang.one
            }）`;
        }
      };
  ...
  methods: {
    ...
    changeLimitRange(val, name, type, model, index, max) {
      ...
      if (val < min) {
        this.limitForm[model][name][index][type] = min;
      }
      if (val > max) {
        this.limitForm[model][name][index][type] = max;
      }
      if (
        typeof this.limitForm[model][name][index].max !== "object" &&
        this.limitForm[model][name][index].max !== ""
      ) {
        if (type === "min" && val >= this.limitForm[model][name][index].max) {
          this.limitForm[model][name][index].max =
            this.limitForm[model][name][index].min;
        }
        if (type === "max" && val <= this.limitForm[model][name][index].min) {
          this.limitForm[model][name][index].min =
            this.limitForm[model][name][index].max;
        }
      }
    ...
    async handlerStore() {
      try {
        let {id, value, type, price, min_value, max_value, step, other_config} =
          this.calcForm;
        price = price.reduce((all, cur) => {
          cur.price && (all[cur.id] = cur.price);
          return all;
