...
const adminOperateVue = new Vue({
  ...
  data () {
    return {
      ...
      treeProps: {
        keys: {
          value: "id", label: "ip_name", children: "children"
        }
      },
      isSync: false
    };
  ...
  methods: {
    ...
    async getIpList (type) {
      try {
        this.ipOptType = type;
        this.ipForm.ip_id = [];
        this.ipForm.checked = false;
        const res = await getIpList(type, { id: this.id });
        const temp = res.data.data.list.map(item => {
          item.ip_name = `${item.ip_name}(${lang.mf_ip_num}:${item.ip.length})`;
          item.children = item.ip.map(sub => {
            sub.ip_name = sub.ip;
            if (type === 'change' && sub.use) {
              this.ipForm.ip_id.push(sub.id * 1)
            }
            delete sub.ip;
            return sub;
          });
        ...
        this.ipList = temp;
        this.networkType = res.data.data.network_type;
        this.ipModel = true;
        this.ipOptTitle = lang[`mf_${type}_ip`];
      } catch (error) {
        this.$message.error(error.data.msg);
      }
    },
    submitIpOPt ({ validateResult, firstError }) {
      if (validateResult === true) {
        if (this.status === 'on' && this.networkType === 'normal') {
          if (!this.ipForm.checked) {
    ...
    async saveHandleIp (e, remember_operate_password = 0) {
      ...
      try {
        ...
        let params = {
          id: this.id,
          ip_id: this.ipForm.ip_id,
          op: this.ipOptType,
          admin_operate_password,
          admin_operate_methods: "saveHandleIp",
          remember_operate_password,
        };
        const res = await handleIpOpt(this.ipOptType, params).catch((error) => {
    ...
    getDetail () {
      apiOperateDetail(this.id)
        .then((res) => {
          ...
          this.showFlow = res.data.data.line.bill_type === 'flow';
        })
        .catch((err) => {
    ...
    handelClick (item) {
      ...
      } else if (item.op === "change" || item.op === "add" || item.op === "delete") {
        this.getIpList(item.op);
      } else {
