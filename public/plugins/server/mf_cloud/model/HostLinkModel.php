...
class HostLinkModel extends Model
{
    ...
    public function detail($hostId)
    {
        ...
        $data['config'] = ConfigModel::field('reinstall_sms_verify,reset_password_sms_verify,manual_manage')->where('product_id', $host['product_id'])->find() ?? (object)[];
...
//                    }
    ...
    public function suspendAccount($param)
    {
        ...
        if (!empty($hostLink['parent_host_id'])){
            ...
            $orderDefaultDefenceId = str_replace($firewallType."_",'',$configData['line']['order_default_defence']??'');
        ...
        if($custom['type'] == 'buy_image'){
            ...
            if(!empty($custom['ipv6_data'])){
                ...
                if(!empty($custom['remove_ipv6'])){
                    ...
                    // 修改IP数量
                    $res = $IdcsmartCloud->cloudModifyIpv6($id, $custom['ipv6_data']['value']);
                    if($res['status'] == 200){
...
//                }
            ...
            if($oldRcParam['ipv6_num'] != $newRcParam['ipv6_num']){
                $res = $IdcsmartCloud->cloudModifyIpv6($id, $newRcParam['ipv6_num']);
                if($res['status'] == 200){
                    $description[] = lang_plugins('log_mf_cloud_upgrade_ipv6_num_success', [
                        '{hostname}'    => $param['host']['name'],
            ...
            if(isset($newConfigData['ipv6_num'])){
                // 修改IP数量
                $res = $IdcsmartCloud->cloudModifyIpv6($id, $newConfigData['ipv6_num']);
                if($res['status'] == 200){
                    $description[] = lang_plugins('log_mf_cloud_upgrade_ipv6_num_success', [
                        '{hostname}'  => $param['host']['name'],
...
//            }
    ...
    public function hostUpdate($param)
    {
        ...
        if(!empty($hostLink) && !$DownstreamCloudLogic->isDownstream()){
            ...
            if($ipv6_change){
                $res = $IdcsmartCloud->cloudModifyIpv6($id, $moduleAdminField['ipv6_num']);
                if($res['status'] == 200){
    ...
    public function currentConfig($hostId)
    {
        ...
        if($hostLink['recommend_config_id'] > 0){
            ...
            $data['system_disk'] = [
                'size'  => $configData['system_disk']['value'] ?? 0,
            ...
            // 获取当前数据盘
            $disk = DiskModel::where('host_id', $hostId)->where('is_free', 0)->where('type2', 'data')->order('is_free', 0)->select();
            foreach($disk as $v){
                $data['data_disk'][] = [
                    'rel_id'    => $v['rel_id'],
                    'size'      => $v['size'],
    ...
    public function getIp($param, $IdcsmartCloud, $detail = NULL)
    {
        ...
        // 获取默认转发
        if(empty($detail['data']['mainip']) && ($detail['data']['nat_acl_limit'] >= 0 || $detail['data']['nat_web_limit'] >= 0)){
            ...
            if(isset($natAclList['data']['data']) && !empty($natAclList['data']['data'])){
                $detail['data']['mainip'] = $natAclList['data']['nat_host_ip'] . ':' . $natAclList['data']['data'][0]['ext_port'];
            }
        }
    ...
    public function exchangeParams($productId, $param, $sence, $host)
    {
        ...
        switch ($sence){
            case 'create_account':
                ...
                $exchangeParams['security_group_id'] = 0;
