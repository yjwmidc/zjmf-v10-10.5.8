...
class RecommendConfigModel extends Model
{
    ...
    protected $schema = [
        ...
        'traffic_type'      => 'int',
        'upstream_id'       => 'int',
        'due_not_free_gpu'  => 'int',
        'ontrial'           => 'int',
        'ontrial_price'     => 'float',
        'ontrial_stock_control'  => 'int',
        'ontrial_qty'       => 'int',
        // 'ipv4_num_upgrade'  => 'int',
        // 'ipv6_num_upgrade'  => 'int',
        // 'flow_upgrade'      => 'int',
        // 'bw_upgrade'        => 'int',
        // 'defence_upgrade'   => 'int',
    ];
     ...
     * @param   int param.peak_defence - 防御峰值(G)/防御配置ID
     * @param   int param.ip_num - IP数量
     * @param   int param.gpu_num 0 显卡数量
     * @param   int param.ipv6_num 0 IPv6数量
     * @param   int param.in_bw - 流入带宽
     * @param   int param.flow - 流量(GB)
     * @param   int param.traffic_type - 流量计费方向(1=进,2=出,3=进+出)
     * @param   int param.due_not_free_gpu - 计费到期不自动释放GPU(0=否,1=是)
     * @param   int param.ipv4_num_upgrade - 是否支持IPv4数量升降级(0=否,1=是)
     * @param   int param.ipv6_num_upgrade - 是否支持IPv6数量升降级(0=否,1=是)
     * @param   int param.flow_upgrade - 是否支持流量升降级(0=否,1=是),流量线路显示
     * @param   int param.bw_upgrade - 是否支持带宽升降级(0=否,1=是),带宽线路显示
     * @param   int param.defence_upgrade - 是否支持防御峰值升降级(0=否,1=是)
    ...
    public function recommendConfigCreate($param)
    {
        ...
        $param['due_not_free_gpu'] = $param['due_not_free_gpu'] ?? 0;
        // $param['ipv4_num_upgrade'] = $param['ipv4_num_upgrade'] ?? 0;
        // $param['ipv6_num_upgrade'] = $param['ipv6_num_upgrade'] ?? 0;
        // $param['flow_upgrade'] = $param['flow_upgrade'] ?? 0;
        // $param['bw_upgrade'] = $param['bw_upgrade'] ?? 0;
        // $param['defence_upgrade'] = $param['defence_upgrade'] ?? 0;
        $param['upgrade_show'] = 1;
        $param['in_bw'] = $param['in_bw'] ?? '';
        $param['traffic_type'] = $param['traffic_type'] ?? 3;
        if(!is_numeric($param['traffic_type'])){
            $param['traffic_type'] = 3;
        }
        // 带宽线路,不能设置流量
        if($line['bill_type'] == 'bw'){
            $param['flow'] = 0;
            $param['flow_upgrade'] = 0;
        }else{
            $param['bw_upgrade'] = 0;
        }

        $param['ontrial'] = $param['ontrial']??0;
        $param['ontrial_price'] = $param['ontrial_price']??0;
        $param['ontrial_stock_control'] = $param['ontrial_stock_control']??0;
        $param['ontrial_qty'] = $param['ontrial_qty']??0;
        ...
        $this->startTrans();
        try{
            $recommendConfig = $this->create($param, ['product_id','name','description','order','data_center_id','line_id','cpu','memory','system_disk_size','system_disk_type','data_disk_size','data_disk_type','bw','flow','peak_defence','ip_num','create_time','gpu_num','ipv6_num','upgrade_show','in_bw','traffic_type','due_not_free_gpu','ontrial','ontrial_price','ontrial_stock_control','ontrial_qty']);
        ...
        $productName = ProductModel::where('id', $param['product_id'])->value('name');
    ...
    public function recommendConfigList($param)
    {
        ...
        $list = $this
                ->field('id,name,description,order,product_id,data_center_id,upgrade_range,hidden,upgrade_show,ontrial,ontrial_price,ontrial_stock_control,ontrial_qty')
                ->where($where)
    ...
    public function recommendConfigUpdate($param)
    {
        ...
        $this->startTrans();
        try{
            $this->update($param, ['id'=>$recommendConfig['id']], ['name','description','order','data_center_id','line_id','cpu','memory','system_disk_size','system_disk_type','data_disk_size','data_disk_type','bw','flow','peak_defence','ip_num','gpu_num','ipv6_num','in_bw','traffic_type','due_not_free_gpu','ontrial','ontrial_price','ontrial_stock_control','ontrial_qty']);
        ...
        $des = [
            ...
            'in_bw'         => lang_plugins('mf_cloud_line_bw_in_bw'),
            'due_not_free_gpu' => lang_plugins('mf_cloud_due_not_free_gpu'),
            // 'ipv4_num_upgrade' => lang_plugins('mf_cloud_ipv4_num_upgrade'),
            // 'ipv6_num_upgrade' => lang_plugins('mf_cloud_ipv6_num_upgrade'),
            // 'flow_upgrade'  => lang_plugins('mf_cloud_flow_upgrade'),
            // 'bw_upgrade'    => lang_plugins('mf_cloud_bw_upgrade'),
            // 'defence_upgrade'   => lang_plugins('mf_cloud_defence_upgrade'),
        ];
        ...
        $old['due_not_free_gpu'] = $switch[ $old['due_not_free_gpu'] ];
        // $old['ipv4_num_upgrade'] = $switch[ $old['ipv4_num_upgrade'] ];
        // $old['ipv6_num_upgrade'] = $switch[ $old['ipv6_num_upgrade'] ];
        // $old['flow_upgrade'] = $switch[ $old['flow_upgrade'] ];
        // $old['bw_upgrade'] = $switch[ $old['bw_upgrade'] ];
        // $old['defence_upgrade'] = $switch[ $old['defence_upgrade'] ];
        ...
        $param['due_not_free_gpu'] = $switch[ $param['due_not_free_gpu'] ];
        // $param['ipv4_num_upgrade'] = $switch[ $param['ipv4_num_upgrade'] ];
        // $param['ipv6_num_upgrade'] = $switch[ $param['ipv6_num_upgrade'] ];
        // $param['flow_upgrade'] = $switch[ $param['flow_upgrade'] ];
        // $param['bw_upgrade'] = $switch[ $param['bw_upgrade'] ];
        // $param['defence_upgrade'] = $switch[ $param['defence_upgrade'] ];
        ...
        if(!empty($description)){
            ...
            $description = lang_plugins('log_mf_cloud_modify_recommend_config_success', [
                '{product}' => 'product#'.$param['product_id'].'#'.$productName.'#',
                '{detail}'  => $description,
            ]);
            active_log($description, 'product', $param['product_id']);
    ...
    public function recommendConfigDelete($id)
    {
        ...
        $productName = ProductModel::where('id', $recommendConfig['product_id'])->value('name');

        $description = lang_plugins('log_mf_cloud_delete_recommend_config_success', [
            '{product}' => 'product#'.$recommendConfig['product_id'].'#'.$productName.'#',
     ...
     * @return  int list[].in_bw - 流入带宽
     * @return  int list[].traffic_type - 流量计费方向(1=进,2=出,3=进+出)
    ...
    public function getUpgradeRecommendConfig($current_recommend_config = [], $remove_ipv6 = false)
    {
        ...
        if($current_recommend_config['upgrade_range'] == self::UPGRADE_DISABLE){
            ...
            $data = $this
                    ->field('rc.*,dc.gpu_name,l.bill_type')
                    ->alias('rc')
    ...
    public function recommendConfigCheck(&$param)
    {
        ...
        if(empty($dataCenter)){
            ...
            if($param['gpu_num'] > 0){
                // 获取gpu配置
                $option = OptionModel::where('product_id', $dataCenter['product_id'])->where('rel_type', OptionModel::DATA_CENTER_GPU)->where('rel_id', $dataCenter->id)->find();
                if(empty($dataCenter['gpu_name']) || empty($option)){
                    $result['data']['error'][] = [
                        'field' => 'data_center_id',
    ...
    public function formatRecommendConfig($recommend_config)
    {
        ...
        if(!empty($line)){
            ...
            if($line['bill_type'] == 'bw'){
                ...
                if(isset($recommend_config['traffic_type']) && in_array($recommend_config['traffic_type'], [1,2,3])){
                    $data['traffic_type'] = $recommend_config['traffic_type'];
                }
            }
        }
        // 如果套餐设置了流入带宽
        if(isset($recommend_config['in_bw']) && is_numeric($recommend_config['in_bw'])){
            $data['in_bw'] = $recommend_config['in_bw'];
        }
        return $data;
    }
     ...
     * @version v1
     * @param   int param.id - 套餐ID require
     * @param   int param.upgrade_show - 升降级是否显示(0=否,1=是) require
     * @return  int status - 状态(200=成功,400=失败)
     * @return  string msg - 信息
     */
    public function updateUpgradeShow($param)
    {
        $recommendConfig = $this->find($param['id']);
        if(empty($recommendConfig)){
            return ['status'=>400, 'msg'=>lang_plugins('recommend_config_not_found')];
        }
        if($recommendConfig['upgrade_show'] == $param['upgrade_show']){
            return ['status'=>200, 'msg'=>lang_plugins('success_message')];
        }
        $this->update(['upgrade_show'=>$param['upgrade_show']], ['id'=>$recommendConfig['id']]);
        ...
        $description = lang_plugins('log_mf_cloud_recommend_config_update_upgrade_show_success', [
            '{product}' => 'product#'.$recommendConfig['product_id'].'#'.$productName.'#',
            '{name}'    => $recommendConfig['name'],
            '{act}'     => $switch[$param['upgrade_show']],
        ]);
        active_log($description, 'product', $recommendConfig['product_id']);
     ...
     */
    public function updateOntrial($param)
    {
        $param['ontrial'] = $param['ontrial']??0;
        $recommendConfig = $this->find($param['id']);
        if(empty($recommendConfig)){
        ...
        if($recommendConfig['ontrial'] == $param['ontrial']){
            return ['status'=>200, 'msg'=>lang_plugins('success_message')];
        }
        $this->update(['ontrial'=>$param['ontrial']], ['id'=>$recommendConfig['id']]);
        ...
        $description = lang_plugins('log_mf_cloud_recommend_config_update_ontrial_success', [
            '{product}' => 'product#'.$recommendConfig['product_id'].'#'.$productName.'#',
            '{name}'    => $recommendConfig['name'],
            '{act}'     => $switch[$param['ontrial']],
        ]);
        active_log($description, 'product', $recommendConfig['product_id']);
