...
class LineModel extends Model
{
    ...
    public function lineCreate($param)
    {
        ...
        $this->startTrans();
        try{
            $line = $this->create($param, ['data_center_id','name','bill_type','bw_ip_group','defence_enable','defence_ip_group','ip_enable','link_clone','order','create_time','ipv6_enable','sync_firewall_rule']);
    ...
    {
        $line = $this
                ->field('id,data_center_id,name,bill_type,bw_ip_group,defence_enable,defence_ip_group,ip_enable,link_clone,order,ipv6_enable,sync_firewall_rule,order_default_defence')
                ->find($id);
        if(empty($line)){
            return (object)[];
        }
    ...
    public function lineUpdate($param)
    {
        ...
        $this->startTrans();
        try{
            $this->update($param, ['id'=>$line['id']], ['name','bw_ip_group','defence_enable','defence_ip_group','ip_enable','link_clone','order','ipv6_enable','sync_firewall_rule','order_default_defence']);
            if($param['sync_firewall_rule'] == 1){
