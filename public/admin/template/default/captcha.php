...
<div id="content" class="captcha" v-cloak>
  <com-config>
    <t-card class="list-card-container">
      <ul class="common-tab">
        <li v-permission="'auth_system_configuration_captcha_configuration_captcha_configuration_view'">
          <a :href="`configuration_security.htm`">{{lang.captcha_setting}}</a>
        </li>
        <li class="active" v-permission="'auth_system_configuration_captcha_configuration_captcha_interface_view'">
          <a href="javascript:;">{{lang.captcha_manage}}</a>
        </li>
      </ul>
      <div class="common-header">
        <t-button @click="addUser" class="add"  v-permission="'auth_system_configuration_captcha_configuration_captcha_interface_get_more_interfaces'">{{lang.get_more}}</t-button>
      </div>
      <t-table row-key="1" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" :hide-sort-tips="hideSortTips">
        ...
        <template #op="{row}">
          ...
          v-if="row.status !== 3 && $checkPermission('auth_system_configuration_captcha_configuration_captcha_interface_configuration')">
          ...
          <t-tooltip :content="row.status==1 ? lang.disable : lang.enable" :show-arrow="false" theme="light">
            ...
            :class="{rotate: row.status== 1,'no-allow': row.name === 'TpCaptcha'}" @click="changeStatus(row)"
             v-if="row.status !== 3 && $checkPermission('auth_system_configuration_captcha_configuration_captcha_interface_deactivate_enable_interface')">
            </t-icon>
          </t-tooltip>
          <t-tooltip :content="row.status !== 3 ? lang.uninstall : lang.install" :show-arrow="false" theme="light">
            ...
            v-permission="'auth_system_configuration_captcha_configuration_captcha_interface_install_uninstall_interface'">
