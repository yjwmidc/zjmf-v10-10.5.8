    ...
    <t-card class="list-card-container">
      <div class="common-header">
        <t-button @click="addUser" class="add"
          v-permission="'auth_system_configuration_oauth_get_more_interfaces'">{{lang.get_more}}</t-button>
      </div>
      <t-table row-key="1" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
        :table-layout="tableLayout ? 'auto' : 'fixed'" :hide-sort-tips="hideSortTips">
        ...
        <template #op="{row}">
          ...
          <t-tooltip :content="lang.config" :show-arrow="false" theme="light"
            v-if="row.status !== 3 && $checkPermission('auth_system_configuration_oauth_interface_configuration')">
          ...
          <t-tooltip :content="row.status==1 ? lang.disable : lang.enable" :show-arrow="false" theme="light">
            <t-icon :name="row.status==1 ? 'minus-circle' : 'play-circle-stroke'" class="common-look"
              ...
              v-if="row.status !== 3 && $checkPermission('auth_system_configuration_oauth_deactivate_enable_interface')">
            </t-icon>
          </t-tooltip>
          <t-tooltip :content="row.status !== 3 ? lang.uninstall : lang.install" :show-arrow="false" theme="light">
            <a class="common-look" @click="deletePay(row)"
              v-permission="'auth_system_configuration_oauth_install_uninstall_interface'">
