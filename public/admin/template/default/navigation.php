  ...
  <com-config>
    <t-card class="list-card-container table">
      <t-tabs v-model="value" @change="menuChange">
        <!-- 前台导航 -->
        <t-tab-panel value="1" :label="lang.front_nav_manage" :destroy-on-hide="false" v-if="$checkPermission('auth_system_configuration_menu_home_menu_view')">
          ...
          <div class="nav_main">
            ...
            <div class="nav_right">
                ...
                <t-form :data="formData" label-align="top" :label-width="60" @submit="saveSet">
                  ...
                  <t-form-item v-show="formData.type == 'module' || formData.type == 'res_module'" name="url" :label="lang.module_type">
                    ...
                    <t-select v-model="formData.multiple" :min-collapsed-num="1" @change="moduleChange" multiple>
                      <t-option v-for="(item,index) in mergeModule" :key="item.key" :value="item.key" :label="item.display_name" :disabled="calcDisabled(item.key, 'formData')">
                      </t-option>
                    </t-select>
                  </t-form-item>
                  <t-form-item v-show="formData.type == 'custom'" name="url" :label="lang.url_address" class="custom-url">
                    ...
                    <div class="second">
                      <p>
                        <t-checkbox v-model="formData.second_reminder" @change="saveSet()">{{lang.second_reminder}}</t-checkbox>
                      </p>
                  ...
                  <t-form-item name="product_id" :label="lang.order_hosts" v-if="formData.type == 'module'" :key="itemNum">
                    <t-tree-select @change="saveSet" :min-collapsed-num="1" v-model="formData.product_id" :data="productList"
                    :tree-props="treeProps" multiple clearable :placeholder="lang.select">
                    </t-tree-select>
                  </t-form-item>
              ...
              <t-button class="sure_sub" @click="subMenu" :loading="submitLoading" v-permission="'auth_system_configuration_menu_home_menu_save_menu'">{{lang.apply_nav}}</t-button>
            </div>
          </div>
        </t-tab-panel>
        <!-- 后台导航 -->
        <t-tab-panel value="2" :label="lang.admin_navigation" :destroy-on-hide="false" v-if="$checkPermission('auth_system_configuration_menu_admin_menu_view')">
          ...
          <div class="nav_main">
            ...
            <div class="nav_right">
              ...
              <t-button class="sure_sub" @click="subMenu" :loading="submitLoading" v-permission="'auth_system_configuration_menu_admin_menu_save_menu'">{{lang.apply_nav}}</t-button>
            </div>
          </div>
        </t-tab-panel>
      </t-tabs>
      ...
      <t-form :data="newFormData" label-align="right" :label-width="120" @submit="confirmNewMenu" v-if="visible">
        ...
        <t-form-item v-show="(newFormData.type !== 'custom') && (newFormData.type != 'module') && (newFormData.type != 'res_module')" name="url" :label="lang.select_page">
          ...
          <t-select v-if="newFormData.type == 'plugin'" v-model="newFormData.url" @change="newUrlSelectChange">
            <t-option-group v-for="(list,index) in selectList" :key="index" :label="list.title" divider>
              <t-option v-for="item in list.navs" :value="`${item.id}@${item.url}`" :key="item.id" :label="item.name"></t-option>
            </t-option-group>
          </t-select>
        </t-form-item>
        <t-form-item v-show="(newFormData.type === 'custom') && (newFormData.type != 'module')" name="url" :label="lang.url_address" class="custom-url">
        ...
        <t-form-item v-show="newFormData.type == 'module'" name="url" :label="lang.module_type">
          <t-select v-model="newFormData.multiple" :min-collapsed-num="1" @change="newModuleChange" multiple>
            <t-option v-for="(item,index) in mergeModule" :key="item.key" :value="item.key" :label="item.display_name" :disabled="calcDisabled(item.key, 'newFormData')">
            </t-option>
          </t-select>
        </t-form-item>
        <!-- 前台图标 -->
        ...
        <t-form-item name="product_id" :label="lang.order_hosts" v-show="newFormData.type == 'module' || newFormData.type == 'res_module'">
