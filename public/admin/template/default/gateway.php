    ...
    <t-card class="list-card-container">
      <div class="common-header">
        <t-button @click="addUser" class="add"
          v-permission="'auth_system_interface_gateway_get_more_interfaces'">{{lang.get_more}}</t-button>
      </div>
      <t-table row-key='name' drag-sort="row-handler" :data="data" size="medium" @drag-sort="onDragSort"
        :columns="columns" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'"
        ...
        <template #op="{row}">
          ...
          <t-tooltip :content="lang.config" :show-arrow="false" theme="light"
            v-if="row.status !== 3 && $checkPermission('auth_system_interface_gateway_interface_configuration')">
          ...
          <t-tooltip :content="row.status !== 3 ? lang.uninstall : lang.install" :show-arrow="false" theme="light">
            <a class="common-look" @click="deletePay(row)"
              v-permission="'auth_system_interface_gateway_install_uninstall_interface'">
