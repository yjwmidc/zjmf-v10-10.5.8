      ...
      <div class="common-header">
        <div class="l-btn">
          <t-button @click="addUser" class="add"
            v-permission="'auth_product_server_create_server'">{{lang.create_interface}}</t-button>
          <t-button @click="toChild" class="add"
            v-if="$checkPermission('auth_product_server_sub_server_sub_server_view') || $checkPermission('auth_product_server_sub_server_group_view')">{{lang.child_module}}</t-button>
        </div>
        <div class="right-search">
          ...
          <div class="com-search">
            <t-input v-model="params.keywords" class="search-input"
              :placeholder="`ID、${lang.nickname}、${lang.interface_group_name}`" @keypress.enter.native="search"
      ...
      <t-table row-key="1" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading"
        ...
        <template #op="{row}">
          <t-tooltip :content="lang.edit" :show-arrow="false" theme="light">
            <t-icon name="edit-1" @click="updateHandler(row)" class="common-look"
              v-permission="'auth_product_server_update_server'"></t-icon>
          </t-tooltip>
          <t-tooltip :content="lang.delete" :show-arrow="false" theme="light">
            <t-icon name="delete" @click="deleteUser(row)" class="common-look"
              v-permission="'auth_product_server_delete_server'"></t-icon>
          </t-tooltip>
        </template>
      </t-table>
