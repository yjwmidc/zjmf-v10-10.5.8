...
<div id="content" class="server child-server admin" v-cloak>
  <com-config>
    <t-card class="list-card-container">
      ...
      <ul class="common-tab">
        <li class="active" v-permission="'auth_product_server_sub_server_sub_server_view'">
          <a href="javascript:;">{{lang.child_interface}}</a>
        </li>
        <li v-permission="'auth_product_server_sub_server_group_view'">
          <a href="child_server_group.htm">{{lang.child_interface}}{{lang.group}}</a>
        </li>
      </ul>
      <div class="common-header">
        <div>
          <t-button @click="addUser" class="add" v-permission="'auth_product_server_sub_server_sub_server_create_server'">{{lang.create_interface}}</t-button>
        </div>
      ...
      <t-table row-key="id" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" @sort-change="sortChange" :hide-sort-tips="hideSortTips" :max-height="maxHeight">
        ...
        <template #op="{row}">
          <t-tooltip :content="lang.edit" :show-arrow="false" theme="light">
            <t-icon name="edit-1" @click="updateHandler(row)" class="common-look" v-permission="'auth_product_server_sub_server_sub_server_update_server'"></t-icon>
          </t-tooltip>
          <t-tooltip :content="lang.delete" :show-arrow="false" theme="light">
            <t-icon name="delete" @click="deleteUser(row)" class="common-look" v-permission="'auth_product_server_sub_server_sub_server_delete_server'"></t-icon>
          </t-tooltip>
        </template>
      </t-table>
    ...
    <!-- 添加子接口弹窗 -->
    <t-dialog :visible.sync="visible" :header="title" :on-close="close" :footer="false" width="600" placement="center" class="child-dialog">
      <t-form :rules="rules" :data="formData" ref="form" @submit="onSubmit">
        ...
        <t-form-item label="hash" name="accesshash">
          <t-textarea placeholder="hash" v-model="formData.accesshash" />
        </t-form-item>
