  ...
  <com-config>
    <t-card class="list-card-container">
      ...
      <div class="box">
        ...
        <t-table row-key="id" :data="list" size="medium" :columns="typeColumns" :hover="hover" :loading="loading"
        ...
        <t-pagination show-jumper :total="total" v-if="total" :page-size="params.limit"
          :page-size-options="pageSizeOptions" :on-change="changePage" :current="params.page" />
      </div>

    </t-card>
    <!-- 新增/编辑 类型管理-->
    <t-dialog :header="`${lang.type_manage}`" :visible.sync="classModel" :footer="false" width="600"
      :close-on-overlay-click="false" class="class-dialog">
      <t-form :data="classParams" ref="classForm" @submit="submitSystemGroup" :rules="typeRules" class="cycle-form"
        v-if="classModel">
      ...
      <!-- 分类表格 -->
      <t-table row-key="id" :data="systemGroup" size="medium" :columns="groupColumns" :hover="hover"
        :loading="typeLoading" table-layout="auto" display-type="fixed-width" :hide-sort-tips="true" max-height="450px"
        class="type-table">
        <template slot="sortIcon">
          <t-icon name="caret-down-small"></t-icon>
        </template>
    ...
    <!-- 反馈详情 -->
    <t-dialog :header="`${lang.feedback_detail}`" :visible.sync="detailModel" :footer="false" width="600"
      :close-on-overlay-click="false" class="class-dialog">
      <p class="des">
