  ...
  <com-config>
    <t-card class="list-card-container" :class="{stop: data.status===0}">
      <div class="common-header">
        <div class="left">
          ...
          <t-button class="add" @click="noticeManage"
            v-permission="'auth_product_management_global_setting'">{{lang.product_set_text1}}</t-button>
        </div>
        <div class="com-search">
          <t-input v-model="params.keywords" class="search-input" :placeholder="`${lang.product_set_text99}`"
            @keypress.enter.native="search" :on-clear="clearKey" clearable>
          </t-input>
          <t-icon size="20px" name="search" @click="search" class="com-search-btn"></t-icon>
        </div>
      </div>
      <t-enhanced-table ref="table" row-key="key" drag-sort="row-handler" :data="data" :columns="columns"
        :tree="{ treeNodeColumnIndex: $checkPermission('auth_product_management_list_order') ? 1 : 0}"
        :loading="loading" :hover="hover" :tree-expand-and-fold-icon="treeExpandAndFoldIconRender"
        :before-drag-sort="beforeDragSort" :key="expandNum" @abnormal-drag-sort="onAbnormalDragSort"
        @drag-sort="changeSort" class="product-table" :row-class-name="rowName"
        ...
        <template #product_group_name="{row}">
          ...
          <span v-else-if="row.parent_id" class="second-name">#{{row.id}}-{{row.name}}</span>
        </template>
        <template #name="{row}">
          <a :href="calcHref(row)" target="_blank"
            v-if="row.qty !== undefined && $checkPermission('auth_product_detail_basic_info_view')" class="product-name"
            style="cursor: pointer;">#{{row.id}}-{{row.name}}</a>
          <span
            v-if="row.qty !== undefined && !$checkPermission('auth_product_detail_basic_info_view')">#{{row.id}}-{{row.name}}</span>
          <!-- <template v-else>
        ...
        <template #server_id="{row}">
          <span v-if="row.server_id">({{row.server_id}}){{row.server_name}}</span>
        </template>
        ...
        <template #op="{row}">
          ...
          <t-tooltip :content="lang.create_group" :show-arrow="false" theme="light"
            v-if="row.key.indexOf('f') !== -1 && $checkPermission('auth_product_management_create_group')">
            <t-icon name="folder-add" size="18px" @click="addGroup(row)" class="common-look"></t-icon>
          </t-tooltip>
          <t-tooltip :content="lang.create_product" :show-arrow="false" theme="light"
            v-if="row.key.indexOf('s') !== -1 && $checkPermission('auth_product_management_create_product')">
            <t-icon name="add-circle" size="18px" @click="addProduct(row)" class="common-look"></t-icon>
          </t-tooltip>
    ...
    <t-dialog :visible.sync="agentVisble" :header="lang.can_be_agency" :on-close="closeAgentDia" :footer="false"
      ...
      <div class="opt">
        <span>
          ...
          <t-checkbox v-model="checkAll" @change="chooseAllAgent"
            :disabled="formData.id===1">{{lang.isCheckAll}}</t-checkbox>
        </span>
      </div>
      <div class="auth">
        <t-tree :data="authArr" checkable activable :line="true" :expand-on-click-node="false" :active-multiple="false"
          v-model="authList" value-mode="all" :expanded="expandArr"
          :keys="{value: 'key', label:'name', children:'children'}" ref="tree" :expand-all="checkExpand"
          :expand-on-click-node="false" @click="clickNode" :indeterminate="true"></t-tree>
      </div>
      <div class="com-f-btn">
    ...
    <t-dialog :header="updateNameTip" :visible.sync="updateNameVisble" :footer="false">
      <t-form :data="updataData" ref="groupForm" @submit="submitUpdateName" :rules="rules">
        <t-form-item :label="lang.group_name" name="name">
          <t-input v-model="updataData.name" :placeholder="`${lang.group_name}${multiliTip}`"></t-input>
        </t-form-item>
        <div class="com-f-btn">
    ...
    <t-dialog :header="lang.create_group" :visible.sync="groupModel" :footer="false" @close="closeGroup">
      <t-form :data="formData" ref="groupForm" @submit="onSubmit" :rules="rules">
        <t-form-item :label="lang.group_name" name="name">
          <t-input v-model="formData.name" :placeholder="`${lang.group_name}${multiliTip}`"></t-input>
        </t-form-item>
    ...
    <t-dialog :header="lang.create_product" :visible.sync="productModel" :footer="false" class="product-dialog"
      ...
      <t-form :data="productData" ref="productForm" @submit="submitProduct" :rules="productRules">
        <t-form-item :label="lang.product_name" name="name">
          <t-input v-model="productData.name" :placeholder="`${lang.product_name}${multiliTip}`"></t-input>
        </t-form-item>
    ...
    <t-dialog :visible.sync="delHasPro" :footer="false" class="connect-group" :on-close="closeMove">
      <template slot="header">
        <div style="display: flex;align-items: center;">
          <t-icon name="error-circle" size="24px" style="color:var(--td-error-color);"></t-icon>
          <span>{{lang.tip8}}</span>
        </div>
      </template>
      <template slot="body">
    ...
    <!-- 复制弹窗 -->
    <t-dialog theme="warning" :header="copyTitle" :visible.sync="copyVisble" :footer="false" class="product-dialog">
      <t-form :data="copyData" ref="copyProForm" @submit="sureCopy" :rules="productRules">
        <t-form-item :label="lang.second_group" name="product_group_id">
          <t-select v-model="copyData.product_group_id" :placeholder="lang.second_group">
            <t-option v-for="item in copySecGroup" :value="item.id" :label="item.name" :key="item.id">
            </t-option>
          </t-select>
        </t-form-item>
        <div class="com-f-btn">
          <t-button theme="primary" type="submit" :loading="submitLoading">{{lang.sure}}</t-button>
          <t-button theme="default" variant="base" @click="closeCopy">{{lang.cancel}}</t-button>
        </div>
      </t-form>
    </t-dialog>
  </com-config>
