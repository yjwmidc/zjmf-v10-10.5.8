      ...
      <div class="common-header">
        <div>
          <t-button @click="addUser" class="add"
            v-permission="'auth_product_server_group_create_group'">{{lang.create_group}}</t-button>
        </div>
        <div class="com-search">
          <t-input v-model="params.keywords" class="search-input" :placeholder="`${lang.nickname}`"
        ...
        <template #op="{row}">
          <t-tooltip :content="lang.update_interface" :show-arrow="false" theme="light">
            <t-icon name="edit-1" @click="updateHandler(row)" class="common-look"
              v-permission="'auth_product_server_group_update_group'"></t-icon>
          </t-tooltip>
          <t-tooltip :content="lang.delete" :show-arrow="false" theme="light">
            <t-icon name="delete" @click="deleteUser(row)" class="common-look"
              v-permission="'auth_product_server_group_delete_group'"></t-icon>
          </t-tooltip>
        </template>
      </t-table>
    ...
    <t-dialog :visible.sync="visible" :header="title" :on-close="close" :footer="false" width="600">
      <t-form :rules="rules" :data="formData" ref="userDialog" @submit="onSubmit" v-if="visible">
        ...
        <t-form-item :label="lang.interface" name="server_id">
          <t-select v-model="formData.server_id" :placeholder="lang.interface" multiple :max-collapsed-num="5"
