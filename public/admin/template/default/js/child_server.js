  ...
  window.onload = function () {
    ...
    new Vue({
        ...
        return {
          ...
          rules: {
            ...
            type: [
              {
                required: true,
                message: lang.select + lang.server_module,
                type: "error",
              },
            ],
            ip_address: [
            ...
            max_accounts: [
              {
                required: true,
                message: lang.input + lang.interface_capacity,
                type: "error",
              },
              {
                pattern: /^([1-9][0-9]*)$/,
      ...
      created() {
        ...
        this.getGroupList();
        document.title =
          lang.child_module + "-" + localStorage.getItem("back_website_name");
      },
      methods: {
        async getGroupList() {
          try {
        ...
        jumpStore() {
          setToken().then((res) => {
            if (res.data.status == 200) {
              ...
              window.open(
                `https://my.idcsmart.com/shop/shop_app.html?from=${from}&token=${token}&appType=sub_server`
              );
            }
          });
        },
        ...
        async onSubmit({validateResult, firstError}) {
          if (validateResult === true) {
            try {
              ...
              const res = await addAndUpdateChildInterface(
                this.type,
                this.formData
              );
