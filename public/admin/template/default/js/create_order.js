  ...
  window.onload = function () {
    ...
    new Vue({
      ...
      created() {
        ...
        // this.getProductList();
        document.title =
          lang.create_order + "-" + localStorage.getItem("back_website_name");
      },
      methods: {
        handelIframeMsg(e) {
          ...
          if (type === "iframeBuy") {
            ...
            if (subPrice !== null) {
              subObj.custom_order_amount = subPrice;
            }
            settleOrder(subObj)
              .then((res) => {
        ...
        choosePro(val) {
          ...
          this.newOrderIframeUrl = `${this.website_url}/cart/goods_iframe.htm?id=${val}&token=${this.userJwt}`;
        },
        // 选择用户
        async chooseUser(e) {
          window.client_id = e;
          ...
          try {
            if (this.formData.type === "new") {
              ...
              this.userJwt = res.data.data.jwt;
            }
          } catch (error) {
            console.log(error);
          }
