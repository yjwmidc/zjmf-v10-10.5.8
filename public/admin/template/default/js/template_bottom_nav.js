      ...
      methods: {
        ...
        getTabList() {
          getTemplateControllerTab({ theme: this.theme }).then((res) => {
            ...
            const curName = this.tabList.filter(
              (item) => item.url === this.tab
            )[0]?.title;
            document.title =
              curName + "-" + localStorage.getItem("back_website_name");
          });
        },
        changeTab(val) {
        ...
        async changeOpen(val, row) {
          try {
            const res = await changeBaseOpen(this.name, {
              id: row.id,
              blank: val,
            });
            this.$message.success(res.data.msg);
