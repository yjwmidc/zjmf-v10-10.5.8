    ...
    new Vue({
      ...
      data () {
        return {
          ...
          rules: {
            name: [
              { required: true, message: lang.input + lang.product_set_text116, type: 'error' }
            ],
            nickname: [
            ...
            product_id: [
              { required: true, message: lang.select + lang.product, trigger: 'blur' }
            ],
          },
          loading: false,
          popupProps: {
      ...
      computed: {
        calcSelectedProductList () {
          let temp = this.allPro.filter(item => this.ruleForm.product_id.includes(`t-${item.id}`));
          if (this.rightSearch.trim()) {
            temp = temp.filter(item => item.name.indexOf(this.rightSearch) >= 0);
          }
          return temp;
        }
      },
      methods: {
        ...
        handleDelProduct (arr) {
          ...
          this.ruleForm.product_id = this.ruleForm.product_id.filter(item =>!arr.includes(item));
        },
            ...
            list.forEach(item => {
              ...
              item.product.forEach(sub => {
                if (this.params.product_name.trim()) {
                  item.allName += sub.name.replace(new RegExp(this.params.product_name, 'g'), `<span class="high-light-text">${this.params.product_name}</span>`) + '、';
                } else {
        ...
        updateRule (row) {
          ...
          this.ruleForm.notice_setting = Object.keys(row.notice_setting).filter(item => row.notice_setting[item] === 1);
          ...
          if (validateResult === true) {
            try {
              ...
              params.notice_setting = this.notice_setting.reduce((all, cur) => {
                all[cur.name] = params.notice_setting.includes(cur.name) ? 1 : 0;
                return all;
              }, {});
              params.product_id =  params.product_id.map(item => Number(item.replace('t-', '')))
