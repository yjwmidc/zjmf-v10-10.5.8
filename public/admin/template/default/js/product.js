    ...
    new Vue({
          ...
          if (val) {
            let temp = JSON.parse(JSON.stringify(this.data)).map((item) => {
              item.children = item.children.filter(
                (el) => el.id !== this.moveProductForm.id
              );
              return item;
            });
            this.tempGroup = temp;
      ...
      computed: {
        calcHref() {
          return (row) => {
            if (row.agent === 1) {
              return `upstream_goods.htm?id=${row.id}`;
            } else {
              return `product_detail.htm?id=${row.id}`;
            }
          };
        },
        lookHost() {
          return (row) => {
            if (row.host_num > 0) {
              return `host.htm?type=product_id&keywords=${row.id}`;
            } else {
              return "javascript:;";
            }
          };
      ...
      methods: {
        noticeManage() {
          location.href = "global_setting_notice.htm";
        },
        addAgency() {
        ...
        copyHandler(row) {
          this.curId = row.id;
          this.copySecGroup = this.tempSecondGroup.filter(
            (item) => item.parent_id === row.product_group_id_first
          );
          this.copyData.product_group_id = row.product_group_id_second;
        ...
        editHandler(row) {
          if (row.key.indexOf("t") !== -1) {
            if (row.agent === 1) {
              window.open(`upstream_goods.htm?id=${row.id}`);
            } else {
              window.open(`product_detail.htm?id=${row.id}`);
            }
            ...
            this.updateNameTip =
              lang.update + lang.second_group + lang.nickname;
            this.changeFisrt(row);
          }
        },
        // 展开/折叠
        ...
        deleteHandler(row) {
          ...
          if (row.key.indexOf("f") !== -1) {
            ...
            if (row.children.length > 0) {
              ...
              this.concat_shop = this.concat_shop.substring(
                0,
                this.concat_shop.length - 1
              );
            } else {
              this.delteProduct(row.id);
            }
          } else if (row.key.indexOf("t") !== -1) {
            // 删除商品
        ...
        async changeSort({currentIndex, current, targetIndex, target}) {
          try {
            ...
            // 一级分组移动
            if (
              current.key.indexOf("f") !== -1 &&
              target.key.indexOf("f") !== -1
            ) {
            ...
            // 移动整个二级分组
            if (
              current.key.indexOf("s") !== -1 &&
              target.key.indexOf("s") !== -1
            ) {
              ...
              this.secondGroupForm.pre_first_product_group_id =
                target.parent_id;
            ...
            // 移动商品到其他二级分组
            if (
              current.key.indexOf("t") !== -1 &&
              target.key.indexOf("t") !== -1
            ) {
            ...
            if (
              ...
              this.secondGroupForm.pre_product_group_id =
                _temp.children.length > 0 ? _temp.children.at(-1).id : 0;
              this.secondGroupForm.backward =
                _temp.children.length > 0 ? 1 : tempForward;
              this.secondGroupForm.pre_first_product_group_id =
                _temp.children.length > 0
                  ? _temp.children.at(-1).parent_id
                  : _temp.id;
              this.movePorductGroup();
            }
            // 特殊情况：拖动商品到无商品二级栏目下
            if (
              ...
              const pArr = this.data.filter(
                (item) => item.id === target.parent_id
              )[0];
              const index = pArr.children.findIndex(
                (item) => item.key === target.key
              );
              this.dragForm.id = current.id;
              // // 目标节点对应的数组
              const _temp = pArr.children[tempForward ? index : index - 1] || {
                children: [],
                id: target.id,
              };
              this.dragForm.pre_product_id =
                _temp.children.length > 0 ? _temp.children.at(-1).id : 0;
              this.dragForm.product_group_id =
                _temp.children.length > 0
                  ? _temp.children.at(-1).product_group_id_second
                  : _temp.id;
              this.dragForm.backward =
                _temp.children.length > 0 ? 1 : tempForward;
              this.movePorductHandel();
            }
          } catch (error) {
        ...
        // 新建商品
        addProduct(row) {
          this.productData.firstId = row.parent_id || "";
          this.productData.product_group_id = row.id || "";
          this.productModel = true;
        },
        addBaiduProduct() {
          location.href = "plugin/baidu_cloud/baiduProduct.htm";
        },
        changeFirId(val) {
          this.secondGroup = this.tempSecondGroup.filter(
        ...
        async submitProduct({validateResult, firstError}) {
          if (validateResult === true) {
            try {
              ...
              this.getProductList();
              this.submitLoading = false;
              window.open(`product_detail.htm?id=${res.data.data.product_id}`);
            } catch (error) {
              this.submitLoading = false;
              this.$message.error(error.data.msg);
            }
        ...
        async sureCopy({validateResult, firstError}) {
          if (validateResult === true) {
            try {
              this.submitLoading = true;
              const res = await copyProduct({
                id: this.curId,
                product_group_id: this.copyData.product_group_id,
              });
              this.$message.success(res.data.msg);
              this.getProductList();
              this.submitLoading = false;
              this.$refs.copyProForm.reset();
      ...
      created() {
        if (!this.$checkPermission("auth_product_management_list_order")) {
