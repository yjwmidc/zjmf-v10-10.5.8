    ...
    new Vue({
      ...
      data() {
        return {
          ...
          creditVisible: false,
          creditFormData: {
            supplier_credit_warning_notice: 0,
            supplier_credit_amount: 0,
            supplier_credit_push_frequency: 1,
          },
          creditData: {
            supplier_credit_warning_notice: 0,
            supplier_credit_amount: 0,
      ...
      methods: {
        ...
        handelRefreshCredit(row) {
          row.loadingCredit = true;
          apiRefreshSupplierCredit({id: row.id})
            .then((res) => {
              res.data.data.credit && (row.credit = res.data.data.credit);
              this.$message.success(res.data.msg);
              row.loadingCredit = false;
            })
            .catch((err) => {
              row.loadingCredit = false;
              this.$message.error(err.data.msg);
        ...
        getCreditInfo() {
          apiGetCreditWarning().then((res) => {
            this.creditFormData = res.data.data;
            this.creditData = res.data.data;
          });
        },
        handelOpenConfig() {
          this.creditVisible = true;
          this.creditFormData = {...this.creditData};
        },
        creditDiaClose() {
          this.$refs.creditDialog && this.$refs.creditDialog.reset();
        },
        submitCredit({validateResult, firstError}) {
          if (validateResult === true) {
            this.submitLoading = true;
            apiUpdateCreditWarning(this.creditFormData)
              .then((res) => {
                this.$message.success(res.data.msg);
