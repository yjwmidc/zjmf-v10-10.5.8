        ...
        <t-form :data="formData" ref="form" @submit="onSubmit" layout="inline" label-align="left">
          ...
          <t-form-item>
            <t-date-range-picker allow-input clearable v-model="range"
              :placeholder="[`${lang.select}${lang.client_care_label44}`,`${lang.select}${lang.client_care_label44}`]"></t-date-range-picker>
          </t-form-item>
      ...
      <t-table row-key="id" :data="data" size="medium" :hide-sort-tips="true" :columns="columns" :hover="hover"
        ...
        <template #retry="{row}">
          <t-tooltip :content="lang.retry" :show-arrow="false" theme="light">
            <a class="common-look" v-if="row.status === 'Failed' && $checkPermission('auth_management_task_retry')"
              @click="retryFun(row.id)">
