      ...
      switch (code) {
        ...
        case 400:
          if (response.data.data) {
            const {admin_operate_methods, operate_password} =
              response.data.data;
            if (admin_operate_methods && operate_password) {
              this.adminOperateVue.$refs.safeRef.openDialog(
                admin_operate_methods
              );
            }
          }
    ...
    if (error.config) {
      if (error.config.url.indexOf("system/autoupdate") !== -1) {
        ...
        if (error.message === "Network Error") {
          this.TDesign.MessagePlugin.error(
            "网络开小差啦，请更换网络或者稍后再试"
          );
          setTimeout(() => {
            location.reload();
