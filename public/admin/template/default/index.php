    ...
    <div class="top-setting">
      ...
      <t-popup placement="right-bottom" overlay-class-name="home-checked">
        ...
        <template #content>
          ...
          <t-checkbox v-for="(item,index) in allWidget" :key="item.id" v-model="item.checked"
            @change="changeCheck($event,item)">{{item.title}}</t-checkbox>
        </template>
      </t-popup>
    </div>
    <template v-show="showList.length > 0">
      ...
      <t-row :gutter="30" id="drag-row">
        <draggable v-model="showList" chosen-class="chosen" handle=".mover" force-fallback="true" animation="300"
          ...
          <transition-group>
            <t-col :sm="calcSm(item.columns)" :md="calcMd(item.columns)" :xl="calcXl(item.columns)"
