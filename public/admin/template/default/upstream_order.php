      ...
      <div class="money-box">
        ...
        <div class="item">
          <div class="l-text">
            ...
            <p class="btn" @click="goAgentList" v-permission="'auth_upstream_downstream_upstream_order_agent'">{{lang.upstream_text4}}</p>
          </div>
    ...
    <t-card class="list-card-container bottom-card b-table">
      <div class="common-header">
        ...
        <div class="client-search">
          <t-input v-model="params.keywords" :placeholder="lang.user_text21" @keypress.enter.native="search">
      ...
      <t-enhanced-table ref="table" row-key="id" drag-sort="row-handler" :data="data" :columns="columns" :tree="{ childrenKey: 'list', treeNodeColumnIndex: 0 }" :loading="loading" :key="new Date().getTime()" :hover="hover" :tree-expand-and-fold-icon="treeExpandAndFoldIconRender" @sort-change="sortChange" class="user-order" :hide-sort-tips="true">
        ...
        <template slot="empty">
          <div style="text-align: center;">
            ...
            <t-button @click="goAgentList" v-permission="'auth_upstream_downstream_upstream_order_agent'">{{lang.upstream_text4}}</t-button>
          </div>
        </template>
        <template #id="{row}">
        ...
        <template #op="{row}">
          <template v-if="row.type">
            <t-tooltip :content="`${lang.look}${lang.detail}`" :show-arrow="false" theme="light">
              <t-icon name="view-module" class="common-look" @click="lookDetail(row)" v-permission="'auth_upstream_downstream_upstream_order_check_order'"></t-icon>
            </t-tooltip>
            <t-tooltip :content="lang.update_price" :show-arrow="false" theme="light">
              ...
              v-if="row.status!=='Paid' && row.status!=='Cancelled' && row.status!=='Refunded' && $checkPermission('auth_upstream_downstream_upstream_order_update_order')"></t-icon>
            </t-tooltip>
            ...
            <t-tooltip :content="lang.delete" :show-arrow="false" theme="light">
              <t-icon name="delete" class="common-look" @click="delteOrder(row)" v-permission="'auth_upstream_downstream_upstream_order_delete_order'"></t-icon>
            </t-tooltip>
          </template>
          <template v-else>
            <t-tooltip :content="lang.edit" :show-arrow="false" theme="light" v-if="row.edit && $checkPermission('auth_upstream_downstream_upstream_order_update_order')">
