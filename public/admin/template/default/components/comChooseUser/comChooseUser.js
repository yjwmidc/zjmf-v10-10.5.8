...
const comChooseUser = {
  template: `
    <div class="choose-user">
      ...
      <t-select v-model="client_id" filterable :placeholder="prePlaceholder"  :loading="searchLoading"
        @search="remoteMethod" clearable @clear="changeUser" :multiple="multiple"
  ...
  methods: {
    ...
    async getMulUser(arr) {
      try {
        ...
        const user = arr
          .filter((item) => !temp.includes(item * 1))
          .map((item) => {
