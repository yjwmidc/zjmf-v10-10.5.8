...
const comViewFiled = {
  template: `
    <div class="view-filed">
      ...
      <!-- 字段设置 -->
      <t-dialog :header="calcTit" :visible.sync="filedModel" :footer="false" width="900"
        class="filed-dialog">
        <div class="con">
          <div class="filed-box">
          ...
          <div class="select-filed">
            ...
            <div class="scroll t-table__content">
              <t-table row-key="key" :data="calcSelectFiled" size="medium" :columns="filedColumns" :hover="hover"
                ...
                <template #op="{row}">
                  <div class="com-opt">
                    <t-icon name="delete" @click="delField('filed', row)" v-if="row.key !== 'id'"></t-icon>
                  </div>
                </template>
              </t-table>
            </div>
  ...
  computed: {
    isPriceType () {
      return type => {
        const arr = [
          'renew_amount', 'first_payment_amount', 'base_price', 'order_amount',
          'order_use_credit', 'order_refund_amount',
        ];
