    ...
    <t-card class="list-card-container">
      <div class="common-header">
        <div class="left flex">
          ...
          <t-button theme="success" @click="exportVisible = true" v-if="$checkPermission('auth_business_order_export_excel') && hasExport">
            {{lang.data_export}}
          </t-button>
        </div>
        <!-- 右侧搜索 -->
        <div class="right-search">
          <div class="flex view-filed" v-if="!isAdvance">
            ...
            <t-select :value="params.view_id" :label="lang.view_filed + ':'" class="choose-view not-default" @change="chooseView" :popup-props="{ overlayClassName: `view-select`}">
            ...
            <t-select v-model="searchType" class="com-list-type" @change="changeType">
              <t-option v-for="item in calcTypeSelect" :value="item.value" :label="item.label" :key="item.value"></t-option>
            </t-select>
            <t-input v-model="params.keywords" class="search-input" :placeholder="lang.input" @keypress.enter.native="search" clearable v-show="searchType !== 'product_id' && searchType !== 'sale'" @clear="clearKey('keywords')" :maxlength="30">
            </t-input>
            <input class="com-empty-input"></input>
            <com-tree-select class="search-input" v-show="searchType === 'product_id'" :value="params.product_id" @choosepro="choosePro">
            </com-tree-select>
            <!-- 选择销售 -->
            <t-select v-model="curSaleId" :placeholder="lang.please_choose_sale" clearable v-show="hasSale && searchType === 'sale'">
          ...
          <t-button @click="changeAdvance" class="ml8">{{isAdvance ? lang.pack_up : lang.advanced_filter}}</t-button>
      ...
      <div class="advanced" v-show="isAdvance">
        <div class="edit-view">
          <t-button class="add" v-if="viewFiledNum && data_range_switch" @click="handleEditView">{{lang.view_data_range}}({{viewFiledNum}})</t-button>
        </div>
        <div class="search">
          <!-- <t-select v-model="searchType" class="com-list-type" @change="changeType">
          ...
          <t-input v-model="params.username" class="search-input" :placeholder="`${lang.input}${lang.username}`" @keypress.enter.native="search" clearable @clear="clearKey('username')">
          </t-input>
          <t-input :placeholder="lang.money" v-model="params.amount" @keypress.enter.native="search" clearable @clear="clearKey('amount')" style="width: 150px;"></t-input>
      ...
      <t-enhanced-table ref="table" row-key="id" drag-sort="row-handler" :data="calcList" :columns="columns" resizable :tree="{ childrenKey: 'list', treeNodeColumnIndex: 0 }" :loading="loading" :hover="hover" :tree-expand-and-fold-icon="treeExpandAndFoldIconRender" @sort-change="sortChange" class="user-order" :hide-sort-tips="true" @select-change="rehandleSelectChange" :selected-row-keys="checkId" @column-resize-change="resizeChange">
        ...
        <template #gateway="{row}">
          ...
          <template v-else>
            ...
            <template v-if="row.credit * 1 >0 && row.credit * 1 < row.amount * 1 && row.gateway_sign !== 'credit'">
              ...
              <span>{{(row.gateway && row.gateway_sign !== 'credit') ? '+ ' + row.gateway: '' }}</span>
            </template>
        ...
        <template #op="{row}">
          <template v-if="row.type">
            <!-- 审核 -->
            <t-tooltip :content="lang.order_audit" :show-arrow="false" theme="light" v-if="row.status === 'WaitReview' && $checkPermission('auth_business_order_review_voucher')">
            ...
            <!-- 上传/重新上传 -->
            <t-tooltip :content="row.status === 'WaitUpload' ? lang.upload_proof : lang.reupload" :show-arrow="false" theme="light" v-if="(row.status === 'WaitUpload' || row.status === 'WaitReview' || row.status === 'ReviewFail')
              && $checkPermission('auth_business_order_upload_voucher')">
              <t-icon name="upload" class="common-look" @click="handleUpload(row)"></t-icon>
            </t-tooltip>
            <!-- 查看凭证 -->
            <t-tooltip :content="lang.look_proof" :show-arrow="false" theme="light" v-if="row.status === 'Paid' && row.voucher.length > 0 && $checkPermission('auth_business_order_view_voucher')">
    ...
    <t-dialog :header="lang.update_price" :visible.sync="priceModel" :footer="false">
      <t-form :data="formData" ref="update_price" @submit="onSubmit" :rules="rules" v-if="priceModel">
        <t-form-item :label="lang.change_money" name="amount">
          <t-input v-model="formData.amount" type="tel" :label="currency_prefix" :placeholder="lang.update_price_tip"></t-input>
        </t-form-item>
      ...
      <div class="con">
        <t-form :data="proofForm" @submit="submitProof">
          <t-form-item name="voucher" :required-mark="false">
            <t-upload theme="custom" multiple v-model="proofForm.voucher" :action="uploadUrl" :before-upload="beforeUpload" :format-response="formatResponse" :headers="uploadHeaders" :max="10" accept="image/*, .pdf, .PDF">
