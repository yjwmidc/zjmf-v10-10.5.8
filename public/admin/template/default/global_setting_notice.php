...
<div id="content" class="product-notice-setting hasCrumb" v-cloak>
  <com-config>
      ...
      <div class="common-tab-content">
        ...
        <div class="notice-box">
          <t-tabs v-model="params.type" @change="changeTab" placement="left" style="flex-shrink: 0; margin-right: 20px">
            <t-tab-panel :value="item.type" :label="item.name" v-for="item in notice_type" :key="item.type"></t-tab-panel>
          </t-tabs>
          <div class="right-box">
            <div class="search">
              <t-button @click="addRule">{{lang.product_set_text110}}</t-button>
              <div class="flex">
                <t-input :placeholder="lang.product_set_text112" v-model="params.name" @keypress.enter.native="getNoticeList" clearable @clear="getNoticeList"> </t-input>
                <t-input :placeholder="lang.product_set_text113" v-model="params.product_name" @keypress.enter.native="getNoticeList" clearable @clear="getNoticeList"> </t-input>
              </div>
            </div>
            <t-table
              row-key="id"
              ...
              <template #product-config="{row}">
                <div class="config-box">
                  <span v-for="(item,index) in notice_setting" class="config-tit" :key="index">
                    <t-switch :custom-value="[1,0]" size="small" v-model="row.notice_setting[item.name]" @change="changeStatus($event,item, row.id)"> </t-switch>
                  </span>
                </div>
              </template>
              <template #op="{row}">
                ...
                <t-tooltip :content="lang.delete" :show-arrow="false" theme="light">
                  <t-icon name="delete" class="common-look" @click="deleteRule(row)" v-if="row.is_default === 0"> </t-icon>
                </t-tooltip>
              </template>
            </t-table>
          </div>
    ...
    <!-- 规则组 -->
    <t-dialog :visible.sync="visible" :header="optTip" :on-close="close" :footer="false" width="1000" class="rule-dialog" placement="center">
      <t-form :rules="rules" :data="ruleForm" ref="ruleDialog" @submit="onSubmit" reset-type="initial" label-align="top">
        <t-form-item :label="lang.product_set_text116" name="name">
          <t-input :placeholder="lang.product_set_text116" v-model="ruleForm.name" :disabled="optType === 'edit' && ruleForm.is_default === 1" />
        </t-form-item>
        <t-form-item :label="lang.product_set_text117" name="notice_setting">
          <t-checkbox-group v-model="ruleForm.notice_setting">
            <t-checkbox v-for="item in notice_setting" :key="item.name" :value="item.name">{{item.name_lang}}</t-checkbox>
          </t-checkbox-group>
        </t-form-item>
        <t-form-item :label="lang.product_set_text118" name="product_id">
          ...
          <div class="flex">
            <div class="left">
              <div class="top">
                <t-input v-model="leftSearch" @change="changeLeftSearch" :placeholder="lang.product_set_text119" clearable> </t-input>
              </div>
              <t-tree
                ref="tree"
                v-model="ruleForm.product_id"
                :keys="{children: 'children', label: 'name', value: 'key'}"
                :data="productList"
                :checkable="true"
                value-mode="onlyLeaf"
                ...
                <t-table
                  ...
                  <template #op="{row}">
                    <t-tooltip :content="lang.user_api_text9" :show-arrow="false" theme="light">
                      <t-icon name="delete" class="common-look" @click="handleDelProduct([row.id])" v-if="!(optType === 'edit' && ruleForm.is_default === 1)"> </t-icon>
                    </t-tooltip>
                  </template>
                </t-table>
              </div>
