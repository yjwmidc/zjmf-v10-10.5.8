...
<div id="content" class="upstream_goods" v-cloak>
  <com-config>
    <t-card class="list-card-container b-table">
      ...
      <div class="common-header">
        ...
        <div class="client-search">
          <t-input v-model="params.keywords" :placeholder="lang.upstream_text1" clearable @keypress.enter.native="search" @clear="clearKey">
      ...
      <t-table row-key="id" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading" :table-layout="tableLayout ? 'auto' : 'fixed'" display-type="fixed-width">
        <template #name="{row}">
          <a :href="`product_detail.htm?id=${row.id}`" target="_blank" class="common-look" v-if="row.mode === 'sync' && row.profit_type === 1">
          ...
          <template v-if="row.mode === 'only_api'">
            <t-tooltip :show-arrow="false" theme="light">
              <div slot="content" class="profit-tip">
                <p>{{lang.upstream_text84}}：{{row.profit_percent}}<span>{{row.profit_type === 0 ? '%' : currency_suffix}}</span></p>
                <p>{{lang.upstream_text85}}：{{row.renew_profit_percent}}<span>{{row.renew_profit_type === 0 ? '%' : currency_suffix}}</span></p>
                <p>{{lang.upstream_text86}}：{{row.upgrade_profit_percent}}<span>{{row.upgrade_profit_type === 0 ? '%' : currency_suffix}}</span></p>
              </div>
        ...
        <template #hidden="{row}">
          <t-switch size="large" :custom-value="[0,1]" v-model="row.hidden" @change="onChange(row)" :disabled="!$checkPermission('auth_upstream_downstream_upstream_product_show_hide')"></t-switch>
        </template>
        <template #op="{row}">
          <t-tooltip :content="lang.edit" :show-arrow="false" theme="light">
            <t-icon name="edit" size="18px" class="common-look" @click="editGoods(row)" v-permission="'auth_upstream_downstream_upstream_product_update_upstream_product'"></t-icon>
          </t-tooltip>
          <t-tooltip :content="lang.delete" :show-arrow="false" theme="light">
            <t-icon name="delete" size="18px" @click="deleteHandler(row.id)" class="common-look" v-permission="'auth_upstream_downstream_upstream_product_delete_upstream_product'"></t-icon>
          </t-tooltip>
        </template>
      </t-table>
    ...
    <!-- 编辑商品 -->
    <t-dialog :header="lang.edit_goods" :visible.sync="productModel" :width="isEn ? 750 : 620" :footer="false" @close="closeProduct">
      <t-form :data="productData" ref="productForm" @submit="submitProduct" :rules="productRules" reset-type="initial">
        <t-form-item :label="lang.upstream_text6" name="supplier_id" :help="supplierTip" class="supplier-select">
          <t-select v-model="productData.supplier_id" :placeholder="lang.upstream_text6" filterable :scroll="{ type: 'virtual' }" :popup-props="{ overlayInnerStyle: { height: '200px' } }" @change="supplierChange" :disabled="isEdit">
        ...
        <t-form-item :label="lang.product" name="upstream_product_id">
          <t-select v-model="productData.upstream_product_id" :placeholder="lang.product" filterable :scroll="{ type: 'virtual' }" :popup-props="{ overlayInnerStyle: { height: '200px' } }" :disabled="isEdit">
        ...
        <t-form-item :label="lang.upstream_text101" name="mode" v-if="calcType === 'default'">
          <t-radio-group v-model="productData.mode" @change="changeMode" :disabled="isEdit">
            ...
            <t-radio value="sync" v-show="isShowSync" :disabled="isEdit || edition !== 1">
              ...
              <t-tooltip :content="lang.upstream_text104 + '\n' + lang.upstream_text105  + '\n' + lang.upstream_text106" :show-arrow="false" theme="light" placement="top-left" overlay-class-name="pre-wrap">
        ...
        <template v-if="productData.mode === 'sync'">
          <t-form-item :label="lang.upstream_text107" class="required">
            <t-form-item name="profit_type" :help="productData.profit_type === 0 ? lang.upstream_text114 : lang.upstream_text115">
            ...
            <t-form-item name="profit_percent" class="profit-input" :rules="[{required: true,message: lang.input, type: 'error'}]" v-show="productData.profit_type === 0" help=" ">
              <t-input-number v-model="productData.profit_percent" theme="normal" :decimal-places="2" :placeholder="lang.input" suffix="%"></t-input-number>
        ...
        <template v-else>
          ...
          <t-form-item :label="lang.upstream_text84" class="required">
            ...
            <t-form-item name="profit_percent" v-else class="profit-input">
              <t-input-number v-model="productData.profit_percent" theme="normal" :decimal-places="2" :placeholder="lang.input"></t-input-number>
            </t-form-item>
          </t-form-item>
          <!--  续费 -->
          <t-form-item :label="lang.upstream_text85" class="required">
            ...
            <t-form-item name="renew_profit_percent" v-if="productData.renew_profit_type === 0" class="profit-input">
              <t-input-number v-model="productData.renew_profit_percent" :placeholder="lang.input" theme="normal" :decimal-places="2" suffix="%"></t-input-number>
              <t-tooltip :content="lang.upstream_text20" :show-arrow="false" theme="light">
                <t-icon name="help-circle" size="18px" style="color: var(--td-brand-color); cursor: pointer; margin-left: 10px;"></t-icon>
              </t-tooltip>
            </t-form-item>
            <t-form-item name="renew_profit_percent" v-else class="profit-input">
              <t-input-number v-model="productData.renew_profit_percent" :placeholder="lang.input" theme="normal" :decimal-places="2"></t-input-number>
            </t-form-item>
          </t-form-item>
          <!-- 升级 -->
          <t-form-item :label="lang.upstream_text86" class="required">
            ...
            <t-form-item name="upgrade_profit_percent" v-if="productData.upgrade_profit_type === 0" class="profit-input">
              <t-input-number v-model="productData.upgrade_profit_percent" :placeholder="lang.input" suffix="%" theme="normal" :decimal-places="2"></t-input-number>
              <t-tooltip :content="lang.upstream_text20" :show-arrow="false" theme="light">
                <t-icon name="help-circle" size="18px" style="color: var(--td-brand-color); cursor: pointer; margin-left: 10px;"></t-icon>
              </t-tooltip>
            </t-form-item>
            <t-form-item name="upgrade_profit_percent" v-else class="profit-input">
              <t-input-number v-model="productData.upgrade_profit_percent" :placeholder="lang.input" theme="normal" :decimal-places="2"></t-input-number>
            </t-form-item>
          </t-form-item>
