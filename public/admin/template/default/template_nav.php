    ...
    <t-card class="list-card-container">
      ...
      <div class="box">
        ...
        <t-enhanced-table row-key="id" ref="navTable" :data="data" size="medium" :columns="columns" :hover="hover" :loading="loading" :hide-sort-tips="true" drag-sort="row-handler" @drag-sort="onDragSort" :before-drag-sort="beforeDragSort" @expanded-tree-nodes-change="changeExpand" :tree="{ treeNodeColumnIndex: 1 }" :expanded-tree-nodes="expandedRowKeys">
          ...
          <template #show="{row}">
            <t-switch size="large" v-model="row.show" :custom-value="[1,0]" v-if="row.id !== 1" @change="(val)=>changeStatus(val,row)">
            </t-switch>
          </template>
          <template #blank="{row}">
            <t-switch size="large" v-model="row.blank" :custom-value="[1,0]" @change="(val)=>changeOpen(val,row)">
            </t-switch>
          </template>
          <template #op="{row}">
    ...
    <t-dialog :header="optTitle" :visible.sync="visible" :footer="false" width="500" @closed="visible = false" placement="center">
      <t-form :rules="rules" ref="comDialog" :data="formData" @submit="onSubmit" :label-width="120" reset-type="initial" label-align="top">
        ...
        <t-form-item :label="lang.temp_nav_icon" class="upload_box" v-show="formData.parent_id">
          <t-upload ref="uploadRef3" :size-limit="{ size: 2, unit: 'MB' }" :action="uploadUrl" v-model="formData.icon" :auto-upload="true" @fail="handleFail" theme="image" :headers="uploadHeaders" accept="image/*" :format-response="formatImgResponse">
          </t-upload>
          <div class="up-tip">
