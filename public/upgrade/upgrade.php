...
class UpgradeSystem
{
    ...
    public $plugin_upgrade_log = IDCSMART_ROOT . "public/upgrade/plugin_upgrade.log"; //记录插件升级进度
    ...
    public function getUpgradeProgress()
    {
        ...
        if(file_exists($this->plugin_upgrade_log) && $data['progress']=='90%'){
            $data['plugin_upgrade_log'] = array_values(json_decode(file_get_contents($this->plugin_upgrade_log), true));
        }
    ...
    public function upgradeUnzip()
    {
        ...
        if($progresslog['setup']=="unzip"){                 
            ...
            $zipFilePath = $this->upload_dir.$progresslog['file_name']; // ZIP文件路径  
            $destinationPath = IDCSMART_ROOT; // 解压目标目录路径  
            $targetDirectoryInZip = $progresslog['package_name'].'/'; // ZIP中要解压的目录（注意末尾的斜杠）   
            ...
            if ($res === TRUE) {  
                for ($i = 0; $i < $zip->numFiles; $i++) {  
                    ...
                    // 检查条目是否属于要解压的目录  
                    if (strpos($entryName, $targetDirectoryInZip) === 0 && substr($entryName, -1) !== '/') {  
                        // 移除ZIP中目录的路径部分，以便在目标目录中创建正确的文件结构  
                        $relativePath = substr($entryName, strlen($targetDirectoryInZip));  
                        $destinationFile = $destinationPath . $relativePath;  

                        $admin_application = DIR_ADMIN;
                        if ($admin_application != 'admin' && strpos($destinationFile, IDCSMART_ROOT . 'public/admin/')===0){
                            $destinationFile = str_replace(IDCSMART_ROOT . 'public/admin/', IDCSMART_ROOT . 'public/' .$admin_application.'/', $destinationFile);
                        }
                        ...
                        // 解压文件  
                        //$zip->extractTo($destinationPath, array($entryName));  
                        file_put_contents($destinationFile, $zip->getFromIndex($i));
                    }  
                }  
                $zip->close();  

                $result = ['status' => 200 , 'msg' => '成功'];
            }else{
                $result = ['status' => 400 , 'msg' => $res];
            }
            ...
            if ($result['status'] == 200){
                ...
                $progress_log['progress'] = "10%";
                $progress_log['msg'] = '解压失败,失败code:' . $result['msg'] . ";请到网站目录下解压下载的文件或者重新更新系统";
    ...
    public function upgradeSql()
    {
        ...
        if (version_compare($last_version, $version, '>')) {
            ...
            $domain = isset($httpHost['host'])? ($httpHost['host'].(isset($httpHost['port']) ? (':'.$httpHost['port']) : '')) : $httpHost['path'];
            ...
            foreach ($arr as $v) {
                ...
                $php_file = IDCSMART_ROOT.'app/upgrade/' . $v[1] . '.php';#php 文件
                if (version_compare($php_version, $version, '>')) {
                    if (file_exists($php_file)) {
                        try {
                            ...
                            // $progress_log['msg'] = '升级失败' . ": PHP执行失败，失败PHP: ".$php_file.", 错误信息: ".$e->getMessage();
                            // $progress_log['status'] = 400;
                            // $this->updateProgress(json_encode($progress_log));
                            // return json_encode(['status'=>400,'msg'=>"PHP执行失败，失败PHP: ".$php_file.", 错误信息: ".$e->getMessage()]);
                            break;
                        }
            ...
            sleep(5);
            $progress_log['progress'] = "90%";
            $progress_log['msg'] = '插件升级开始';
            $progress_log['status'] = 200;
            $this->updateProgress(json_encode($progress_log));

            require_once IDCSMART_ROOT.'app/upgrade/upgradePlugin.php';
        }

        //sleep(10);
        //$this->updateConfiguration('system_version', $last_version);
        $this->updateConfiguration('executed_update', 1);
        if(empty($this->configuration('system_version_type'))){
            $this->updateConfiguration('system_version_type', 'beta');
        }
        $this->updateConfiguration('system_version_type_last', $this->configuration('system_version_type'));
        ...
        $this->deleteDir(WEB_ROOT.'/upgrade', ['api','css','img','js','lang','utils','update.html','upgrade.log','upgrade.php']);

        \think\facade\Cache::delete('pluginLang_zh-cn');
        \think\facade\Cache::delete('pluginLang_en-us');
        \think\facade\Cache::delete('pluginLang_zh-hk');
    ...
    public function updateProgress($progress, $type = 'progress')
    {
        if($type=='file'){
            ...
            file_put_contents($this->plugin_upgrade_log,$progress."\n");
        }else{
    ...
    public function updateConfiguration($name, $value)
    {
        ...
        if(!empty($configuration)){
            ...
            $db->exec("INSERT INTO {$database['prefix']}configuration (`setting`,`value`,`create_time`,`description`) VALUES('{$name}','{$value}',{$time},'')");
        }
        return true;
    }
    ...
    # 公共curl
    public function curl($url, $data = [], $timeout = 300, $request = 'POST', $header = [])
    {
        $curl = curl_init();
