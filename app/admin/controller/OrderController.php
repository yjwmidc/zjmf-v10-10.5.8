...
class OrderController extends AdminBaseController
{
     ...
     * @return string order.status - 状态Unpaid未付款Paid已付款Cancelled已取消Refunded已退款WaitUpload待上传WaitReview待审核ReviewFail审核失败
     ...
     * @return string order.refund_credit - 已退款余额
     * @return string order.refund_gateway - 已退款渠道
     * @return string order.gateway_sign - 支付接口标识(credit=余额,credit_limit=信用额)
     ...
     * @param   float amount - 退款金额 require
     * @param   string type - 退款类型(credit_first=余额优先,gateway_first=渠道优先,credit=余额,transaction=支付接口) require
     * @param   string notes - 备注
     * @param   string gateway - 支付接口 requireIf:type=transaction
     * @return  int id - 退款记录ID
     */
     ...
     * @param   int page - 页数
     * @param   int limit - 每页条数
     * @param   string host_status - 产品状态Unpaid未付款Pending开通中Active已开通Suspended已暂停Deleted已删除Failed开通失败Cancelled已取消
     * @param   string refund_status - 退款状态(Pending=待审核,Reject=已拒绝,Refunding退款中,Refunded已退款)
     * @param   string keywords - 关键字:商品名称,产品标识,产品IP
     ...
     * @method POST
     * @param   int id - 订单ID require
     * @param   int pass - 审核状态(0=不通过,1=通过) require
     * @param   string review_fail_reason - 审核失败原因 requireIf,pass=0
     * @param   string transaction_number - 流水号 requireIf,pass=1
     */
     ...
     * @method GET
     * @param  int id - 订单ID require
     * @param  string keywords - 关键字:商品名称,产品标识,备注
