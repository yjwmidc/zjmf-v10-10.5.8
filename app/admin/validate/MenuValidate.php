    ...
    public function checkHomeMenu($value)
    {
        ...
        foreach ($value as $k => $v) {
            ...
            if($v['type']=='custom'){
                ...
                if(!is_string($v['module']) || !is_array($v['res_module'])){
                ...
                if($v['module'] == 'mf_cloud' && !empty($v['res_module']) && !empty(array_diff($v['res_module'], $resCloud))){
                    return false;
                }
                if($v['module'] == 'mf_dcim' && !empty($v['res_module']) && !empty(array_diff($v['res_module'], $resDcim))){
                    return false;
                }
                // 也可以只选择云和dcim的代理
            ...
            if(!empty($v['child'])){
                foreach ($v['child'] as $ck => $cv) {
                    ...
                    if($cv['type']=='custom'){
                        ...
                        if(!is_string($cv['module']) || !is_array($cv['res_module'])){
                        ...
                        if($cv['module'] == 'mf_cloud' && !empty($cv['res_module']) && !empty(array_diff($cv['res_module'], $resCloud))){
                            return false;
                        }
                        if($cv['module'] == 'mf_dcim' && !empty($cv['res_module']) && !empty(array_diff($cv['res_module'], $resDcim))){
                            return false;
                        }
                        // 也可以只选择云和dcim的代理
