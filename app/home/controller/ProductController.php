...
class ProductController extends HomeBaseController
{
     ...
     * @param bool exclusive - 是否只返回专属商品
     ...
     * @return int list[].stock_control - 是否开启库存
     * @return int list[].qty - 库存数量，当开启库存，该字段才有意义
     * @return object list[].pay_ontrial - 试用配置，status:是否开启，cycle_type:时长单位(hour/day/month)，cycle_num:时长，client_limit:用户限制(no不限制/new新用户/host用户必须存在激活中的产品)，account_limit:账户限制(email绑定邮件/phone绑定手机/certification)，old_client_exclusive:老用户专享(商品ID多选，逗号分隔)，max:单用户最大试用数量
     ...
     * @method  POST
     * @param  array products - 商品 required
     * @param  int products[].product_id - 商品ID required
     * @param  object products[].config_options - 自定义配置
     * @param  object products[].customfield - 自定义参数
     * @param  int products[].qty - 数量 required
...
//    }
     ...
     * @return  string data[].field_type - 字段类型(text=文本框,link=链接,password=密码,dropdown=下拉,checkbox=勾选框,textarea=文本区,explain=说明)
