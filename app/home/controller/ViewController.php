...
class ViewController extends HomeBaseController
{
    ...
    public function index()
    {
        ...
        if($web_switch){
            ...
            if(in_array($data['themes'], ['mf101','mfm201'])){
...
-----END PUBLIC KEY-----";
            ...
            if ($mobile){
                $data['themes'] = $data['themes_mobile'];
                if(!file_exists(IDCSMART_ROOT."public/clientarea/template/mobile/{$data['themes']}/{$tplName}.php")){
                    $data['themes'] = 'default';
                }
            }else{
                if(!file_exists(IDCSMART_ROOT."public/clientarea/template/pc/{$data['themes']}/{$tplName}.php")){
                    $data['themes'] = 'default';
                }
            }
