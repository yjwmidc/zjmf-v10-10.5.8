...
class ProductController
{
     ...
     * @return string list[].mode - 代理模式：only_api仅调用接口，sync同步商品，空表示非代理商品
     * @return int list[].stock_control - 是否库存控制：1是，0否
    ...
    public function index()
    {
        ...
        custom_host_name_string_allow,custom_host_name_string_length,renew_rule,stock_control,qty')
            ->where('hidden', 0)
            ->where('agentable', 1)
            ->where('id', $id)
            ->find();
     ...
     * @version v1
     * @param   int id - 商品ID require
     * @return  array plugin - 插件列表
     * @return  string plugin[].module - 插件模块
     * @return  string plugin[].name - 插件标识
     * @return  string plugin[].url - zip包完整下载路径
     * @return  string plugin[].version - 版本号
     */
