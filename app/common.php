...
function amount_format($amount, $scale = 2){
    $amount = (float)bcdiv($amount, 1, $scale);
...
function get_client_ip($type = 0, $adv = true)
{
    ...
    if ($ip) {
        if ($ipWhiteList === null) {
            ...
            $ipWhiteList = array_filter(array_map('trim', $list), function($ip) {
        ...
        if (count($ipList)==1){
            // 代理ip是否在白名单
            if (in_array(request()->ip($type, $adv),$ipWhiteList)){
            ...
            foreach ($ipList as $index => $proxyIp) {
                ...
                if ($index > 0 && in_array($proxyIp, $ipWhiteList)) {
