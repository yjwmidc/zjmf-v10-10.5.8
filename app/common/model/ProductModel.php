...
//                }
    ...
    public function indexProduct($id)
    {
        ...
        pay_type,auto_setup,type,rel_id,product_id,price,cycle,renew_rule,show_base_info,product_group_id,pay_ontrial')
            ->find($id);
            ...
            for ($i=1; $i<=$param['qty']; $i++) {
                ...
                foreach ($param['sub_host'] as $subHost){
                    $host = HostModel::create([
                        ...
                        'renew_amount' => ($product['pay_type']=='recurring_postpaid' || $product['pay_type']=='recurring_prepayment') ? $subHost['renew_price'] : 0,
                        'billing_cycle' => $product['pay_type'],
                        ...
                        'ratio_renew' => self::isRenewByRatio(['product_group_id'=>$product['product_group_id'],'server_id'=>$serverId]),
                        'base_renew_amount' => ($product['pay_type']=='recurring_postpaid' || $product['pay_type']=='recurring_prepayment') ? $subHost['renew_price'] : 0,
...
//                    if(isset($result['data']['sub_host'])){
//                        foreach ($result['data']['sub_host'] as $key=>$subHost)
//                        {
//                            $hookPromoCodeResultsOrgins = hook('apply_promo_code', [
//                                    'host_id'=>0,
//                                    'price'=>$subHost['price'],
...
//                                if ($hookPromoCodeResultsOrgin['status']==200){
//                                    $promocodeDiscount = $hookPromoCodeResultsOrgin['data']['discount']??0;
//                                    $result['data']['price_promo_code_discount'] = bcadd($result['data']['price_promo_code_discount'],$promocodeDiscount,2);
//                                    $result['data']['price_total'] = bcsub($result['data']['price_total'],$promocodeDiscount,2);
//                                }
//                            }
//                            // 续费单独处理
//                            $hookPromoCodeResultsOrginsRenew = hook('apply_promo_code', [
//                                    'host_id'=>0,
//                                    'price'=>$subHost['renew_price'],
//                                    'scene'=>'new',
//                                    'qty'=>1,
//                                    'duration'=>$result['data']['duration']??0,
//                                    'product_id'=>$param['product_id'],
...
//                                'client_id'=>get_client_id(),
//                                'product_id'=>$param['product_id'],
//                                'amount'=>bcmul($subHost['price'], $qty, 2)
//                            ]);
//                            foreach ($hookClientLevelResultsOrgins as $hookClientLevelResultsOrgin){
//                                if ($hookClientLevelResultsOrgin['status']==200){
//                                    $clientLevelDiscount = $hookClientLevelResultsOrgin['data']['discount']??0;
//                                    $result['data']['price_client_level_discount'] = bcadd($result['data']['price_client_level_discount'],$clientLevelDiscount,2);
//                                    $result['data']['price_total'] = bcsub($result['data']['price_total'],$clientLevelDiscount,2);
//                                }
//                            }
//                            // wyh 20240521 续费单独处理
...
//                                    // 活动促销已计算数量
//                                    $eventPromotionDiscount = $hookEventPromotionResultsOrgin['data']['discount']??0;
//                                    $result['data']['price_event_promotion_discount'] = bcadd($result['data']['price_event_promotion_discount'],$eventPromotionDiscount,2);
//                                    $result['data']['price_total'] = bcsub($result['data']['price_total'],$eventPromotionDiscount,2);
//                                }
//                            }
//                            // 续费单独处理
//                            $hookEventPromotionResultsOrginsRenew = hook("event_promotion_by_amount",[
...
//                    }
    ...
    public function productListSearch($param)
    {
       ...
       $app = app('http')->getName();
        ...
        if($app!='home'){
            foreach ($products as $key => $value) {
                $products[$key]['agent'] = (!empty($value['upstream_product_id']) && $value['mode']=='only_api') ? 1 : 0;
