...
class SmsTemplateModel extends Model
{
    ...
    {
        $sms_template = $this->field('id,template_id,type,title,content,notes,status,sms_name,product_url,remark')
		->where('sms_name',$name)
		->select()
		->toArray();		
    ...
    {
        $sms_template = $this->field('id,template_id,type,title,content,notes,status,product_url,remark')
		->where('id',$param['id'])
    ...
    public function createSmsTemplate($param)
    {
		...
		if($param['name']=='Aliyun'){
			if(!isset($param['product_url'])){
				return ['status'=>400, 'msg'=>lang('aliyun_sms_template_product_url_cannot_empty')];
			}
			if(empty($param['product_url'])){
				return ['status'=>400, 'msg'=>lang('aliyun_sms_template_product_url_cannot_empty')];
			}
			if(strlen($param['product_url'])>255){
				return ['status'=>400, 'msg'=>lang('aliyun_sms_template_product_url_error')];
			}

			if(!isset($param['remark'])){
				return ['status'=>400, 'msg'=>lang('aliyun_sms_template_remark_cannot_empty')];
			}
			if(empty($param['remark'])){
				return ['status'=>400, 'msg'=>lang('aliyun_sms_template_remark_cannot_empty')];
			}
			if(strlen($param['remark'])>255){
				return ['status'=>400, 'msg'=>lang('aliyun_sms_template_remark_error')];
			}
		}
        ...
        try {
            $sms_id =$this->create([
                ...
                'product_url' => isset($param['product_url']) ? trim($param['product_url']) : '',
    ...
    public function updateSmsTemplate($param)
    {
		...
		if($param['status']!=2){
			if($param['status']==0){
				...
				if(!empty($sms_methods['status']) && ($sms_methods['status']==400 || $sms_methods['status']=='error')){
                    $sms_methods['status'] = 400;
					return $sms_methods;
				}
				if(!empty($sms_methods['status']) && $sms_methods['status']=='success'){
					$param['template_id']=$sms_methods['template']['template_id']?:'';
