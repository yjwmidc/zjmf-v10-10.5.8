...
class SelfDefinedFieldModel extends Model
{
     ...
     * @param   string param.field_type - 字段类型(text=文本框,link=链接,password=密码,dropdown=下拉,tickbox=勾选框,textarea=文本区,explain=说明) require
     ...
     * @param   string param.explain_content - 说明内容 field_type=explain可用
    ...
    public function selfDefinedFieldCreate($param)
    {
        ...
        // 说明类型时
        if($param['field_type'] == 'explain'){
            $param['is_required'] = 0;
            $param['description'] = '';
            $param['regexpr'] = '';
            $param['show_order_page'] = 1;
            $param['show_order_detail'] = 0;
            $param['show_client_host_detail'] = 0;
            $param['show_admin_host_detail'] = 0;
            $param['show_client_host_list'] = 0;
            $param['show_admin_host_list'] = 0;
            $param['explain_content'] = $param['explain_content'] ?? '';
        }else{
            $param['explain_content'] = '';
        }

        $selfDefinedField = $this->create($param, ['type','relid','field_name','is_required','field_type','description','regexpr','field_option','show_order_page','show_order_detail','show_client_host_detail','show_admin_host_detail','show_client_host_list','show_admin_host_list','create_time','explain_content']);
        ...
        }else if($param['type'] == 'product_group'){
            $description = lang('log_product_group_self_defined_field_create_success', [
                '{field_name}'  => $param['field_name'],
            ]);
    ...
    public function selfDefinedFieldUpdate($param)
    {
        ...
        $this->update($param, ['id'=>$selfDefinedField['id'] ], ['field_name','is_required','field_type','description','regexpr','field_option','show_order_page','show_order_detail','show_client_host_detail','show_admin_host_detail','show_client_host_list','show_admin_host_list','update_time','explain_content']);
        ...
        $fieldType = [
            ...
            'explain'   => lang('self_defined_field_type_explain'),
        ];
        ...
        if(!empty($description)){
            if($selfDefinedField['type'] == 'product'){
                ...
                active_log($description, 'product', $product->id);
            }else if($selfDefinedField['type'] == 'product_group'){
                $description = lang('log_product_group_self_defined_field_update_success', [
                    '{detail}'  => implode(',', $description),
                ]);
                active_log($description);
            }
     ...
     * @author hh
     * @version v1
     * @param   string param.type product 类型(product=商品,product_group商品组)
     * @param   int param.relid - 关联ID(商品ID) require
    ...
    public function selfDefinedFieldList($param)
    {
        ...
        $list = $this
                ->field('id,field_name,field_type,description,regexpr,field_option,is_required,show_order_page,show_order_detail,show_client_host_detail,show_admin_host_detail,show_client_host_list,show_admin_host_list,upstream_id,explain_content')
                ->where($where)
                ->order('order,id', 'asc')
                ->select()
                ->toArray();
        ...
        if(!empty($list) && $param['type']=='product_group'){
            $SelfDefinedFieldLinkModel = new SelfDefinedFieldLinkModel();
            $link = $SelfDefinedFieldLinkModel->alias('a')
                ->field('a.self_defined_field_id,b.id,c.name first_group_name,b.name')
                ->leftJoin('product_group b', 'b.id=a.product_group_id')
                ->leftJoin('product_group c', 'c.id=b.parent_id')
                ->whereIn('a.self_defined_field_id', array_column($list, 'id'))
                ->select()
                ->toArray();
            $linkArr = [];
            foreach ($link as $key => $value) {
                if(!isset($linkArr[$value['self_defined_field_id']])){
                    $linkArr[$value['self_defined_field_id']] = [];
                }
                $linkArr[$value['self_defined_field_id']][] = ['id' => $value['id'], 'first_group_name' => $value['first_group_name'], 'name' => $value['name']];
            }
    ...
    public function selfDefinedFieldDelete($param)
    {
        ...
        if($selfDefinedField['type'] == 'product'){
            ...
            $description = lang('log_product_group_self_defined_field_delete_success', [
                '{field_name}'  => $selfDefinedField['field_name'],
            ]);
    ...
    public function withDelete($type, $relid)
    {
        ...
        if(!empty($selfDefinedFieldId)){
            ...
            SelfDefinedFieldLinkModel::whereIn('self_defined_field_id', $selfDefinedFieldId)->delete();
        }
    }
     ...
     * @return  string data[].field_type - 字段类型(text=文本框,link=链接,password=密码,dropdown=下拉,tickbox=勾选框,textarea=文本区,explain=说明)
     ...
     * @return  string data[].explain_content - 说明内容
     */
    public function showOrderPageField($param)
    {
        if(isset($param['need_upstream_id']) && $param['need_upstream_id']){
            $field = 'id,field_name,field_type,description,regexpr,field_option,is_required,show_client_host_list,upstream_id,explain_content';
        }else{
            $field = 'id,field_name,field_type,description,regexpr,field_option,is_required,show_client_host_list,explain_content';
        }
        $data = $this
                ->field($field)
        ...
        foreach($data as $k=>$v){
            $multiLanguage = hook_one('multi_language', [
                'replace' => [
                    ...
                    'explain_content'   => $v['explain_content'],
                ],
            ]);
            $data[$k]['field_name'] = $multiLanguage['field_name'] ?? $v['field_name'];
            $data[$k]['description'] = $multiLanguage['description'] ?? $v['description'];
            $data[$k]['explain_content'] = $multiLanguage['explain_content'] ?? $v['explain_content'];
        }
        return ['data'=>$data];
    }
    ...
    public function showOrderDetailField($param)
    {
        ...
        $data = SelfDefinedFieldValueModel::alias('sdfv')
                ...
                ->withAttr('field_name', function($value){
                    $multiLanguage = hook_one('multi_language', [
                        'replace' => [
                            'field_name'    => $value,
                        ],
                    ]);
                    $value = $multiLanguage['field_name'] ?? $value;
                    return $value;
                })
        ...
        if($param['type'] == 'whmcs'){
            ...
            foreach($param['self_defined_field'] as $v){
                $data = [
                    ...
                    'show_order_page'           => 1,
                    'show_order_detail'         => $v['field_type'] == 'explain' ? 0 : (!in_array($v['field_type'], ['link','password']) ? 1 : 0),
                    'show_admin_host_detail'    => $v['field_type'] == 'explain' ? 0 : 1,
                    'show_client_host_detail'   => $v['field_type'] == 'explain' ? 0 : 1,
    ...
    public function getHostListSelfDefinedFieldValue($param)
    {
        ...
        $selfDefinedField = $this
                            ...
                            ->where($where)
                            ->withAttr('field_name', function($val){
