...
class OrderModel extends Model
{
    ...
    public function orderList($param, $scene = '')
    {
        ...
        $param['orderby'] = isset($param['orderby']) && in_array($param['orderby'], ['id','amount','client_id','reg_time']) ? $param['orderby'] : 'id';
        ...
        $where = function (Query $query) use ($param, $app, $scene, $selectDataRange){
            ...
            if(!empty($param['keywords'])){
                $query->where('o.id|p.name|c.username|c.email|c.phone|h.name', 'like', "%{$param['keywords']}%");
            }
            if(!empty($param['type'])){
            ...
            if($scene != 'recycle_bin'){
                ...
                if(!empty($selectDataRange)){
                    ...
                    foreach ($selectDataRange as $v) {
                        ...
                        }else if(in_array($v['key'], ['order_time', 'reg_time'])){
                            if($v['rule']=='equal'){
        ...
        $count = $this->alias('o')
            ...
            ->leftjoin('host h', 'o.id=h.order_id')
            ->where($where)
            ->whereOr($whereOr)
            ->group('o.id')
            ->count();
        ...
        if($app!='home'){
            $totalAmount = $this->alias('o')
                ...
                ->select()
                ->toArray();
            $totalAmount = amount_format(array_sum(array_column($totalAmount, 'amount')));
        }
    ...
    public function indexOrder($id)
    {
        ...
        $order = $this->field('id,type,amount,create_time,pay_time,status,gateway_name gateway,gateway gateway_sign,credit,client_id,notes,refund_amount,amount_unpaid,admin_id,is_recycle,voucher,review_fail_reason')->find($id);
        if (empty($order)){
            return (object)[]; // 转换为对象
        }
...
//        }
    ...
    public function orderRefund($param)
    {
        ...
        // 信用额不能订单退款
        if(in_array($order['type'], ['recharge','credit_limit','combine'])){
    ...
    public function hostOrderHandle($id)
    {
        ...
        $host = $HostModel->alias('h')
            ->field('h.id,h.order_id,h.client_id,h.status,h.billing_cycle,h.billing_cycle_name,h.name,h.billing_cycle_time,p.auto_setup,h.order_id,p.name as product_name,h.active_time,h.due_time')
        ...
        foreach($orderItem as $v){
            ...
            if($param['host_id'] > 0 && $v['host_id'] == $param['host_id']){
                ...
                $hostOrderItem[] = [
                    ...
                    'name'          => $isHost ? $host['name'] : '',
                    'status'        => $isHost ? $host['status'] : '',
                    'amount'        => $v['amount'],
