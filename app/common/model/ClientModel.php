...
class ClientModel extends Model
{
    ...
    public function clientList($param)
    {
        ...
        if($param['orderby'] == 'host_active_num'){
            $hostActiveNum = $this
                ...
                ->alias('c')
                ->where($where)
                ->leftJoin('host h', 'c.id=h.client_id AND h.status="Active" AND h.is_delete=0')
                ->limit($param['limit'])
                ->page($param['page'])
            ...
            $hostNum = $this
                ...
                ->alias('c')
                ->where($where)
                ->leftJoin('host h', 'c.id=h.client_id AND h.is_delete=0')
                ->limit($param['limit'])
                ->page($param['page'])
                ->order($param['orderby'], $param['sort'])
                ->group('c.id')
        ...
        if(isset($ids) && !empty($ids)){
            ...
            if(isset($selectField['oauth'])){
                ...
                $oauthList = $OauthModel->field('client_id,type')->whereIn('client_id', $clientId)->select()->toArray();
                $oauth = [];
            ...
            if(!isset($hostActiveNum)){
                $hostActiveNum = HostModel::field('COUNT(id) num,client_id')->where('status', 'Active')->whereIn('client_id', $clientId)->where('is_delete', 0)->group('client_id')->select()->toArray();
                $hostActiveNum = array_column($hostActiveNum, 'num', 'client_id');
            }
            if(!isset($hostNum)){
                $hostNum = HostModel::field('COUNT(id) num,client_id')->whereIn('client_id', $clientId)->where('is_delete', 0)->group('client_id')->select()->toArray();
    ...
    public function indexClient($id)
    {
        ...
        $client = $this->field('id,username,email,phone_code,phone,company,country_id,address,language,notes,client_notes,status,create_time register_time,last_login_time,last_login_ip,credit,operate_password,notice_open,notice_method,receive_sms,receive_email')->find($id);
        if (empty($client)){
            return (object)[]; // 转换为对象
        }
        ...
        if($app=='admin'){
            ...
            $client['host_num'] = HostModel::where('client_id', $id)->where('is_delete', 0)->count();  // 产品数量
            $client['host_active_num'] = HostModel::where('status', 'Active')->where('client_id', $id)->where('is_delete', 0)->count(); // 已激活产品数量
    ...
    private function passwordLogin($param)
    {
        ...
        // wyh 20240529 临时处理，开启操作密码验证并输入了操作密码，不验证图形验证码(导致开了操作密码验证，可绕过图形验证码)
        ...
        $verifyOperatePassword = in_array('operate_password', $homeLoginIpExceptionVerify) && isset($param['client_operate_password']) && !empty($param['client_operate_password']);
...
//            }
    ...
    private function emailLogin($param)
    {
        ...
        // 正常登录日志
        $log = lang('log_client_login',['{client}'=>'client#'.$client->id.'#'.$client->username.'#']);
        ...
        if(!$isCommonIpLogin){
            ...
            if(in_array('operate_password', $homeLoginIpExceptionVerify) && !empty($client->operate_password)){
                // 验证操作密码
                if(!isset($param['client_operate_password']) || $param['client_operate_password'] === ''){
                    return [
                        'status'=> 400,
                        'msg'   => lang('login_without_common_ip_need_verify'),
                        'data'  => [
                            'ip_exception_verify' => $homeLoginIpExceptionVerify,
                        ]
                    ];
                }
                if(!idcsmart_password_compare($param['client_operate_password'], $client->operate_password)){
                    $request = request();
                    $request->client_id = $client->id;
                    $request->client_name = $client['username'];
                    active_log(lang('log_client_login_operate_password_error', ['{client}'=>'client#'.$client->id.'#'.$client->username.'#']),'login',$client->id);
                    return ['status'=>400, 'msg'=>lang('operate_password_error') ];
                }else{
                    // 通过操作密码的日志
                    $log = lang('log_client_login_operate_password_success',['{client}'=>'client#'.$client->id.'#'.$client->username.'#']);
                }
            }
        }
