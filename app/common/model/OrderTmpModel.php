...
class OrderTmpModel extends Model
{
    ...
    public function pay($param)
    {
        ...
        $hookRes = hook('before_order_pay', ['order_id'=>$id, 'gateway'=>$param['gateway'], 'order'=>$order ]);
        ...
        if ($order->amount_unpaid <= 0 || $param['gateway']=='credit'){
            ...
            try{
                ...
                if (($client->credit >= $order->credit && $order->amount_unpaid <= 0) || ($client->credit >= $order->amount && $param['gateway']=='credit')){
                    $order = $OrderModel->lock(true)->find($id);
                    # 订单已支付
                    if ($order->status == 'Paid'){
                        throw new \Exception(lang('order_is_paid'));
                    }
                    $order->save([
                        'gateway' => $gateway,
    ...
    private function startPay($id,$gateway)
    {
        # 客户基础信息
        $ClientModel = new ClientModel();
        $client = $ClientModel->field('id,username,company,address,country_id,phone_code,phone,email')->find(get_client_id());
        if(!empty($client)){
            ...
            $country = CountryModel::where('id', $client['country_id'])->value($countryName);
        ...
        if ($type == 'recharge'){
            $productName = [
                lang('client_recharge',['{client_id}'=>get_client_id()])
            ];
        }else{
            $OrderItemModel = new OrderItemModel();
            $products = $OrderItemModel->alias('oi')
                ->field('p.name')
                ->leftjoin('host h','oi.rel_id=h.id AND h.is_delete=0')
    ...
    {
        # 未开启充值功能
        if (!$this->isAdmin && !configuration('recharge_open')){
            return ['status'=>400,'msg'=>lang('recharge_is_not_open')];
        }

        if (!$this->isAdmin && check_certification_recharge() && !check_certification(get_client_id())){
            return ['status'=>400,'msg'=>lang('client_credit_no_certification_recharge')];
        }
        ...
        try{
            ...
            $data = [
                ...
                'items' => [
                    [
                        'amount' => $param['amount'],
                        'description' => lang('client_recharge',['{client_id}'=>$this->isAdmin?($param['client_id']??0):get_client_id()]),
                        'type' => 'recharge',
                    ]
                ]
            ];
        ...
        # 后台直接标记支付
        if ($this->isAdmin){
            $OrderModel->orderPaid(['id'=>$orderId, 'transaction_number' => $param['transaction_number'] ?? '','notes'=>$param['notes']??'']);
        }
    ...
    public function credit($param)
    {
        ...
        if ($order->status != 'Unpaid'){
            return ['status'=>400,'msg'=>lang('order_use_credit_status_must_be_unpaid')];
        }
        # 充值订单不可使用余额
        ...
        if (!$use){
            ...
            try{
                # 客户余额为0
                $ClientModel = new ClientModel();
                $client = $ClientModel->where('id',$order->client_id)->lock(true)->find();
                if ($client->credit <= 0){
                    throw new \Exception(lang('client_credit_is_0'));
                }
                // 处理并发问题，重新获取订单数据
    ...
    public function orderPayHandle($param)
    {
        ...
        if (empty($tmpOrder)){
            active_log(lang('log_pay_tmp_order_miss',['{tmp_order_id}'=>$tmpOrderId]),'order_tmp',$tmpOrderId);
            return false;
        }
        ...
        if (empty($order) || $order['is_recycle']){
            active_log(lang('log_pay_order_miss',['{order_id}'=>$tmpOrder->order_id]),'order',$tmpOrder->order_id);
            return false;
        }
        ...
        if ($flag){
            ...
            // wyh 20240403 (不能随便删除)不删除临时订单，需要处理关联关系
            // $tmpOrder->delete();
            # 处理订单类型
     ...
     * @return string
     */
    public function getGatewayReturnUrl($tmpOrderId)
    {
        $domain = configuration('website_url');

        $defaultUrl = "{$domain}/home.htm";
        ...
        return redirect((isset($order['return_url']) && !empty($order['return_url']))?$order['return_url']:$defaultUrl);
    }
