...
class MenuModel extends Model
{
     ...
     * @return array menu[].res_module - res模块类型
     ...
     * @return array menu[].child[].res_module - res模块类型
    ...
    public function getHomeMenu()
    {
        ...
        $menus = $this->field('id,menu_type type,name,language,url,second_reminder,icon,nav_id,parent_id,module,product_id,res_module')
            ->where('type', 'home')
            ->order('order','asc')
     ...
     * @version v1
     * @param array menu - 菜单 required
     * @param string menu[].type - 菜单类型system系统plugin插件custom自定义module产品列表embedded内嵌 required
     * @param string menu[].name - 名称 required
     * @param object menu[].language - 语言 required
     * @param string menu[].url - 网址 菜单类型为自定义时需要传递
     ...
     * @param string menu[].module - 模块类型 菜单类型为产品列表时可以传递
     * @param array menu[].res_module - res模块类型 菜单类型为产品列表时可以传递,云(whmcs_cloud,mf_cloud,mf_finance),DCIM(whmcs_dcim,mf_dcim,mf_finance_dcim)可以多选
     * @param array menu[].product_id - 商品ID 菜单类型为产品列表时需要传递
     * @param array menu[].child - 子菜单 required
     * @param string menu[].child[].type - 菜单类型system系统plugin插件custom自定义module产品列表embedded内嵌 required
     * @param string menu[].child[].name - 名称 required
     * @param object menu[].child[].language - 语言 required
     * @param string menu[].child[].url - 网址 菜单类型为自定义时需要传递
     ...
     * @param string menu[].child[].module - 模块类型 菜单类型为产品列表时可以传递
     * @param array menu[].child[].res_module - res模块类型 菜单类型为产品列表时可以传递,云(whmcs_cloud,mf_cloud,mf_finance),DCIM(whmcs_dcim,mf_dcim,mf_finance_dcim)可以多选
     * @param array menu[].child[].product_id - 商品ID 菜单类型为产品列表时需要传递
     */
    public function saveHomeMenu($param)
    {
        ...
        foreach ($param['menu'] as $key => $value) {
            if(in_array($value['type'], ['system', 'plugin'])){
            ...
            }else if($value['type']=='module'){  // 包含模块和res模块
                $moduleProductIds = [];
                $value['module'] = $value['module'] ?? '';
                $value['res_module'] = $value['res_module'] ?? [];
                // 验证模块
                if(!empty($value['module'])){
                    ...
                    $products = ProductModel::alias('p')
                        ...
                        ->whereIn('p.id', $value['product_id'])
                        ->column('p.id');
                    $moduleProductIds = $products;
                }
                // 验证res模块
                if(!empty($value['res_module'])){
                    if(!empty(array_diff($value['res_module'], $resModule))){
                        return ['status' => 400, 'msg' => lang('module_error')];
                    }
                    $products = ProductModel::alias('p')
                        ->leftjoin('upstream_product up','up.product_id=p.id')
                        //->where('p.hidden', 0)
                        ->whereIn('up.res_module', $value['res_module'])
                        ->whereIn('p.id', $value['product_id'])
                        ->column('p.id');
                    $moduleProductIds = array_merge($moduleProductIds, $products);
                }
                $param['menu'][$key]['product_id'] = $moduleProductIds;
            }
            if(isset($value['child'])){
                foreach ($value['child'] as $k => $v) {
                    if(in_array($v['type'], ['system', 'plugin'])){
                    ...
                    }else if($v['type']=='module'){  // 包含模块和res模块
                        $moduleProductIds = [];
                        $v['module'] = $v['module'] ?? '';
                        ...
                        if(!empty($v['module'])){
                            ...
                            ->leftjoin('server ss','ss.server_group_id=sg.id')
                            //->where('p.hidden', 0)
                            ->where('s.module|ss.module', $v['module'])
                            ->whereIn('p.id', $v['product_id'])
                            ->column('p.id');
                            $moduleProductIds = $products;
                        }
                        // 验证res模块
                        if(!empty($v['res_module'])){
                            if(!empty(array_diff($v['res_module'], $resModule))){
                                return ['status' => 400, 'msg' => lang('module_error')];
                            }
                            $products = ProductModel::alias('p')
                            ->leftjoin('upstream_product up','up.product_id=p.id')
                            //->where('p.hidden', 0)
                            ->whereIn('up.res_module', $v['res_module'])
                            ->whereIn('p.id', $v['product_id'])
                            ->column('p.id');
                            $moduleProductIds = array_merge($moduleProductIds, $products);
                        }
                        $param['menu'][$key]['child'][$k]['product_id'] = $moduleProductIds;
                    }
                }
            }
        }
        ...
        try {
            ...
            foreach ($param['menu'] as $key => $value) {
                $menu = $this->create([
                    ...
                    'parent_id'         => 0,
                    'module'            => $value['module'] ?? '',
                    'res_module'        => isset($value['res_module']) ? json_encode(array_values($value['res_module'])) : '',
    ...
    public function saveChild($type = '', $parentId = 0, $child = [], $order = 0)
    {
        foreach ($child as $k => $v) {
            $menu = $this->create([
                ...
                'parent_id'         => $parentId,
                'module'            => $v['module'] ?? '',
                'res_module'        => isset($v['res_module']) ? json_encode(array_values($v['res_module'])) : '',
        ...
        foreach ($menus as $key => $value) {
            ...
            if(!empty($value['url']) && !in_array($value['url'], $auths) && in_array($value['url'], $urls)){
                if(!empty($value['plugin'])){
                    ...
                    }else if(!empty($pluginUrl[$value['plugin']])){
                        $auth = $AuthModel->where('url', $value['url'])->where('plugin', $value['plugin'])->find();
                        if(!empty($auth['parent_id'])){
                            $auth = $AuthModel->whereIn('url', $pluginUrl[$value['plugin']])->where('parent_id', $auth['parent_id'])->order('order', 'asc')->find();
                            if(!empty($auth)){
                                $value['url'] = $auth['url'];
                                $menus[$key]['url'] = $value['url'];
                            }
                    ...
                    if(in_array('configuration_debug.htm', $auths)){
                        ...
                        $menus[$key]['url'] = $value['url'];
                    }else if(in_array('configuration_api.htm', $auths)){
                        $value['url'] = 'configuration_api.htm';
                        $menus[$key]['url'] = $value['url'];
                    }else if(in_array('configuration_oss.htm', $auths)){
                        $value['url'] = 'configuration_oss.htm';
                        $menus[$key]['url'] = $value['url'];
                    }
                }
                if(!empty($value['url']) && !in_array($value['url'], $auths) && in_array($value['url'], $urls)){
    ...
    public function homeMenu(){
        ...
        if(!empty($clientId)){
            ...
            foreach ($menus as $key => $data) {
                ...
                // wyh 20240424 新增内嵌类型
                if ($data['menu_type']=='embedded'){
                    $menus[$key]['url'] = 'custom_iframe.htm?iframe_url='.$data['url'];
                }
            ...
            $hookWhere = function (Query $query){
                hook('home_menu_plugin_where_query_append',['query'=>$query]);
            };
            $plugins = PluginModel::where('status', 1)->where($hookWhere)->column('name');
            foreach ($menus as $key => $value) {
                if(!empty($value['plugin'])){
                    if(!in_array($value['plugin'], $plugins)){
                        unset($menus[$key]);
            ...
            if(in_array($productId, $value['product_id'])){
                ...
                if($k!==false){
                    ...
                    if(empty($value['product_id'])){
                        ...
                        $value['product_id'] = array_values($value['product_id']);
                        $this->where('id', $value['id'])->update(['product_id' => json_encode($value['product_id'])]);
                    }
                }
            }
        }
