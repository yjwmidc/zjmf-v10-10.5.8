...
class UpstreamOrderModel extends Model
{
     ...
     * @return string list[].gateway_sign - 支付接口标识(credit=余额,credit_limit=信用额)
    ...
    public function orderList($param)
    {
        ...
        $count = $this->alias('a')
            ...
            ->leftjoin('host h', 'h.id=a.host_id AND h.is_delete=0')
            ->leftjoin('product p', 'p.id=h.product_id')
            ->where($where)
            ->group('o.id')
            ->count();
        $orders = $this->alias('a')
            ->field('o.id,o.type,o.create_time,o.amount,sum(a.profit) profit,o.status,o.gateway_name gateway,o.credit,o.client_id,c.username client_name,c.email,c.phone_code,c.phone,c.company,p.name product_name,o.gateway gateway_sign')
            ->leftjoin('order o', 'o.id=a.order_id')
            ->leftjoin('client c', 'c.id=o.client_id')
            ->leftjoin('host h', 'h.id=a.host_id AND h.is_delete=0')
            ->leftjoin('product p', 'p.id=h.product_id')
    ...
    public function sellInfo($param)
    {
        ...
        $total = $this->alias('a')->leftjoin('order o', 'o.id=a.order_id')->where('o.status', 'Paid')->where('o.is_recycle', 0)->where($where)->sum('a.amount');
        $profit = $this->alias('a')->leftjoin('order o', 'o.id=a.order_id')->where('o.status', 'Paid')->where('o.is_recycle', 0)->where($where)->sum('a.profit');
        $productCount = UpstreamProductModel::alias('a')->leftjoin('product p', 'p.id=a.product_id')->where('p.id', '>', 0)->where($where)->count();
        $hostCount = UpstreamHostModel::alias('a')->leftjoin('host h', 'h.id=a.host_id')->where('h.id', '>', 0)->where('h.is_delete', 0)->where($where)->count();
