...
class TransactionModel extends Model
{
     ...
     * @return string list[].transaction_notes - 管理员备注
    ...
    public function transactionList($param)
    {
        ...
        if($app=='home'){
            ...
            if(empty($param['client_id'])){
                ...
                $adminView = $AdminViewModel->adminViewIndex(['id' => $param['view_id'] ?? 0, 'view'=>'transaction']);
        ...
        $where = function (Query $query) use ($param){
            ...
            if(!empty($param['start_time']) && !empty($param['end_time'])){
                $query->where('t.create_time', '>=', strtotime(date('Y-m-d', $param['start_time'])))->where('t.create_time', '<=', strtotime(date('Y-m-d 23:59:59', $param['end_time'])));
            } 
            if(!empty($param['type'])){
                $query->where('o.type', $param['type']);
            }
            if(!empty($param['payment_channel'])){
                $hookRes = hook('search_payment_channels', ['payment_channel' => $param['payment_channel']]);
                $transactionId = [];
                foreach ($hookRes as $key => $value) {
                    if(isset($value['transaction_id']) && !empty($value['transaction_id'])){
                        $transactionId = array_merge($transactionId, $value['transaction_id']);
                    }
                }
                $query->whereIn('t.id', $transactionId);
            }
        ...
        if($app!='home'){
            $totalAmount = $this->alias('t')
                ->field('t.id')
                ->leftjoin('client c', 'c.id=t.client_id')
                ->leftjoin('order o', 'o.id=t.order_id')
                ->where($where)
                ->sum('t.amount');
            $totalAmount = amount_format($totalAmount);
        }
        ...
        $transactions = $this->alias('t')
            ->field('t.id,t.amount,t.gateway_name gateway,t.transaction_number,t.client_id,c.username client_name,c.email,c.phone_code,c.phone,c.company,t.order_id,t.create_time,o.type,c.status client_status,c.create_time reg_time,co.'.$countryName.' country,c.address,c.language,c.notes,t.notes as transaction_notes')
            ->leftjoin('client c', 'c.id=t.client_id')
            ->leftjoin('order o', 'o.id=t.order_id')
        ...
        $pageTotalAmount = amount_format(array_sum(array_column($transactions, 'amount')));

        if(isset($selectField['payment_channel'])){
            $hookRes = hook('get_payment_channels', ['transaction_id' => array_column($transactions, 'id')]);
            $paymentChannel = [];
            foreach ($hookRes as $key => $value) {
        ...
        foreach ($transactions as $key => $transaction) {
            ...
            // 前台接口去除字段
            if($app=='home'){
                unset($transactions[$key]['client_id'], $transactions[$key]['client_name'], $transactions[$key]['email'], $transactions[$key]['phone_code'], $transactions[$key]['phone'], $transactions[$key]['company'], $transactions[$key]['client_status'], $transactions[$key]['reg_time'], $transactions[$key]['country'], $transactions[$key]['address'], $transactions[$key]['language'], $transactions[$key]['notes'], $transactions[$key]['transaction_notes']);
            }
            ...
            // 销售字段
            if(isset($selectField['sale'])){
                $transactions[$key]['sale'] = $sale[ $transaction['client_id'] ]['name'] ?? '';
            }
            ...
            // 销售字段
            if(isset($selectField['payment_channel'])){
                $transactions[$key]['payment_channel'] = $paymentChannel[ $transaction['id'] ]['payment_channel'] ?? '';
            }
        ...
        if($app=='home'){
            ...
            return ['list' => $transactions, 'count' => $count, 'total_amount' => $totalAmount, 'page_total_amount' => $pageTotalAmount];
        }
    }
