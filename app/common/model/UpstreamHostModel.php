    ...
    public function hostList($param)
    {
        ...
        $where = function (Query $query) use($param) {
            ...
            if(!empty($param['start_time']) && !empty($param['end_time'])){
                $query->where('h.create_time', '>=', strtotime(date('Y-m-d', $param['start_time'])))->where('h.create_time', '<=', strtotime(date('Y-m-d 23:59:59', $param['end_time'])));
            }
        ...
        $hosts = $this->alias('a')
            ->field('h.id,h.name,p.name product_name,h.status,h.first_payment_amount,h.renew_amount,h.billing_cycle_name,h.billing_cycle,h.due_time,h.client_id,c.username,c.company,c.email,c.phone_code,c.phone,hi.ip_num,h.base_info')
            ->leftJoin('host h','h.id=a.host_id')
            ->leftjoin('client c','h.client_id=c.id')
            ->leftjoin('product p','p.id=h.product_id')
