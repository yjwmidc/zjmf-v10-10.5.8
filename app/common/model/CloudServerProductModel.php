...
/**
 * @title 模板控制器-云服务器商品模型
 * @desc 模板控制器-云服务器商品模型
 * @use app\common\model\CloudServerProductModel
 */
class CloudServerProductModel extends Model
{
    protected $name = 'cloud_server_product';
    ...
    public function productList($param)
    {
        ...
        $count = $this->field('id')
            ->count();
        $list = $this->alias('a')
            ->field('a.id,a.area_id,b.first_area,b.second_area,a.title,a.description,a.cpu,a.memory,a.system_disk,a.bandwidth,a.duration,a.tag,a.original_price,a.original_price_unit,a.selling_price,a.selling_price_unit,a.product_id')
            ->leftjoin('cloud_server_area b', 'b.id=a.area_id')
            ->limit($param['limit'])
            ->page($param['page'])
     ...
     * @param string param.system_disk - 系统盘 required
    ...
    public function createProduct($param)
    {
        ...
        $area = $CloudServerAreaModel->where('id', $param['area_id'])->find();
        if(empty($area)){
        ...
        try {
            ...
            # 记录日志
            active_log(lang('log_add_cloud_server_product', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#', '{name}' => $param['title']]), 'cloud_server_product', $product->id);
    ...
    public function updateProduct($param)
    {
        ...
        if(empty($product)){
            return ['status'=>400, 'msg'=>lang('cloud_server_product_not_exist')];
        }
        ...
        try {
            ...
            $desc = [
                'area_id'               => lang('cloud_server_product_area_id'),
                'title'                 => lang('cloud_server_product_title'),
                'description'           => lang('cloud_server_product_description'),
                'cpu'                   => lang('cloud_server_product_cpu'),
                'memory'                => lang('cloud_server_product_memory'),
                'system_disk'           => lang('cloud_server_product_system_disk'),
                'bandwidth'             => lang('cloud_server_product_bandwidth'),
                'duration'              => lang('cloud_server_product_duration'),
                'tag'                   => lang('cloud_server_product_tag'),
                'original_price'        => lang('cloud_server_product_original_price'),
                'original_price_unit'   => lang('cloud_server_product_original_price_unit'),
                'selling_price'         => lang('cloud_server_product_selling_price'),
                'selling_price_unit'    => lang('cloud_server_product_selling_price_unit'),
                'product_id'            => lang('cloud_server_product_product_id'),
            ];

            $param['area_id'] = $area['first_area'].'-'.$area['second_area'];
            $old = $CloudServerAreaModel->where('id', $product['area_id'])->find();
            ...
            if(!empty($description)){
                $description = lang('log_update_cloud_server_product', [
                    '{admin}' => 'admin#'.get_admin_id().'#'.request()->admin_name.'#',
                    '{name}'   => $product['title'],
                    '{detail}' => implode(',', $description),
                ]);
                active_log($description, 'cloud_server_product', $product->id);
            }
    ...
    public function deleteProduct($id)
    {
        ...
        try {
            # 记录日志
            active_log(lang('log_delete_cloud_server_product', ['{admin}'=> 'admin#'.get_admin_id().'#'.request()->admin_name.'#', '{name}' => $product['title']]), 'cloud_server_product', $product->id);
     ...
     * @return array list[].children[].children[].system_disk - 系统盘
    ...
    {
        $list = $this->alias('a')
            ->field('a.id,a.area_id,b.first_area,b.second_area,a.title,a.description,a.cpu,a.memory,a.system_disk,a.bandwidth,a.duration,a.tag,a.original_price,a.original_price_unit,a.selling_price,a.selling_price_unit,a.product_id,up.mode')
            ->leftjoin('cloud_server_area b', 'b.id=a.area_id')
            ->leftjoin('upstream_product up','up.product_id=a.product_id')
            ->select()
            ->toArray();
        ...
        foreach ($list as $key => $value) {
            ...
            $product[$value['area_id']][] = [
                ...
                'system_disk' => explode(',', $value['system_disk']),
        ...
        $banners = $CloudServerBannerModel->field('id,img,url,notes')
            ->where('show', 1)
        ...
        $configuration = $ConfigurationModel->cloudServerList();

        if($configuration['cloud_server_more_offers']==1){
            $CloudServerDiscountModel = new CloudServerDiscountModel();
            $discount = $CloudServerDiscountModel->field('id,title,description,url')
                ->select()
                ->toArray();
        }else{
            $discount = [];
        ...
        return ['list' => array_values($first), 'banner' => $banners, 'more_offers' => $configuration['cloud_server_more_offers'], 'discount' => $discount];
