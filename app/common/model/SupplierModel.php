...
class SupplierModel extends Model
{
    ...
    public function supplierList($param)
    {
        ...
        $list = $this->alias('a')
            ->field('a.id,a.type,a.name,a.url,a.currency_code,a.rate,a.auto_update_rate,a.rate_update_time,a.credit')
            ->where($where)
            ->group('a.id')
            ->limit($param['limit'])
            ->page($param['page'])
...
-----END PRIVATE KEY-----';
        ...
        try{
            ...
            if($token!=$param['token']){
                ...
                $hostId = $UpstreamHostModel->where('supplier_id', $param['id'])->where('upstream_host_id', '>', 0)->column('upstream_host_id');

                // 检查状态
                $UpstreamLogic = new UpstreamLogic();
                $res = $UpstreamLogic->upstreamApiAuth(['type' => $param['type'], 'url' => $param['url'], 'username' => $param['username'], 'password' => $param['token'], 'host_id' => $hostId]);
                if($res['status']==400){
    ...
    public function deleteSupplier($id)
    {
        ...
        $hookRes = hook('before_supplier_delete', ['id'=>$id]);
     ...
     * @return int status - 状态码,200成功,400失败
     * @return string msg - 提示信息
     * @return string data.credit - 余额
     */
    public function supplierCredit($id)
    {
        $supplier = $this->find($id);
        if(empty($supplier)){
            return ['status'=>400, 'msg'=>lang('supplier_is_not_exist') ];
        ...
        if($supplier['type'] == 'whmcs'){
            ...
            $path = 'user_info';
            $res = idcsmart_api_curl($id, $path, [], 30, 'GET');
            if($res['status'] == 200){
            ...
            $path = 'console/v1/account';
            $res = idcsmart_api_curl($id, $path, [], 30, 'GET');
            if($res['status'] == 200){
                $credit = $res['data']['account']['credit'] ?? '';
