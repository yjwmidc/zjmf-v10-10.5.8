...
class ResModuleLogic
{
	...
	public function createAccount(HostModel $HostModel): array
	{
        ...
        if ($res['status']==200){
        	...
        	elseif($res['data']['supplier']['type']=='finance'){ // 对接老财务
                ...
                try {
                    $ClientModel->lock(true)->find($HostModel['client_id']);
                    ...
                    if ($res['status']==200){
                        if (isset($res['data']['order_id']) && $res['data']['order_id']){ # 已在上游下单,但未支付
                            ...
                            // 同步，转换参数
                            if ($this->upstreamProduct['mode']=='sync'){
                                $product = (new ProductModel())->find($this->upstreamProduct['product_id']);
                                $configoptions = (new ModuleLogic())->exchangeParams($product,$configoptions,'create_account',$HostModel);
                            }
                            $cartData = [
                            ...
                            if ($res['status']==200){
                                ...
                                if ($res['status']==200){
                                    ...
                                    $UpstreamOrderModel->where('order_id',$HostModel['order_id'])
                                        ->where('host_id',$id)
                                        ->update([
                                            'profit' => Db::raw('amount-'.($res['data']['amount']??0)),
                                        ]);
                    ...
                    $res = ['status'=>400,'msg'=>$e->getMessage()];
                }
	...
	public function terminateAccount(HostModel $HostModel): array
	{
        ...
        if($supplier['type']=='whmcs'){
            ...
            hook('before_module_host_terminate',['id'=>$id]);
	        // TODO
	        $terminateData = [
	 ...
	 * @param   HostModel $HostModel - 产品模型
	 */
	 public function renew(HostModel $HostModel, $orderId=0)
	 {
         $id = $HostModel['id'];
	 ...
	 */
	public function cartCalculatePrice($ProductModel, $params = [], $qty=1, $scene="cal_price",$create=false,$cartSettle=false)
	{
        $apiId = $this->upstreamProduct['supplier_id'];
        ...
        if($supplier['type']=='whmcs'){
	        ...
	        if ($res['status']==200){
                ...
                if ($this->upstreamProduct['profit_type']==1){
                    ...
                    $profitAndBasePercent = $res['data']['profit'] = bcadd(1,$this->upstreamProduct['profit_percent']/100,4);
                ...
                if ($this->upstreamProduct['renew_profit_type']==1){
                    ...
                    $renewProfitAndBasePercent = bcadd(1,$this->upstreamProduct['renew_profit_percent']/100,4);
        ...
        elseif ($supplier['type']=='finance'){
	        ...
	        if ($res['status']==200){
                // 子产品
                if (!empty($res['data']['sub_host']) && is_array($res['data']['sub_host'])){
                    foreach ($res['data']['sub_host'] as &$v){
                        foreach ($v['preview'] as &$v1){
                            $v1['price'] = bcsub($v1['price'],$v['discount'],2);
                            if ($this->upstreamProduct['profit_type']==1){ // 固定利润不处理

                            }else{
                                $profitAndBasePercent = bcadd(1,$this->upstreamProduct['profit_percent']/100,4);
                                $v1['price'] = bcmul($v1['price'],$profitAndBasePercent,2)>0?bcmul($v1['price'],$profitAndBasePercent,2):0;
                            }
                        }
                        unset($v['discount']);
                        $res['data']['price'] = bcsub($res['data']['price'],$v['price']*($cartSettle?$qty:1),2);
                        $res['data']['renew_price'] = bcsub($res['data']['renew_price'],$v['renew_price'],2);
                        if ($this->upstreamProduct['renew_profit_type']==1){

                        }else{
                            $renewProfitAndBasePercent = bcadd(1,$this->upstreamProduct['renew_profit_percent']/100,4);
                            $v['renew_price'] = bcmul($v['renew_price'],$renewProfitAndBasePercent,2)>0?bcmul($v['renew_price'],$renewProfitAndBasePercent,2):0;
                        }
                    }
                }
	            // 去掉不需要的返回
                ...
                if ($this->upstreamProduct['profit_type']==1){
                    // 利润数据，返回给购物车使用
                    $profit = $res['data']['profit'] = bcadd(0, $this->upstreamProduct['profit_percent'], 2);
                    $res['data']['price'] = bcadd($res['data']['price']/$qty, $profit, 2)>0?bcmul(bcadd($res['data']['price']/$qty, $profit, 2),$qty,2):0;
                    $res['data']['base_price'] = bcadd($res['data']['base_price']/$qty,$profit,2)>0?bcmul(bcadd($res['data']['base_price']/$qty,$profit,2),$qty,2):0;
                    $res['data']['price_total'] = bcadd($res['data']['price_total']/$qty,$profit,2)>0?bcmul(bcadd($res['data']['price_total']/$qty,$profit,2),$qty,2):0;
                }else{
                    $profitAndBasePercent = $res['data']['profit'] = bcadd(1,$this->upstreamProduct['profit_percent']/100,4);
                    $res['data']['price'] = bcmul($res['data']['price'],$profitAndBasePercent,2)>0?bcmul($res['data']['price'],$profitAndBasePercent,2):0;
                    $res['data']['base_price'] = bcmul($res['data']['base_price'],$profitAndBasePercent,2)>0?bcmul($res['data']['base_price'],$profitAndBasePercent,2):0;
            ...
            if ($res['status']==200){
                ...
                foreach ($cycles as $cycle){
                    ...
                    if ($this->upstreamProduct['renew_profit_type']==1){
                        ...
                        $price = bcmul($cycle['amount'], bcadd(1,$this->upstreamProduct['renew_profit_percent']/100,4), 2)>0?bcmul($cycle['amount'], bcadd(1,$this->upstreamProduct['renew_profit_percent']/100,2), 2):$formatZero;
                    }
                    $hosts[] = [
            ...
            if ($res['status']==200){
                ...
                foreach ($res['data']['host'] as $item){
                    ...
                    if ($this->upstreamProduct['renew_profit_type']==1){
                        ...
                        $item['profit'] = bcmul($item['price'], bcadd($this->upstreamProduct['renew_profit_percent']/100,0,4), 2);
                        $item['price'] = bcadd($item['price'], $item['profit'], 2)>0?bcadd($item['price'], $item['profit'], 2):$formatZero;
                        $item['base_price'] = bcmul($item['price_save'], bcadd($this->upstreamProduct['renew_profit_percent']/100,1,4), 2)>0?bcmul($item['price_save'], bcadd($this->upstreamProduct['renew_profit_percent']/100,1,4), 2):$formatZero;
    ...
    public function downloadPluginResource($ProductModel){
        $apiId = $this->upstreamProduct['supplier_id'];
        $supplier = SupplierModel::find($apiId);
        if ($supplier['type']=='whmcs'){
            $res = ['status' => 200, 'msg' => lang('success_message')];
        } else if ($supplier['type']=='finance'){
            $res = ['status' => 200, 'msg' => lang('success_message')];
        } else {
            $plugin = [];
            ...
            $res = idcsmart_api_curl($apiId, sprintf('api/v1/product/%d/plugin_resource', $this->upstreamProduct['upstream_product_id']), [] ,30, 'GET');
            if(isset($res['data']['plugin'])){
                foreach ($res['data']['plugin'] as $key => $value) {
                    if(!isset($value['unlimited']) || $value['unlimited']!=1){
                        unset($res['data']['plugin'][$key]);
                    }
                }
                $res['data']['plugin'] = array_merge(array_values($res['data']['plugin']), $plugin);
            }
        ...
        if($supplier['type']=='whmcs'){
            ...
            if ($res['status']==200){
                ...
                $result['data'] = [
                    ...
                    'due_time' => $res['data']['host_data']['nextduedate'] ?? 0,
                ];
                // 匹配操作系统
                foreach($res['data']['config_options'] as $v){
                    if(isset($v['name_k']) && $v['name_k'] == 'os'){
                        $result['data']['image_name'] = $v['sub_name'];
                        $result['data']['image_icon'] = $v['os_group'];
                        break;
                    }
                }
            }else{
            ...
            if ($res1['status']==200 && $res2['status']==200){
                ...
                $result['data'] = [
                    ...
                    'base_info' => $res2['data']['host']['base_info'] ?? '',
                    'due_time' => $res2['data']['host']['due_time'] ?? 0,
                ];
                if(isset($res2['data']['host']['addition']) && !empty($res2['data']['host']['addition'])){
                    $result['data'] = array_merge($res2['data']['host']['addition'], $result['data']);
                }

                $result['data']['other_params'] = $res2['data']['host']['other_params'] ?? [];
            }else{
                $result['status'] = 400;
