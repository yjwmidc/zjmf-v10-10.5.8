...
class UpstreamLogic
{
    ...
    public function upstreamProductDetail($param)
    {
        ...
        if($param['type']=='whmcs'){
            ...
            if(!empty($supplierId)){
                $res = idcsmart_api_curl($supplierId,'api/v1/product/'.$param['id'],['mode'=>$param['mode']??''],30,'GET');
            }else{
                $res = $this->upstreamRequest(rtrim($param['url'],'/').'/api/v1/product/'.$param['id'], ['mode'=>$param['mode']??''], 30, 'GET');
            }
        ...
        return ['data' => $res['data']['product'] ?? [], 'self_defined_field'=>$selfDefinedField,'other_params'=>$otherParams??[]];
    }
