...
class UploadLogic
{
     ...
     */
    public function uploadHandle(UploadedFile $file, $origin = true, $split = '^', $move = false)
    {
        $param = [
            'file' => $file
        ];
        ...
        if ($info instanceof \SplFileInfo) {
            ...
            if(configuration("oss_method")!='LocalOss' && $move){
                $result = $this->moveTo($saveName);
                if (isset($result['error'])){
                    return ['status'=>400,'msg'=>$result['error']];
                }
                $FileLogModel = new FileLogModel();
                $uuid = explode("^",$saveName)[0]??explode(".",$saveName)[0];
                $fileLog = $FileLogModel->where('uuid', $uuid)->find();
                if(!empty($fileLog)){
                    $data['image_url'] = $fileLog['url'];
                }else{
