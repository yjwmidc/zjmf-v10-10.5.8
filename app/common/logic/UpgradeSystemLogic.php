...
class UpgradeSystemLogic
{
     ...
     * @return int expire - 是否即将到期0未到期1即将到期2已到期
     */
    public function getSystemVersion()
    {
        ...
        $expire = 0;
        if(strtotime(configuration('idcsmart_due_time'))>0){
            if((strtotime(configuration('idcsmart_due_time'))-time())>0){
                if((strtotime(configuration('idcsmart_due_time'))-time())<30*24*3600){
                    $expire = 1;
                }
            }else{
                $expire = 2;
